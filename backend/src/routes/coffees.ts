...
// GET /api/coffees/:id/dimensions — numeric dimension ranges from merged cupping scores
...
// GET /api/coffees/:id/ai-summary — AI-generated tasting note
router.get('/:id/ai-summary', async (req, res) => {
  ...
  try {
    ...
    if (!coffeeResult.rows.length) {
      res.status(404).json({ error: 'Coffee not found' }); return;
    }

    const coffee = coffeeResult.rows[0];

    // Get most recent merged overall notes
    ...
    const summary = await getCoffeeSummary({
      ...
      dimensions: dimsResult.rows.map(r => ({

...
- **Alias editing gap, and a correction to the first fix.** Create-alias and edit-rank were built; renaming an existing alias's `platform_name` or toggling `is_active` were never scoped at all. The first pass at fixing this assumed `platform_name` was a per-coffee thing (editable via one coffee's alias row) — it isn't. Checking the seed data confirmed `platform_name` is stored once *per `coffee_alias` row*, so a slot with two fulfillment choices (e.g. "Classic Balanced" — Feather In Cap and Guatemala) has the same name duplicated across two rows. Renaming through a single row would desync the other, the same failure mode as the original `dial_sort_order`/`archetype` duplication this whole reorg started by fixing. Corrected: renaming now happens at the slot level (archetype + live dial position), fanning out to every row that currently belongs to that slot, and the edit control lives on the "Slot Name" column in the Coffees matrix — not buried inside a single coffee's edit form. `is_active` stays genuinely per-coffee (a specific fulfillment choice being active or not), so that one's still edited per-row as first planned.
...
## 6. Categories — decoupling "category" from "archetype" (resolved decision)

This is the decoupling flagged as out-of-scope future work in §4 — now designed. `experimental` was never a true archetype; it's a cross-cutting category, the same kind of thing as Decaf, Half-Caf, or Flavored (none of which were tracked at all before this — several real coffees sit with `archetype = NULL` and no categorization, identified only by name).

**Decisions:**
- **Categories management is embedded in the Coffees page** — no separate admin nav entry. That's already where every other placement decision (archetype, position, alias) happens; categories join the same page rather than fragmenting into a new one.
- **Coffee ↔ category is many-to-many** (`coffee_category_assignment` join table) — a coffee can carry more than one tag. **Archetype and dial position stay singular per coffee**, unchanged, through the existing `archetype_assignments` / `dial_archetype_positions` tables — a category doesn't get its own dial, it borrows whichever real archetype the coffee is genuinely assigned to.
- **A coffee can have a category with no archetype yet, no category with an archetype, both, or neither** — "unplaced" remains a perfectly normal state (matches the existing "Unplaced" section on Coffees already).
- **"Flavored" (Vanilla, Hazelnut, Chocolate) is a category too**, same pattern as Decaf/Half-Caf — a flavored coffee is a real base profile plus an added-flavor tag, structurally identical to decaf being a real profile minus caffeine.
- **Hops can eventually target a category, not just a specific coffee — but only `experimental`.** Decaf/Half-Caf/Flavored are format constraints, not flavor destinations, so a "try this next" hop doesn't make sense for them the way it does for Experimental — they're not connected. Enforced via `coffee_category.is_hoppable` (defaults `false`, only `experimental` set `true`) rather than a hardcoded check, same pattern as `dial_archetype_config.is_archetype` — opening another category to hopping later is a one-row update, not a schema or code change. Given a category doesn't have one dimension score the way a specific coffee does, this is a fuzzier kind of hop and gets its own `hop_type` (`category_hop`) rather than being forced through the `within_archetype`/`bridge_archetype` archetype-consistency logic built for coffee-to-coffee hops.
- **Categories are admin-extensible** (a `coffee_category` table, not a hardcoded list) — seeded with the 4 known today (Experimental, Decaf, Half-Caf, Flavored), more addable later without a schema change.
- **`experimental` is retired as an archetype choice going forward** — `POST /coffees/:id/archetype` rejects it outright once this ships. The `is_archetype = false` flag added earlier only ever guarded suggestion/adjacency logic; this is the actual assignment-time guard.
- **Backfilling the *category tag* for known coffees is mechanical** (we already know Decaf is decaf by name) and ships with the schema. **Backfilling their *archetype*** (Kopi Safari, Decaf, Sleepwalker Half-Caf, Vanilla, Hazelnut, Chocolate all currently have none) **is a cupping/tasting decision**, not something to script — stays a to-do for Dana + Camila.

`CLAUDE_CODE_PROMPT_BLOOM_DIAL_CATEGORIES_DB.md`, in this folder, covers both the database infrastructure (Phase 1 — tables, the `dial_coffee_relationships` extension, the `category_hop` enum value, the assignment-time guard, the mechanical category-tag backfill) **and** the Coffees page UI to manage and apply categories (Phase 2 — Categories panel, per-coffee tagging, category badges in the matrix). Its filename still says "DB" from when it was scoped narrower; content now covers both, rename if it bothers you. Category-*hop* creation (an actual hop with a category endpoint, from the Navigation Hops page) remains out of scope for both phases — `is_hoppable` is infrastructure for that feature, not the feature itself.

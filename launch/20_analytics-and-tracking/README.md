...
| 06 | `06_B3_reporting_views_admin_links.md` | 4 reporting views + `reporting_ro` role + admin Marketing links row | Opus/Fable | Step 02 | ✅ done: views + role live in Cloud SQL, admin row live, reporting_ro in use by Looker Studio connection. SQL is committed in `backend/src/db/schema.sql` (idempotent, runs on every backend startup) — a note here previously said it was never committed, which was wrong; corrected 2026-07-23, see WHAT_WE_BUILT_DB.md for the full note |
...
- [x] /admin shows the Marketing row; cupping cards still present below and correct
- [ ] Standing trio
...
- [x] admin config holds the report URL — all 3 Marketing links (`looker_studio_url`, `adspend_sheet_url`, `mailchimp_audience_url`) set 2026-07-23. Mailchimp: `https://us11.admin.mailchimp.com/audience/contacts?id=111655` (all-contacts view)

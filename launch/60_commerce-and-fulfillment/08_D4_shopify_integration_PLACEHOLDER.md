...
> Global step 08 of 11 · Workstream: commerce-and-fulfillment · Model: **Fable — strongest available, dedicated session, no exceptions** (touches payments; a lost paid order costs more than any tokens) · Depends on: roastery account activated (Aug 17–23) + roastery answers from Dana's calls · Target: Aug 24 – Sep 6 · **This is the launch blocker.**
...
3. Archetype label handoff per the roastery's mechanism. **Clarified 2026-07-18: no per-customer personalization — bags carry the archetype design only**, so this is per-SKU label setup (one SKU per archetype/blend, label pre-uploaded), not per-order data.

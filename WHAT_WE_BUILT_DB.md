...
| `v_coffee` *(added Catalog Blueprint brief 1, 2026-09-13)* | One row per coffee, active or not. `roaster_name` prefers `roaster_id`'s real FK, falling back to the legacy free-text `coffees.roaster` column only when `roaster_id IS NULL` (flagged via `roaster_name_is_fallback`); `match_archetype`/`match_confidence`/`match_source`/`match_session_id` from the coffee's current (non-superseded) `archetype_assignments` row — its flavor identity (D1), never to be confused with placement; `category_codes TEXT[]` aggregated from `coffee_category_assignment`. **As of brief 3**, read via `catalogReads.getCoffee`/`getCoffees` everywhere a coffee's match archetype or owned-coffee identity is needed (D1/D2). |
...
One writer per fact, via a named service function (house rule, see `backend/src/features/catalog_blueprint/README.md`). All catalog writes now go through `backend/src/services/catalogService.ts` — the old inline SQL in `routes/admin.ts` is retired (410, `Catalog Blueprint brief 2`). Legacy tables (`dial_archetype_config`, `dial_slot_alias`, `dial_position_vocabulary`, `dial_archetype_positions`, `coffee_alias`) stay live and unchanged through brief 4 — the one exception is `coffee_alias.is_active`, which `deactivateRoastery`/`reactivateRoastery` still keep in step (documented inline in `catalogService.ts`, removed in brief 5). **As of brief 4 (2026-09-14)**, `routes/admin.ts` itself reads only through `catalogReads`/the views — its own `dial_archetype_positions`/`coffee_alias`/`archetype_assignments` reads (the last consumer of those outside `catalogService.ts`'s roastery-lifecycle cascade) are gone, so the lint allow-list for `routes/admin.ts` (rules 3 and 4) is removed. Brief 5 drops the legacy tables themselves.
...
| Coffee identity + metadata | `coffees.name`, `.roaster_id`, `.roaster` (text, kept in sync), `.origin`, `.origin_region_id`, `.process`, `.roast_level`, `.roast_shade`, `.blend_or_single`, `.flavor_descriptors_roaster`, `.is_active`/`.deactivated_at`/`.deactivation_reason` | `catalogService.createCoffee` / `.updateCoffee` / `.retireCoffee` / `.restoreCoffee` | `v_coffee` |
...
| Slot price | `dial_slot_price.slot_id`, `.retail_price_cents` (+ legacy `.archetype`/`.dial_sort_order` still kept in sync for `admin.ts`'s brief-4-expiring reads) | `catalogService.setSlotPrice` | `v_coffee_sellable_candidate`, `v_coffee_sellable_slot` |
...
| Hop physics | `dial_coffee_relationships.from_coffee_id`, `.to_coffee_id`, `.dimension_id`, `.direction`, `.delta`, `.is_recommended`, `.confidence`, `.notes`, `.hop_type` (provisional stored value, D3) | `catalogService.setHop` / `.removeHop` (retired `POST/DELETE /dial/relationships*` used to write this) | `v_coffee_hop` |
| Hop type (derived, D3) | *(not stored going forward — `dial_coffee_relationships.hop_type` is the legacy stored column, dropped in brief 5)* | n/a — computed from each endpoint's current active home assignment | `v_coffee_hop.hop_type_derived` |
| Roastery activation state | `roaster.is_active`, `.deactivated_at`, `.deactivation_note` + the cascade onto `coffees`/`roaster_blend`/`coffee_alias`/`coffee_slot_assignment` | `catalogService.deactivateRoastery` / `.reactivateRoastery` (moved from inline `routes/admin.ts`, `POST /roasters/:id/deactivate`\|`reactivate` are now thin wrappers) | `v_coffee`, `v_coffee_slot`, `v_coffee_sellable_slot` |
| Dial position signal (Phase 5, dormant) | `dial_position_signal.*` | `dialSuggestion.recordCuppingSignal` — unchanged this brief except moving off pool-level `BEGIN`/`COMMIT` onto `withTransaction` | `v_dial_position_consensus` |

...
**QR Door** *(added HOME_TASK_7, 2026-08-02/03, `SOMMELIER_BUILT.md` S81)* — "never print a URL whose meaning is fixed — print a pointer the server re-aims."
...
- **Known drift, flagged not fixed**: `v_dial_navigation` (the view `sommelierRag.ts` queries via `vdn.from_coffee_id`/`to_coffee_id`) has a checked-in definition in `schema.sql` (from `bloom_dial_seed_2026_06_23.sql`) that only exposes `from_coffee`/`to_coffee` as **names**, not ids — meaning the live view must have been altered directly against prod at some point without the file being updated. `qrDoor.ts`'s nearest-hop lookup queries `dial_coffee_relationships` directly instead (same pattern `GET /api/admin/dial/navigation` already uses, for the identical reason) to sidestep it. Re-running the checked-in `CREATE VIEW v_dial_navigation` as written would silently break `sommelierRag.ts`'s own working queries — a landmine for whoever next touches the Bloom Dial views, not something this task's scope covers fixing.

**Beats v1** *(added HOME_TASK_8, 2026-08-02, WHAT_WE_BUILT.md #133, `SOMMELIER_BUILT.md` S81 — note: this task also used "S81"; if both land, whichever commits first keeps the number and the other is renumbered before push, see the build-log entry itself)* — the bag cycle's first three beats, lifecycle-aware, degrading on silence.

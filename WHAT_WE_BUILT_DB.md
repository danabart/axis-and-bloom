...
| `v_coffee_archetype` *(added Catalog Blueprint brief 1, 2026-09-13)* | One row per `archetype` (all six). Columns: `code, label (= name), description, sort_order, has_bloom_dial, is_archetype, dominant_dimension_id, dominant_dimension_name, descriptor_families, uuid (= id)`. Replaces every hand-typed `archetype_enum` → name/label CASE map (e.g. `v_archetype_dimension_comparison` above, `admin.ts` L55/L1760) — `code` is the one identity going forward (N2); `uuid` exists only for the legacy quiz FKs still keyed off it. Nothing reads this yet — brief 3 moves readers onto it. |
| `v_coffee` *(added Catalog Blueprint brief 1, 2026-09-13)* | One row per coffee, active or not. `roaster_name` prefers `roaster_id`'s real FK, falling back to the legacy free-text `coffees.roaster` column only when `roaster_id IS NULL` (flagged via `roaster_name_is_fallback`); `match_archetype`/`match_confidence`/`match_source`/`match_session_id` from the coffee's current (non-superseded) `archetype_assignments` row — its flavor identity (D1), never to be confused with placement; `category_codes TEXT[]` aggregated from `coffee_category_assignment`. Nothing reads this yet. |
| `v_coffee_slot` *(added Catalog Blueprint brief 1, 2026-09-13)* | One row per `coffee_slot_assignment`, active or not. Exposes `placement_archetype` (the slot it fulfils) and `match_archetype` (from `v_coffee`) as deliberately separate, never-renamed columns — `placement_matches_match BOOLEAN` flags divergence, which is a legitimate state (D1), not a bug. Nothing reads this yet. |
| `v_coffee_sellable_slot` *(added Catalog Blueprint brief 1, 2026-09-13)* | One row per (slot, weight_oz) a customer can actually buy right now. Candidates: active `coffee_slot_assignment` rows on an active, named slot, on an active coffee, ordered `role = 'home'` first then `priority` (D5 — a guest only resolves once every home candidate is gone); a candidate only counts with an active `roaster_blend` row at that weight; `blendResolver.ts`'s category exclusions (decaf/half_caf/flavored never fill a flavor slot; experimental-tagged coffees only fill the experimental dial) enforced here too, against `v_coffee.category_codes`, so the rule lives in exactly one place; price only present when `dial_slot_price` has a row for that (slot, weight). Nothing reads this yet — brief 3's `fetchSommelierCoffees()` is its first intended reader (D2). |
| `v_coffee_hop` *(added Catalog Blueprint brief 1, 2026-09-13)* | One row per `dial_coffee_relationships` row. `hop_type_derived` is computed fresh from each endpoint's current active **home** `coffee_slot_assignment` (D3 — `hop_type` is never stored going forward; the stored column stays exposed as `hop_type_stored` so the integrity check can diff the two until brief 5 drops it). **As of this brief's first boot, all 48 existing rows derive to NULL** (neither endpoint has an active home — the catalog is empty) — a real, expected consequence of zero active coffees, not a bug in the view. |
...
| Placement (home/guest, priority, active) | `coffee_slot_assignment.slot_id`, `.coffee_id`, `.role`, `.priority`, `.is_active`, `.deactivated_at`, `.deactivation_reason` | `catalogService.placeCoffee` / `.moveCoffee` / `.addGuest` / `.removeFromSlot` / `.setPriority` (still empty in prod, N3 — no backfill, no roastery onboarded yet) | `v_coffee_slot`, `v_coffee_sellable_slot`, `v_coffee_hop` |
...
| Slot price | `dial_slot_price.slot_id`, `.retail_price_cents` (+ legacy `.archetype`/`.dial_sort_order` kept in sync for old readers until brief 3) | `catalogService.setSlotPrice` | `v_coffee_sellable_slot` |

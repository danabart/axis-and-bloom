...
| `v_archetype_adjacency` *(added #75; redefined on `v_coffee_hop` in Catalog Blueprint brief 3, 2026-09-14)* | Cross-archetype bridge-hop summary. **As of brief 3**, derived from `v_coffee_hop` (`hop_type_derived = 'bridge_archetype'`, both coffees active, both `is_archetype` via `v_coffee_archetype`) instead of `dial_coffee_relationships` + `archetype_assignments` directly — same D1/D3 discipline as every other brief-3 reader (bridge adjacency is a placement fact now, not a match-archetype one). Same 6 output columns as before this redefinition: `archetype_a`, `archetype_b`, `hop_count`, `more_count`, `less_count`, `avg_confidence`. Shown on the Bloom Dial admin page (`AdminDial.tsx`) and publicly via `GET /api/axis/adjacency` (now a bare `SELECT * FROM v_archetype_adjacency`, the route's own inline SQL deleted) — the real source for `/coffees` and `/bloom`'s "Worth exploring" compatibility badge tier. Kept under its old name until brief 5 renames it `v_coffee_archetype_adjacency`. |
...
| `v_coffee_hop` *(added Catalog Blueprint brief 1, 2026-09-13)* | One row per `dial_coffee_relationships` row. `hop_type_derived` is computed fresh from each endpoint's current active **home** `coffee_slot_assignment` (D3 — `hop_type` is never stored; brief 5a, 2026-09-15, dropped the stored column and `hop_type_enum` entirely, so `hop_type_derived` is now plain `TEXT` and `hop_type_stored` is gone from this view). **As of brief 3**, this is the one hop source read anywhere outside `admin.ts` (`catalogReads.getHops`) — `/coffees/:id/hops`, `sommelierRag.ts`'s discovery/alternatives supplements, `/api/axis/adjacency`/`/stats` via `v_archetype_adjacency` below — `dial_coffee_relationships` is never read directly for a hop's archetype identity again. |
...
| Match archetype (flavor identity, D1) | `archetype_assignments.archetype`, `.confidence`, `.source`, `.assigned_from_session_id` (current row = `superseded_at IS NULL`) | `catalogService.setMatchArchetype` (retired `POST /coffees/:id/archetype` used to write this) | `v_coffee`, `v_coffee_slot` |
| SKU | `roaster_blend.coffee_id`, `.weight_oz`, `.is_active`, `.roaster_sku`, `.shopify_variant_id`, `.cost_to_us`, `.quantity_available`, `.safety_stock_buffer`, `.inventory_status` | `catalogService.upsertSku` / `.restockSku` (retired `PATCH /inventory/:id` and `POST /inventory/:id/restock` used to write this) | `v_coffee_sellable_slot` |
| Slot price | `dial_slot_price.slot_id` (NOT NULL + `UNIQUE (slot_id, weight_oz)` as of brief 5a), `.retail_price_cents` — the legacy `.archetype`/`.dial_sort_order` columns this row listed through brief 4 are dropped | `catalogService.setSlotPrice` | `v_coffee_sellable_candidate`, `v_coffee_sellable_slot` |
...
| Hop physics | `dial_coffee_relationships.from_coffee_id`, `.to_coffee_id`, `.dimension_id`, `.direction`, `.delta`, `.is_recommended`, `.confidence`, `.notes` — `.hop_type` (the provisional stored value, D3) is dropped as of brief 5a | `catalogService.setHop` / `.removeHop` (retired `POST/DELETE /dial/relationships*` used to write this) | `v_coffee_hop` |
| Hop type (derived, D3) | *(never stored — `dial_coffee_relationships.hop_type` and `hop_type_enum`, the legacy stored column/type, were dropped by brief 5a)* | n/a — computed from each endpoint's current active home assignment | `v_coffee_hop.hop_type_derived` (plain `TEXT` as of brief 5a, no longer `hop_type_enum`) |
| Roastery activation state | `roaster.is_active`, `.deactivated_at`, `.deactivation_note` + the cascade onto `coffees`/`roaster_blend`/`coffee_slot_assignment` (the `coffee_alias` leg of this cascade is gone along with the table, brief 5a) | `catalogService.deactivateRoastery` / `.reactivateRoastery` (moved from inline `routes/admin.ts`, `POST /roasters/:id/deactivate`\|`reactivate` are now thin wrappers) | `v_coffee`, `v_coffee_slot`, `v_coffee_sellable_slot` |

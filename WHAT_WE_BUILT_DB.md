...
**Renamed in brief 5b** (Catalog Blueprint, 2026-09-16 — naming convention, Dana 2026-09-13 + N8 2026-09-15: every catalog table/enum/view starts with `coffee_`). Old names below and throughout this doc's history sections are accurate as of when each entry was written; every reference elsewhere in this doc to the *current* schema uses the new names.
...
> **As of 2026-09-25 (quiz interpretation v2.1 · brief 3)**: still 69 tables — read-only SQL only (`WHAT_WE_BUILT.md` #194): one `IMMUTABLE` function `quiz_archetype_canonical(text)` (retired names → today's `coffee_archetype.name`), `v_subscriber_quiz_results` reworked to read the current `quiz_session_interpretation` row (see Views, below), and a new view `v_quiz_session_interpretation_history`. No table or column changed; nothing is inserted, updated or deleted. The brief-2 table DDL now sits before the views that read it in `schema.sql` (no effect on existing databases).
>
> **As of 2026-09-25 (quiz interpretation v2.1 · brief 2)**: 69 tables — one new table, `quiz_session_interpretation` (`WHAT_WE_BUILT.md` #193), the SCD Type 2 dimension over the immutable `quiz_session` fact: one row per session **per interpretation version** (`v1` = the ruleset that ran before, retroactively named and seeded verbatim from `context_data`; `v2.1` = the current ruleset), exactly one `is_current` row per session (partial unique index `quiz_session_interpretation_current`), `UNIQUE (quiz_session_id, interpretation_version)`, FK to `quiz_session(id)` with **no** `ON DELETE CASCADE` (we never delete; the only Postgres delete that can reach `quiz_session` is guest cleanup, which only targets users with no quiz session), `computed_by IN ('scored','seed','backfill')`. The primary archetype, `branchedFrom`, scores, answers, treat and gate are **not** in it: they live on the fact. Written by `saveQuizInterpretation()` (live path: one `scored` `v2.1` current row, best-effort right after the `quiz_session` insert) and by `backend/scripts/backfillQuizInterpretation.ts` (insert-only, `--dry-run` default, refuses unless the connected database is the one named by `--expect-db`). Read by `GET /api/quiz/results/latest`, `userSignals.ts`, the Liam `/start` context and `sommelierEvaluator.ts` through `LEFT JOIN ... AND i.is_current` with `context_data` as the fallback (whole-row precedence, not per-column COALESCE). **Until the prod backfill runs, the table is empty on prod and every read path falls back to `context_data`, behaving exactly as before.** No existing table or column changed; `quiz_session` and `newsletter_subscriber` are never written by this feature. Dev readings (`axisandbloom_test`, 78 sessions): count + md5 of `quiz_session` and `newsletter_subscriber` identical before the DDL, after the DDL, after the script and after the tests.
>
> **As of 2026-09-25 (quiz interpretation v2.1 · brief 1)**: no schema change, still 68 tables — `WHAT_WE_BUILT.md` #192 is logic + a read-only endpoint + tests. `POST /api/quiz/score` still only SELECTs (`quiz_answer_archetype_score`, `quiz_answer`, `quiz_question`, `coffee_archetype`; the queries now live in `services/quizScorer.ts`); nothing is written and `quiz_session` / `context_data` are untouched. (The table that holds v1/v2.1 rows arrived in brief 2, below.)
>
> **As of 2026-09-25**: still 68 tables — Quiz Resync Fix (`WHAT_WE_BUILT.md` #191, Parts B2/A4) is purely additive, no new/dropped tables. `transactional_email_log` gained `archetype TEXT`/`resend_message_id TEXT` (both nullable — rows sent before this deploy have both NULL by design, not backfilled). `quiz_funnel_event.event`'s CHECK constraint widened to accept a new value, `quiz_final` (the post-branch, on-screen archetype — `quiz_complete` stays the pre-branch score, unchanged); the constraint's default auto-generated name (`quiz_funnel_event_event_check`) was confirmed live against prod before the `DROP CONSTRAINT IF EXISTS` + re-`ADD` in `schema.sql`. Also see #190: the read-only audit that diagnosed the bug this brief fixes.
>
> **As of 2026-09-16**: still 68 tables — Catalog Blueprint · brief 5b (`WHAT_WE_BUILT.md` #183) is a pure rename, no table/column/behavior change: the five tables and one view in the mapping table above. Every FK, PK, sequence and row survives the rename untouched (Postgres keeps them attached through `ALTER TABLE ... RENAME TO`); every index/constraint carrying an old table name in its own identifier was also renamed (e.g. `archetype_code_key` → `coffee_archetype_code_key`) so `pg_indexes`/`pg_constraint` stay readable — full list in the closing report. Narrative migration: `backend/src/db/migrations/catalog_blueprint_5b_2026-09-16.sql`.
>
> **As of 2026-09-15**: 68 tables (down from 73) — Catalog Blueprint · brief 5a (`WHAT_WE_BUILT.md` #182) dropped `dial_archetype_positions`, `coffee_alias`, `dial_slot_alias`, `dial_position_vocabulary` and `dial_archetype_config` outright, along with `roaster_blend.archetype_id`, `dial_coffee_relationships.hop_type` (+ `hop_type_enum`), `dial_slot_price.{archetype,dial_sort_order}`, `user_bloom_dial_current_position.dial_sort_order` and `coffees.roaster`; `coffees.roaster_id`, `roaster_blend.coffee_id`, `archetype.code` and `dial_slot_price.slot_id` gained NOT NULL. Two views dropped (`v_dial_positions`, `v_dial_navigation`); `v_dial_position_consensus` rebuilt onto `dial_position_signal.suggested_slot_id` and renamed `v_coffee_dial_position_consensus`; `v_coffee_hop.hop_type_stored` removed, `hop_type_derived` now plain `TEXT`; `v_coffee.roaster_name_is_fallback` removed. Narrative migration: `backend/src/db/migrations/catalog_blueprint_5a_2026-09-15.sql`.
>
> **As of 2026-09-14**: same 73 tables — Catalog Blueprint · brief 3 (`WHAT_WE_BUILT.md` #180) is readers-only at the table level: one new view (`v_coffee_sellable_candidate`) and one redefinition (`v_archetype_adjacency`, now derived from `v_coffee_hop`), no table/column changes. The real change is structural: every browse/recommend/hop/QR/beat/cron surface in the backend now answers from the `v_coffee_*` views instead of `dial_archetype_positions`/`coffee_alias`/`archetype_assignments` directly — see `services/catalogReads.ts` and the reader-by-reader table in that entry. Narrative migration: `backend/src/db/migrations/catalog_blueprint_3_2026_09_14.sql`.
>
> **As of 2026-09-13**: 73 tables. Catalog Blueprint · brief 1 (`WHAT_WE_BUILT.md` #178) added `coffee_dial_slot` (24 rows, backfilled from `dial_slot_alias` + `dial_position_vocabulary`) and `coffee_slot_assignment` (0 rows, no backfill — N3). Additive only: `archetype` gained `code`/`sort_order`/`has_bloom_dial`/`is_archetype`/`dominant_dimension_id`/`descriptor_families`; `archetype_assignments`/`roaster_blend`/`dial_slot_price`/`user_bloom_dial_current_position` each gained one tightening index or backfilled column. Five new views — see Views, below. See "Catalog ownership table," below, for the one-writer-per-fact map. Narrative migration: `backend/src/db/migrations/catalog_blueprint_1_2026_09_13.sql`.
>
...
- `coffee_archetype` (renamed from `archetype` by Catalog Blueprint brief 5b, 2026-09-16) — named flavor profiles: Chocolate & Nutty, Balanced, Fruity, Floral, Earthy, Experimental. **Application-layer key bug fixed 2026-07-13** (Find My Flavor Part 1, `WHAT_WE_BUILT.md` #91) — `backend/src/routes/users.ts` derived a lookup key via `archetype.name.toLowerCase()` (e.g. `"Chocolate & Nutty"` → `"chocolate & nutty"`), which never matched the `archetype_enum` values (`chocolate_nutty`, `balanced_sweet`, `earthy`, etc.) used everywhere else in the schema/API. No data was wrong — this was purely a code-side key mismatch, silently mis-serving 3 of 6 archetypes' color/features/`.id` on `GET /api/users/profile` and `/homepage-state`. Fixed with an explicit name→`archetype_enum` map. **Display name changed 2026-09-19 — "Balanced & Sweet" → "Balanced"** (`WHAT_WE_BUILT.md` #187): display-name-only, same `id`, `code = 'balanced_sweet'`, colour and description. `schema.sql` renames the row idempotently at boot, immediately BEFORE the archetype seed INSERT (with `code NOT NULL`, a `WHERE NOT EXISTS` miss on the old name would try to insert a code-less row and fail 23502), and sweeps `newsletter_subscriber.archetype` (the one text column that held the display name as a key) from the old names to `'Balanced'`; the standalone record is `backend/src/db/migrations/archetype_rename_balanced_2026-09-19.sql`. **`name` is still the natural key inside the quiz seed/scoring blocks** (`WHERE name = 'Balanced'` lookups and VALUES rows in `schema.sql`, plus `db/seeds/archetype_vectors.sql`/`scoring_v1.sql`) and in the `v_archetype_dimension_comparison` JOIN CASE (`WHEN 'balanced_sweet' THEN 'Balanced'` — a join condition, not a label). At runtime nothing keys on it any more: **`catalogReads.archetypeCode()` is the only accepted way to turn an inbound display name into a key** (it also resolves the retired names `Balanced & Sweet` / `Balanced and Sweet` / `Fruity & Complex` via a code-valued legacy map), and `catalogReads.archetypeUuid()` replaced the two `SELECT id FROM coffee_archetype WHERE name = $1` lookups in `routes/quiz.ts` and `services/quizSession.ts`. Left as history on purpose: `quiz_funnel_event.archetype` (event log, old rows keep the old name), jsonb snapshots (`quiz_session.context_data`, `api_event`, `sommelier_*.context_data`), and a handful of free-text editorial fields (`coffees.ai_summary`/`surprise_note`/`three_voice_story`, cupping/hop `notes`, one `sommelier_messages.content`) — listed with row ids in `backend/src/features/archetype_rename_balanced/CLOSING_REPORT.md`.
...
**Blends / roastery**
- `coffee_sku` (renamed from `roaster_blend` by Catalog Blueprint brief 5b, 2026-09-16) — sellable package variant of a coffee (12oz / 5lb); links to Shopify variant IDs, roaster SKU, cost, inventory columns. Now has `coffee_id INTEGER REFERENCES coffees(id)` linking it to the tasting catalogue, and `last_restocked_at TIMESTAMPTZ` for manual restock tracking. A name-match backfill in `schema.sql` auto-populates `coffee_id` on startup; unmatched rows (flavored coffees etc.) are surfaced in Admin → Supply & Inventory for manual assignment.
- `roastery_blend_vector` — where each blend sits in flavor-dimension space
- `user_roaster_link` — roastery staff accounts
- `roaster` — drop-ship roastery partners; fields: name, contact_person, email, phone, address, website, api_endpoint, avg_fulfillment_hours, roaster_notes, is_active; new contact fields added May 2026. **Roastery lifecycle (2026-08-25, `WHAT_WE_BUILT.md` #170)** — gained `deactivated_at`/`deactivation_note` (the free-text note Dana types into the confirm dialog). `is_active` here is written **only** by `POST /api/admin/roasters/:id/deactivate`/`.../reactivate`, and read by nothing at request time — by design (Decision 2): the deactivate/reactivate cascade is what keeps the real per-row flags (`coffees.is_active`, and matching columns on `roaster_blend`/`coffee_alias`, all three gaining `deactivated_at`/`deactivation_reason TEXT` — `'roaster' | 'manual' | NULL`) truthful; every browse/recommend/resolve read checks `coffees.is_active` (via `services/activeCatalog.ts`'s `ACTIVE_COFFEE_SQL`), never `roaster.is_active`, never a join back to `roaster`. `coffees` also gained `roaster_id UUID REFERENCES roaster(id)` — backfilled from `roaster_blend.roaster_id` then by name-match against the pre-existing free-text `coffees.roaster` column (left in place, still read by content generation) — the first real FK from a coffee to its roastery; any coffee the backfill couldn't resolve is logged at every backend startup (`index.ts`, `[roastery-lifecycle]` tag), never guessed at. **CTO review round (2026-08-26)** — coffee identity is `(roastery, coffee)`, not name alone: `coffees_active_natural_key`, a partial `UNIQUE` index on `(roaster_id, lower(trim(name))) WHERE is_active = true`, now enforces that live. Found by this exact check, not theorized: coffee 7 "Colombia" had `roaster_blend` rows from *both* roasters (the old, untightened name-only backfill's ambiguous-join bug — now fixed, `AND rb.roaster_id = c.roaster_id` added), and coffees 19/33 "Guatemala" (both Temecula) were a real duplicate, 33 an empty stub — see `WHAT_WE_BUILT.md` #170's CTO review addendum and `backend/src/db/migrations/coffees_colombia_guatemala_datafix_2026_08_26.sql` for the fix and its exact run order. The index is wrapped in `DO $$ ... EXCEPTION WHEN unique_violation` (not a bare statement) specifically so a still-live duplicate at deploy time can't roll back the rest of `schema.sql`'s single implicit transaction — it just logs a warning and self-heals on the next boot after the data fix runs. **Approved and run, 2026-08-26**: both `UPDATE`s executed against prod (repointed coffee 7's two Temecula `roaster_blend` rows to coffee 20; deactivated coffee 33), and `coffees_active_natural_key` is now live — confirmed by its absence from the "NOT created" startup warning on the following boot, and directly by `pg_indexes`. Never to be re-run (each `UPDATE` is a one-time repoint/deactivate, not idempotent against a moved-again row). `index.ts` also warns at startup for any `roaster_blend` whose `roaster_id` disagrees with its own coffee's `roaster_id`.

**Live end-to-end confirmation, 2026-08-26** — Temecula Coffee Roasters deactivated for real through the admin UI (not a test): `roaster.is_active = false`, `deactivation_note = 'pausing for now'`, cascade correctly applied to 16 coffees / 32 blends / 16 aliases and skipped coffee 33 (already `deactivation_reason = 'manual'` from the data fix above, left untouched rather than overwritten).
...
- `quiz_session_interpretation` *(added 2026-09-25, quiz interpretation v2.1 · brief 2, `WHAT_WE_BUILT.md` #193)* — SCD Type 2 over `quiz_session`: `id` UUID PK, `quiz_session_id` FK (no cascade), `interpretation_version` (`'v1'`, `'v2.1'`, …), `secondary_archetype`, `secondary_path` and `pair_confidence` (NULL on `v1` rows), `recommendation_mode`, `food_signal_alignment`, `explore_archetype`, `explore_reason`, `primary_margin`, `is_current`, `valid_from`, `valid_to` (NULL while current), `computed_by`, `computed_at`. `secondary_archetype` is `coffee_archetype.name` as of writing (`v1` seed rows keep the legacy spelling found in `context_data`; brief 3 normalizes on read). Never per user; old rows are history, kept forever.
...
- `newsletter_subscriber` — `email` PK; `first_name TEXT`; `source_id` FK → `subscriber_source`; `user_id` FK → `user_profile` (optional, now populated by `POST /api/newsletter/subscribe` via `optionalAuth` when the caller is signed in — was always nullable but never actually set before this); `subscribed BOOLEAN`; `created_at`. **Extended 2026-07-20/21** (`WHAT_WE_BUILT.md` #110, launch/10_quiz-and-archetypes Step 04): `archetype TEXT`, `experimental BOOLEAN`, `confidence TEXT`, `quiz_session_key TEXT` — all nullable, populated when the signup originates from the post-quiz firm email gate (`COALESCE(new, existing)` on conflict, so a later non-quiz signup never wipes a captured quiz result, but a quiz retake's new archetype does overwrite the old one). Feeds Step 05's Mailchimp personalization without a second lookup. No `updated_at`/history — the upsert silently overwrites; a mismatch is only recoverable from `quiz_session`, the real source of truth (see #191). **Write path hardened 2026-09-25** (`WHAT_WE_BUILT.md` #191, Quiz Resync Fix Part B1) — a `post_quiz` `archetype` is now verified against `quiz_funnel_event` (`quiz_complete`/`quiz_final`, slug-compared) before this table (or the Mailchimp `archetype:*` tag, or a quiz-complete email) ever sees it; an unbacked value is silently dropped (existing value kept via COALESCE), not written. Root cause of the drift this closed: `FlavorQuiz.tsx`'s recognized-guest resync effect could previously fire with the frontend's placeholder default before any real score existed — see #190/#191.
- `quiz_funnel_event` — **added 2026-07-20** (`WHAT_WE_BUILT.md` #109, launch/20_analytics-and-tracking Step 02). First-party quiz funnel logging, source of truth over GA4/Pixel since `POST /api/quiz/score` is public and guests dominate. `session_key TEXT` (client-generated `crypto.randomUUID()`, not a FK — most rows are guests), `event TEXT` CHECK'd to `quiz_start`/`quiz_complete`/`email_submitted`/`quiz_final`, `archetype TEXT` (nullable, populated on `quiz_complete`/`quiz_final`), `created_at`. Indexed on `session_key` and `created_at`. Written via `POST /api/quiz/event` (rate-limited, handler in `backend/src/features/marketing/funnelEvents.ts`). **`quiz_final` added 2026-09-25** (`WHAT_WE_BUILT.md` #191, Quiz Resync Fix Part A4) — the post-branch, on-screen archetype the visitor actually saw, on its own row; `quiz_complete` is unchanged, still the pre-branch score only.
- `transactional_email_log` — **added 2026-08-18** (`WHAT_WE_BUILT.md` #169, Step 07/C3). At-most-once guard for backend-originated transactional email (Resend), separate from Mailchimp's tag/journey-driven sends. `email TEXT`, `template TEXT`, `sent_at TIMESTAMPTZ DEFAULT timezone('utc', now())`, `PRIMARY KEY (email, template)`. First consumer: the quiz-complete "Your archetype card is here" email, template key `quiz_complete_v2` — an `INSERT ... ON CONFLICT DO NOTHING` claims the send before it goes out; a failed Resend call deletes the claim so the next quiz completion can retry. **`archetype TEXT`/`resend_message_id TEXT` added 2026-09-25** (`WHAT_WE_BUILT.md` #191, Quiz Resync Fix Part B2) — both nullable, set on the claim row after a successful send; rows from before this deploy have both NULL (not backfilled). `archetype` is always the *server-verified* archetype (see `newsletter_subscriber` below) — never a client-supplied value taken on faith.
...
- `coffee_archetype_assignment` (renamed from `archetype_assignments` by Catalog Blueprint brief 5b, 2026-09-16) — archetype tag per coffee with confidence level; `superseded_at = NULL` for the current assignment, populated when a newer one replaces it
...
**Bloom Dial** *(added June 2026; reorg + suggestion/adjacency #75, priority-swap/validation/suggestions follow-up #76, categories decoupling #78, Coffees page cleanup #79, 2026-07-09/11; admin Map & Journey rebuild #168, 2026-08-16)*
...
- ~~**"Retired" has no dedicated column on `coffees`**~~ — it now does. **Roastery lifecycle (2026-08-25, `WHAT_WE_BUILT.md` #170)**: `coffees.is_active` is the primary retirement signal for `isCoffeeRetired()`; "no `roaster_blend` row with `is_active = true`" is kept as a secondary signal (a coffee can go unfulfillable — every weight discontinued — without its roastery ever being deactivated), so either one marks a coffee retired. Original note preserved struck through, same convention as the S84 correction directly below.
...
| `v_dial_positions` | Current coffee positions on each archetype's Bloom Dial. Columns: `archetype`, `coffee`, `roaster`, `origin`, `dimension`, `position_sort`, `dial_label`, `has_bloom_dial`, `is_default`, `is_guest` (added #93), `delta_from_default`, `is_computed`, `last_computed_at`. Joins `dial_archetype_positions` → `coffees`, `dial_position_vocabulary`, `coffee_dimensions`, `dial_archetype_config`. **No TS readers as of Catalog Blueprint brief 3 (2026-09-14)** — superseded by `v_coffee_slot`/`v_coffee_sellable_slot`. Left in place (dormant-data discipline); dropped in brief 5. |
| `v_dial_navigation` | Directional hop graph between coffees. Columns: `from_coffee`, `to_coffee`, `dimension`, `direction` (`more`/`less`), `hop_type` (`within_archetype`/`bridge_archetype`), `delta`, `is_recommended`, `confidence`, `notes`. Joins `dial_coffee_relationships` → `coffees` (×2), `coffee_dimensions`. **No `_id` columns — never has, in this file or live** (confirmed via `pg_get_viewdef` against production, `HOME_TASK_7B`/`SOMMELIER_BUILT.md` S84). `sommelierRag.ts`'s two queries against this view select `from_coffee_id`/`to_coffee_id` anyway and have silently failed (`42703`) in production since #93/S43 gave the graph real data — caught by their own try/catch, degrading to archetype-only RAG every time. Flagged, not fixed at the time (out of that task's scope); **as of Catalog Blueprint brief 3, moot** — `sommelierRag.ts` reads `v_coffee_hop` via `catalogReads.getHops()` now, this view has **no TS readers left**. Left in place (dormant-data discipline); dropped in brief 5. |
| `v_coffee_archetype_adjacency` *(added #75 as `v_archetype_adjacency`; redefined on `v_coffee_hop` in Catalog Blueprint brief 3, 2026-09-14; renamed by brief 5b, 2026-09-16)* | Cross-archetype bridge-hop summary. **As of brief 3**, derived from `v_coffee_hop` (`hop_type_derived = 'bridge_archetype'`, both coffees active, both `is_archetype` via `v_coffee_archetype`) instead of `dial_coffee_relationships` + `archetype_assignments` directly — same D1/D3 discipline as every other brief-3 reader (bridge adjacency is a placement fact now, not a match-archetype one). Same 6 output columns as before that redefinition: `archetype_a`, `archetype_b`, `hop_count`, `more_count`, `less_count`, `avg_confidence`. Shown on the Bloom Dial admin page (`AdminDial.tsx`) and publicly via `GET /api/axis/adjacency` (a bare `SELECT * FROM v_coffee_archetype_adjacency`, the route's own inline SQL deleted) — the real source for `/coffees` and `/bloom`'s "Worth exploring" compatibility badge tier. |
| `v_coffee_dial_position_consensus` *(added #75, dormant; renamed + re-keyed from `v_dial_position_consensus` by Catalog Blueprint brief 5a, 2026-09-15)* | Weighted rollup of current (`superseded_at IS NULL`) `dial_position_signal` rows per `(coffee_id, archetype)`, weighted by `dial_source_weight.reliability_weight`. Columns: `coffee_id`, `archetype`, `consensus_vocabulary_id` (weighted mode; ties take the highest-weighted single source — now sourced from `suggested_slot_id`, kept under its old column name since nothing outside this dormant view reads the field), `total_sample_size`, `weighted_sample_size`. With only `cupping` weighted above zero today, this mirrors the live Phase 3 suggestion. Read-only, not wired into any frontend page. |
...
| `v_coffee_archetype` *(added Catalog Blueprint brief 1, 2026-09-13)* | One row per `archetype` (all six). Columns: `code, label (= name), description, sort_order, has_bloom_dial, is_archetype, dominant_dimension_id, dominant_dimension_name, descriptor_families, uuid (= id)`. Replaces every hand-typed `archetype_enum` → name/label CASE map (e.g. `v_archetype_dimension_comparison` above, `admin.ts` L55/L1760) — `code` is the one identity going forward (N2); `uuid` exists only for the legacy quiz FKs still keyed off it. **As of brief 3 (2026-09-14)**, this is the real source for `catalogReads.getArchetypes()`/`archetypeLabel()`/`archetypeCode()`, read by every service/route in the backend except `admin.ts` (brief 4). |
| `v_coffee` *(added Catalog Blueprint brief 1, 2026-09-13)* | One row per coffee, active or not. `roaster_name` is `roaster_id`'s real FK — always populated as of brief 5a (2026-09-15), since `roaster_id` is now `NOT NULL` and the legacy free-text `coffees.roaster` fallback (and its `roaster_name_is_fallback` flag) is dropped; `match_archetype`/`match_confidence`/`match_source`/`match_session_id` from the coffee's current (non-superseded) `archetype_assignments` row — its flavor identity (D1), never to be confused with placement; `category_codes TEXT[]` aggregated from `coffee_category_assignment`. **As of brief 3**, read via `catalogReads.getCoffee`/`getCoffees` everywhere a coffee's match archetype or owned-coffee identity is needed (D1/D2). |
| `v_coffee_slot` *(added Catalog Blueprint brief 1, 2026-09-13)* | One row per `coffee_slot_assignment`, active or not. Exposes `placement_archetype` (the slot it fulfils) and `match_archetype` (from `v_coffee`) as deliberately separate, never-renamed columns — `placement_matches_match BOOLEAN` flags divergence, which is a legitimate state (D1), not a bug. **As of brief 3**, read via `catalogReads.getHomeSlot`/`getSlotsForCoffee`/`getSlotsForCoffees` (aliases, hops, legacy-slot, brew notes). |
| `v_coffee_sellable_candidate` *(added Catalog Blueprint brief 3, 2026-09-14)* | The pre-`DISTINCT ON` candidate list `v_coffee_sellable_slot` (below) now picks from — one row per (active slot, active assignment, active coffee, weight in `{12, 80}`), `blend_id`/`retail_price_cents` both nullable, `rank` = row number over `(slot_id, weight_oz)` ordered home-first then priority, `is_sellable` = blend and price both present. Exposed so `blendResolver.ts` can report each skipped candidate's own reason (`'no active blend at that weight'` vs. `'no price at that weight'`) and honor `excludeCoffeeIds`, instead of only ever seeing the one winner. |
| `v_coffee_sellable_slot` *(added Catalog Blueprint brief 1, 2026-09-13; redefined on `v_coffee_sellable_candidate` in brief 3)* | One row per (slot, weight_oz) a customer can actually buy right now — `SELECT DISTINCT ON (slot_id, weight_oz) ... FROM v_coffee_sellable_candidate WHERE is_sellable ORDER BY slot_id, weight_oz, rank`. Same 16 output columns as brief 1 (its own tests assert this unchanged). Home ranks before every guest (D5); `blendResolver.ts`'s category exclusions (decaf/half_caf/flavored never fill a flavor slot; experimental-tagged coffees only fill the experimental dial) live in `v_coffee_sellable_candidate` now, enforced once. **As of brief 3**, this is D2's one candidate pool — `sommelierRag.getCandidatePool()` (Liam), `blendResolver.ts` (checkout), and `coffees.ts`'s `/archetypes` all read it via `catalogReads.getSellableSlots`. |
| `v_coffee_hop` *(added Catalog Blueprint brief 1, 2026-09-13)* | One row per `coffee_hop` row (table renamed from `dial_coffee_relationships` by brief 5b). `hop_type_derived` is computed fresh from each endpoint's current active **home** `coffee_slot_assignment` (D3 — `hop_type` is never stored; brief 5a, 2026-09-15, dropped the stored column and `hop_type_enum` entirely, so `hop_type_derived` is now plain `TEXT` and `hop_type_stored` is gone from this view). **As of brief 3**, this is the one hop source read anywhere outside `admin.ts` (`catalogReads.getHops`) — `/coffees/:id/hops`, `sommelierRag.ts`'s discovery/alternatives supplements, `/api/axis/adjacency`/`/stats` via `v_coffee_archetype_adjacency` below — `coffee_hop` is never read directly for a hop's archetype identity again. |
| `v_subscriber_quiz_results` *(added 2026-09-20, `backend/src/features/marketing/CLAUDE_CODE_PROMPT_SUBSCRIBER_QUIZ_RESULTS_VIEW.md`)* | One row per `newsletter_subscriber`, joined to that subscriber's **most recent** `quiz_session` via `user_id` (subscribers with no linked user/session still appear, quiz columns NULL). Read-only reporting; granted to `reporting_ro`. Column groups: **subscriber** — `email`, `first_name`, `subscribed_at`, `source` (`subscriber_source.label`), `subscribed`, `user_id`; **campaign** — `campaign`, `campaign_attributed_at`; **at signup** — `archetype_at_signup`, `confidence_at_signup`, `experimental_at_signup` (the values captured on `newsletter_subscriber`); **quiz outcome** — `primary_archetype` (`coffee_archetype.name` via `resulting_archetype_id`), `secondary_archetype`, `recommendation_mode`, `food_signal`, `food_signal_alignment`, `experimental`, `decaf`; **scores** — `score_chocolate_nutty`, `score_balanced`, `score_fruity`, `score_earthy`, `score_floral`, `score_experimental` (from `context_data->'scores'`, keyed by archetype display name as scored at the time; `score_balanced`/`score_fruity` COALESCE the pre-rename `Balanced`/`Fruity & Complex` keys), plus `scores_json`; **raw** — `quiz_result_json` (full `context_data`: answers, answerIds, branchedFrom, tie info, anything not broken out), `quiz_completed_at`, `quiz_session_id`. **Data caveats (2026-09-20 prod)**: the `scores` object is sparse (only archetypes that scored appear), so an unscored archetype reads NULL, not 0; no stored session has ever carried `Earthy`/`Floral`/`Experimental` score keys, so those three columns are currently always NULL; `secondary_archetype` is stored as the display name at scoring time and can still read `Balanced & Sweet` for sessions before the 2026-09-19 rename, while `primary_archetype` follows the renamed `coffee_archetype.name`. **Reworked 2026-09-25 (quiz interpretation v2.1 · brief 3, `WHAT_WE_BUILT.md` #194)**: now 41 columns. The interpretation columns (`secondary_archetype`, `recommendation_mode`, `food_signal_alignment`) come from the session's **current** `quiz_session_interpretation` row instead of `context_data`; the values frozen on the day stay as `secondary_archetype_as_scored`, `recommendation_mode_as_scored`, `food_signal_alignment_as_scored`; `confidence_at_signup` was renamed in place to `food_signal_alignment_at_signup`; `archetype_at_signup`, `secondary_archetype`, `food_signal` and the new `branched_from` are canonicalized through `quiz_archetype_canonical()` (raw treat as `food_signal_raw`); new columns appended after `user_id`: `secondary_path`, `pair_confidence`, `explore_archetype`, `explore_reason`, `primary_margin`, `interpretation_version`, `interpretation_computed_by`, `interpretation_valid_from`. A session with no current row shows NULL interpretation columns. |
| `v_quiz_session_interpretation_history` *(added 2026-09-25, brief 3)* | One row per session per interpretation version (the calibration surface): `quiz_session_id`, `user_id`, `completed_at`, `final_archetype`, `branched_from`, `interpretation_version`, `is_current`, `valid_from`, `valid_to`, `computed_by`, `secondary_archetype` (canonicalized), `secondary_path`, `recommendation_mode`, `food_signal_alignment`, `pair_confidence`, `explore_archetype`, `explore_reason`, `primary_margin`. `WHERE interpretation_version IN ('v1','v2.1')` pivoted by session gives the before/after for any set of sessions; the next event's export is `WHERE completed_at >= <event>`. Exactly one `is_current` row per session. Granted to `reporting_ro`. |
...
### Catalog ownership table *(added Catalog Blueprint brief 1, 2026-09-13; writers filled in by brief 2, 2026-09-14)*

One writer per fact, via a named service function (house rule, see `backend/src/features/catalog_blueprint/README.md`). All catalog writes now go through `backend/src/services/catalogService.ts` — the old inline SQL in `routes/admin.ts` is retired (410, `Catalog Blueprint brief 2`). **As of brief 4 (2026-09-14)**, `routes/admin.ts` itself reads only through `catalogReads`/the views — its own `dial_archetype_positions`/`coffee_alias`/`archetype_assignments` reads (the last consumer of those outside `catalogService.ts`'s roastery-lifecycle cascade) are gone, so the lint allow-list for `routes/admin.ts` (rules 3 and 4) is removed. **As of brief 5a (2026-09-15)**: `dial_archetype_config`, `dial_slot_alias`, `dial_position_vocabulary`, `dial_archetype_positions` and `coffee_alias` are dropped outright (no longer "stay live," gone from the schema entirely) — `deactivateRoastery`/`reactivateRoastery`'s `coffee_alias` cascade is removed along with the table.

| Fact | table.column | Only writer | Read through |
|---|---|---|---|
| Archetype identity + label | `coffee_archetype.code`, `.name`, `.sort_order`, `.has_bloom_dial`, `.is_archetype`, `.dominant_dimension_id` (table renamed from `archetype` by brief 5b) | brief 1's one-time backfill (schema.sql); no runtime writer | `v_coffee_archetype` |
| Archetype descriptor families | `coffee_archetype.descriptor_families`, `.descriptor_families_seeded_at` (brief 4, 2026-09-14 — guards the one-time seed so it never silently re-runs over an admin edit, see schema note below) | `catalogService.setArchetypeDescriptorFamilies` (validated against `DISTINCT wheel_category FROM cupping_note`; `PUT /catalog/archetypes/:code/descriptor-families` is a thin wrapper) | `v_coffee_archetype` |
| Coffee identity + metadata | `coffees.name`, `.roaster_id` (NOT NULL as of brief 5a; the free-text `.roaster` column it lists here through brief 4 is dropped), `.origin`, `.origin_region_id`, `.process`, `.roast_level`, `.roast_shade`, `.blend_or_single`, `.flavor_descriptors_roaster`, `.is_active`/`.deactivated_at`/`.deactivation_reason` | `catalogService.createCoffee` / `.updateCoffee` / `.retireCoffee` / `.restoreCoffee` | `v_coffee` |
| Coffee category tags | `coffee_category_assignment.coffee_id`, `.category_id` | `catalogService.updateCoffee({ categoryCodes })` (full-set upsert; `POST/DELETE /coffee-categories` are thin wrappers over it, keeping their paths) | `v_coffee.category_codes` |
| Slot identity + name | `coffee_dial_slot.archetype`, `.sort_order`, `.name`, `.position_label`, `.position_description`, `.dimension_id` | brief 1's backfill; `catalogService.renameSlot` going forward | `v_coffee_slot`, `v_coffee_sellable_slot` |
| Slot landing default | `coffee_dial_slot.is_landing_default` | brief 1's backfill (sort_order = 2, one per archetype); `catalogService.setLandingDefault` | `v_coffee_sellable_slot` |
| Slot spec (band + descriptor families) | `coffee_dial_slot.spec_band_lo`, `.spec_band_hi`, `.spec_descriptor_families` | `catalogService.setSlotSpec` (NULL until brief 4's admin page starts calling it) | `v_coffee_slot` |
| Placement (home/guest, priority, active) | `coffee_slot_assignment.slot_id`, `.coffee_id`, `.role`, `.priority`, `.is_active`, `.deactivated_at`, `.deactivation_reason` | `catalogService.placeCoffee` / `.moveCoffee` / `.addGuest` / `.removeFromSlot` / `.setPriority` | `v_coffee_slot`, `v_coffee_sellable_slot`, `v_coffee_sellable_candidate`, `v_coffee_hop` — **as of brief 3, every one of these is actually read** (`catalogReads`), no longer write-only |
| Placement certification | `coffee_slot_assignment.certified_at`, `.certified_by`, `.certification_note`, `.placement_note` | `catalogService.certifyPlacement` (note also written by `placeCoffee`/`moveCoffee` when D6 requires one) | `v_coffee_slot` |
| Match archetype (flavor identity, D1) | `coffee_archetype_assignment.archetype`, `.confidence`, `.source`, `.assigned_from_session_id` (current row = `superseded_at IS NULL`; table renamed from `archetype_assignments` by brief 5b) | `catalogService.setMatchArchetype` (retired `POST /coffees/:id/archetype` used to write this) | `v_coffee`, `v_coffee_slot` |
| SKU | `coffee_sku.coffee_id`, `.weight_oz`, `.is_active`, `.roaster_sku`, `.shopify_variant_id`, `.cost_to_us`, `.quantity_available`, `.safety_stock_buffer`, `.inventory_status` (table renamed from `roaster_blend` by brief 5b) | `catalogService.upsertSku` / `.restockSku` (retired `PATCH /inventory/:id` and `POST /inventory/:id/restock` used to write this) | `v_coffee_sellable_slot` |
| Slot price | `coffee_slot_price.slot_id` (NOT NULL + `UNIQUE (slot_id, weight_oz)` as of brief 5a), `.retail_price_cents` — the legacy `.archetype`/`.dial_sort_order` columns this row listed through brief 4 are dropped; table renamed from `dial_slot_price` by brief 5b | `catalogService.setSlotPrice` | `v_coffee_sellable_candidate`, `v_coffee_sellable_slot` |
| User dial position | `user_bloom_dial_current_position.slot_id` | existing write path (unchanged this brief; `slot_id` backfilled in brief 1) | — (not yet exposed through a `v_coffee_*` view) |
| Hop physics | `coffee_hop.from_coffee_id`, `.to_coffee_id`, `.dimension_id`, `.direction`, `.delta`, `.is_recommended`, `.confidence`, `.notes` — `.hop_type` (the provisional stored value, D3) is dropped as of brief 5a; table renamed from `dial_coffee_relationships` by brief 5b | `catalogService.setHop` / `.removeHop` (retired `POST/DELETE /dial/relationships*` used to write this) | `v_coffee_hop` |
| Hop type (derived, D3) | *(never stored — the legacy stored column/type, dropped by brief 5a, lived on the table now called `coffee_hop`, renamed from `dial_coffee_relationships` by brief 5b)* | n/a — computed from each endpoint's current active home assignment | `v_coffee_hop.hop_type_derived` (plain `TEXT` as of brief 5a, no longer `hop_type_enum`) |
| Roastery activation state | `roaster.is_active`, `.deactivated_at`, `.deactivation_note` + the cascade onto `coffees`/`coffee_sku`/`coffee_slot_assignment` (the `coffee_alias` leg of this cascade is gone along with the table, brief 5a) | `catalogService.deactivateRoastery` / `.reactivateRoastery` (moved from inline `routes/admin.ts`, `POST /roasters/:id/deactivate`\|`reactivate` are now thin wrappers) | `v_coffee`, `v_coffee_slot`, `v_coffee_sellable_slot` |
| Dial position signal (Phase 5, dormant) | `dial_position_signal.*` (re-keyed `.suggested_vocabulary_id` → `.suggested_slot_id` by brief 5a, A1) | `dialSuggestion.recordCuppingSignal` — unchanged this brief except moving off pool-level `BEGIN`/`COMMIT` onto `withTransaction` | `v_coffee_dial_position_consensus` (renamed from `v_dial_position_consensus` by brief 5a) |
...
|---|---|---|---|---|---|---|
| Crosshatch | Nicaragua & Ethiopia | Blend | Washed | Light-medium | Balanced | High |

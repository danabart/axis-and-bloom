...
### Catalog ownership table *(added Catalog Blueprint brief 1, 2026-09-13)*

One writer per fact, via a named service function (house rule, see `backend/src/features/catalog_blueprint/README.md`). Brief 1 creates every table/column below and their read-through views; nothing writes to the new ones yet (`coffee_slot_assignment` starts empty, N3) — brief 2's `catalogService.ts` is the only writer for anything marked below from brief 2 onward. Legacy tables (`dial_archetype_config`, `dial_slot_alias`, `dial_position_vocabulary`, `dial_archetype_positions`, `coffee_alias`) stay live and unchanged through brief 4; brief 5 drops them.

| Fact | table.column | Only writer | Read through |
|---|---|---|---|
| Archetype identity + label | `archetype.code`, `.name`, `.sort_order`, `.has_bloom_dial`, `.is_archetype`, `.dominant_dimension_id`, `.descriptor_families` | this brief's one-time backfill (schema.sql); no runtime writer yet | `v_coffee_archetype` |
| Slot identity + name | `coffee_dial_slot.archetype`, `.sort_order`, `.name`, `.position_label`, `.position_description`, `.dimension_id` | this brief's backfill; brief 4's admin "slot spec editing" going forward | `v_coffee_slot`, `v_coffee_sellable_slot` |
| Slot landing default | `coffee_dial_slot.is_landing_default` | this brief's backfill (sort_order = 2, one per archetype); brief 2: `catalogService.<fn>` | `v_coffee_sellable_slot` |
| Slot spec (band + descriptor families) | `coffee_dial_slot.spec_band_lo`, `.spec_band_hi`, `.spec_descriptor_families` | brief 4: admin page (NULL until then) | `v_coffee_slot` |
| Placement (home/guest, priority, active) | `coffee_slot_assignment.slot_id`, `.coffee_id`, `.role`, `.priority`, `.is_active`, `.deactivated_at`, `.deactivation_reason` | brief 2: `catalogService.<fn>` (empty until then, N3) | `v_coffee_slot`, `v_coffee_sellable_slot`, `v_coffee_hop` |
| Placement certification | `coffee_slot_assignment.certified_at`, `.certified_by`, `.certification_note`, `.placement_note` | brief 2: `catalogService.<fn>` | `v_coffee_slot` |
| Match archetype (flavor identity, D1) | `archetype_assignments.archetype`, `.confidence`, `.source`, `.assigned_from_session_id` (current row = `superseded_at IS NULL`) | existing cupping/manual/import write paths (unchanged this brief; `source` backfilled) | `v_coffee`, `v_coffee_slot` |
| SKU | `roaster_blend.coffee_id`, `.weight_oz`, `.is_active`, `.roaster_sku`, `.shopify_variant_id` | existing admin write paths (unchanged this brief; one tightening index added) | `v_coffee_sellable_slot` |
| Slot price | `dial_slot_price.slot_id` (new), `.retail_price_cents` | existing admin write path (unchanged this brief; `slot_id` backfilled) | `v_coffee_sellable_slot` |
| User dial position | `user_bloom_dial_current_position.slot_id` (new) | existing write path (unchanged this brief; `slot_id` backfilled) | — (not yet exposed through a `v_coffee_*` view) |
| Hop physics | `dial_coffee_relationships.from_coffee_id`, `.to_coffee_id`, `.dimension_id`, `.direction`, `.delta`, `.is_recommended`, `.confidence`, `.notes` | existing admin write path (unchanged this brief) | `v_coffee_hop` |

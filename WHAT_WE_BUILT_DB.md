...
**Bloom Dial** *(added June 2026; reorg + suggestion/adjacency #75, priority-swap/validation/suggestions follow-up #76, categories decoupling #78, 2026-07-09/10)*
- `dial_archetype_config` — dominant dimension and Bloom Dial flag per archetype (seeded, 6 rows as of #75). `is_archetype BOOLEAN` (added #75) — `true` for the 5 real flavor families, `false` for `experimental` (a cross-cutting category, not a peer archetype — see `BLOOM_DIAL_ALLOCATION_SPEC.md` §3). Suggestion (below) and archetype-adjacency logic check this flag. **As of #78**, `experimental` is also rejected outright at assignment time (`POST /api/admin/coffees/:id/archetype`) — `is_archetype = false` only ever gated suggestion/adjacency logic, not assignment itself.

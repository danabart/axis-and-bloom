...
**Open for Dana**: deploy → boot applies the DDL to prod (empty table; every read path falls back, behaving exactly as today) → run `--dry-run --expect-db axisandbloom`, check the 37 crawl rows match → `--apply --expect-db axisandbloom` → from then on every session has a current row. Rollback: revert the code; the table is ignored and nothing on the fact needs undoing. Brief 3 (`v_subscriber_quiz_results` reads the table) is next.

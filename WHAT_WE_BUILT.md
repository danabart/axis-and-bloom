...
- `archetype` — named flavor profiles: Chocolate & Nutty, Balanced, Fruity, Floral, Earthy, Experimental
...
- Q4-D split: +0.5 to Chocolate & Nutty AND +0.5 to Balanced (two rows in `quiz_answer_archetype_score`, `resulting_archetype_id = NULL`)
...
- **Compatibility badge**: three tiers — "In your wheelhouse" (exact match, filled rust), "Worth exploring" (adjacent archetype, amber outlined), "Outside your comfort zone" (no match, grey outlined + explanatory note). Adjacency is hardcoded per archetype (e.g. Balanced ↔ Chocolate & Nutty, Fruity ↔ Floral).
...
- `findWinner(ranked, byQ)` — veto cascade (Q6 → Q5 → Q3 → Q1, fallback: Balanced)
...
- Cascade exhausted → Balanced fallback
...
**Problem**: `POST /api/quiz/score` resolved ties with a hardcoded order (Balanced > Chocolate & Nutty > Fruity) regardless of the user's actual answers. This meant two users with identical scores but different answers would always get the same archetype — wrong by design.  
**Fix**: Replaced with a veto cascade: Q5 → Q4 → Q2 → Q1. For each question in that order, if the user's answer pointed to one of the tied archetypes, that archetype wins. Q3 is intentionally excluded (contributes to raw score only). Fallback: Balanced. A second DB query fetches the `q_number → archetype` mapping from `quiz_answer_archetype_score` only when a tie is detected — no extra cost on the happy path.
...
| Flavor quiz (V7) | ✅ Active — V7 replaces V4; 5 scored questions + 1 food signal (Q6, weight 0), reordered from V4, experimental gate on Q3-C, 5 archetypes (Chocolate & Nutty, Balanced, Fruity, Floral, Earthy); branch question after scoring reclassifies Fruity→Floral or CN→Earthy if user selects answer B |
...
| **Balanced** | `#d1ac11` | Reliable habit — smooth, easy, approachable |
...
| Q1 — Identity | 1 | It's a reliable habit. I just like having it. | Balanced | +1 |
...
| Q2 — Perfect cup | 2 | It was smooth and easy the whole way through — nothing got in the way. | Balanced | +2 |
...
| Q3 — Black coffee | 1 | It's fine, easy to drink. I might add something to smooth it out. | Balanced | +1 |
...
| Q4 — Disappointment | 2 | It's too bitter or too intense. | Balanced | +2 |
...
| Q5 — Bitterness | 3 | I'd rather have something gentler and smoother. | Balanced | +3 |
...
Fallback (cascade exhausted without resolution): Balanced
```
...
```json
{
  ...
  "archetypeId": "uuid",
  "scores": { "Chocolate & Nutty": 7, "Balanced": 3, "Fruity": 2 },
  "experimental": false,
  "secondaryArchetype": "Balanced",
  "foodSignal": "Fruity",
  "confidence": "low",
...
**Change**: Populated `archetype_vector` with target dimension ranges for all 5 archetypes (Chocolate & Nutty, Balanced, Earthy, Floral, Fruity) across 7 numeric dimensions (Sweetness, Acidity, Bitterness, Body, Texture, Savory/Depth, Finish Length). Source: `backend/src/dimensions/dimensions-vector.JPG`.
...
- Six archetype sections (Chocolate & Nutty, Balanced, Fruity, Earthy, Floral, Experimental)
...
**Structure:**
- Grouped by **archetype** (Chocolate & Nutty, Balanced, …)
...
**Context**: three related fixes from Dana's latest screenshots (Fruity — active position, text/buttons wrapping; Balanced — "Lively" position, temporarily unavailable, rendering as a small stray pill). Diagnosis: Part 7's photo column was tuned by measuring a neighbor's height and matching it, and that neighbor's height isn't constant between the active and unavailable card states — plus Part 7's bag placement had eaten into the card column's own width. Spec in `backend/src/features/the_bloom_page/CLAUDE_CODE_PROMPT_THE_BLOOM_PART9_ROW_POLISH.md`.
...
**Verified in a real browser against production Cloud SQL**: Fruity's "Balanced — Bright & Tart" now reads on 2 lines (was 3), weight buttons side by side, Add to cart/Compare on one row — confirmed via direct DOM measurement, not just visual inspection. Simulated a real drag gesture on Balanced's dial (mouse down/move/up, not a shortcut) to reach the "Lively" position and confirmed via its own live label ("Lively · Temporarily unavailable") that the correct state was reached; the redesigned unavailable card now measures 176px tall with real border/padding/centered content, vs. the previous tiny pill, and the "Balanced" heading sits in its normal large position and size, unmoved. Photo column height confirmed exactly 319px across all 6 archetypes and both the active and unavailable position states (state-invariant, per Phase C's goal) — spot-checked Floral's naturally longer title ("Perfumed & Expressive") wrapping to 3 lines at the same card width as Fruity's 2; confirmed this is pre-existing text-length variance unrelated to Part 9, not a regression, since the width fix (Phase A) was already applied when measured. Bag still renders with real vertical presence (244px tall active / 176px unavailable, both well above the dial's own footprint) despite the narrower width cap — Phase A's regression check. Mobile (390px) and both flip orientations (Fruity flip, Balanced non-flip) confirmed clean: zero horizontal overflow, zero console/page errors.
...
the backend already sends (`Floral`, `Fruity`, `Balanced`, `Chocolate & Nutty`, `Earthy`
...
| Balanced | Acidity (*Brightness*) | flat | very bright / sharp |
...
**Verified**: `vite build` (frontend) and `tsc` (backend) both clean. Live-checked on `/bloom`: hop chips render pre-reveal on Balanced (ink-on-mustard `#9a2918` — zoomed screenshot confirms transparent bg/thin border/pill shape reading correctly against the mustard field) and Chocolate & Nutty (beige-on-terracotta), text format correct including a cross-archetype " · Balanced" suffix; footer confirmed no longer showing hop chips after reveal (only the 3 standard links remain). Stop layout verified two ways: Balanced's default slot ("Classic Balanced") sits dead-center on the ruler; dragging from center to a non-center point (mouse down mid-ruler, drag, release) snapped cleanly onto the next real stop, never resting between positions. Keyboard: focused the slider directly, `ArrowRight`/`ArrowLeft` stepped `aria-valuenow` by exactly 1 each press, clamped correctly at `aria-valuemax` (3) without exceeding it. Evidence full-width fallback confirmed on two real, different-shaped cases: "Coado" (Chocolate & Nutty, dimensions-only) and "Bright & Balanced" (Balanced, coffee 18 — the same coffee nulled above, wheel-descriptor-only) both render one full-width column, no dead half-column. Content guard verified end-to-end against the real backend + live Cloud SQL, not just read from cache: found coffee 33 ("Guatemala") — the only coffee in the catalogue with zero dimensions, zero descriptors, zero cupping notes, and all three content fields never-generated (`NULL`) — and called `generateAndStoreAllContent` directly (capturing `console.warn`); all three expected warnings fired in the exact spec'd format, no Claude API call was made for any field (confirmed by execution returning near-instantly), and a follow-up query confirmed all three DB columns landed as real `NULL` (not empty string — the `""` seen in the JSON return value is a pre-existing `?? ''` response-shaping default for `aiSummary` specifically, unrelated to what's persisted). The graceful-placeholder UI path was also seen live and unprompted: coffee 18 (nulled above) rendered "A Balanced coffee for this turn of the dial. Its full tasting notes are arriving soon." instead of any broken/blank state.
...
**§A — doors.** Target resolution is computed server-side in `computeDoorMap()` (`backend/src/routes/coffees.ts`), called once per `GET /api/coffees/archetypes` and `GET /api/coffees/experimental` request and returned as each archetype's `doors: {left, right}` field. **Revised after initial ship** (see Correction below) — doors now follow ONE canonical symmetric chain around the site's nav-strip order, wrapping: `Floral ↔ Fruity ↔ Balanced ↔ Chocolate & Nutty ↔ Earthy ↔ Experimental ↔ Floral`. Left door = the previous archetype in the chain, right door = the next. The full 6×2 map, verified against the live dev server:

| Archetype | ← left door | right door → |
|---|---|---|
| Floral | Experimental | Fruity |
| Fruity | Floral | Balanced |
| Balanced | Fruity | Chocolate & Nutty |
| Chocolate & Nutty | Balanced | Earthy |
...
**CTO review round, third pass (2026-08-26)** — `npm test` after the second pass's deploy came back flaky (20 then 18 failures across two back-to-back runs, different tests each time). Root cause: no `vitest.config.ts` existed, so Vitest ran test **files** concurrently by default against the one shared prod DB every file uses — a fast-finishing file's `afterAll` `'Vitest%'` sweep could delete a still-running file's fixture rows out from under it. Fixed with `backend/vitest.config.ts` (`fileParallelism: false`) — a real per-run test database is a tracked follow-up, not fixed here. Confirmed the fix: two more runs came back byte-for-byte identical (16 failed / 98 passed, same 16 test names both times) — flakiness gone, but 16 ≠ the 6-test quizScoring baseline, so every failure got reconciled by hand rather than assumed:

- **12 of the 16** — the quizScoring baseline (6 real tests), doubled by the pre-existing stale-`dist/`-copy artifact already documented in #169 (`dist/services/quizScoring.test.js` running alongside `src/services/quizScoring.test.ts`). Not this feature, not new.
- **2 of the 16** — genuine bugs in this session's own new test code, fixed this pass: `admin.roasters.test.ts`'s cascade test (~6 sequential HTTP + DB round trips over the Cloud SQL proxy tunnel) had no timeout override and blew past Vitest's 5000ms default — given the same `20000` convention `coffees.test.ts`'s slow tests already use. `coffees.test.ts`'s hops fixture created a target coffee's `dial_archetype_positions` row but never a matching `archetype_assignments` row — the `/hops` route's `dap.archetype = aa.archetype` join never matched with `aa` NULL, so every hop's `target_archetype` came back null and the route's own `if (!row.target_archetype) continue` silently skipped all of them (0 hops, not 3). The production route was never broken; the fixture was incomplete.
- **2 of the 16** — a real but pre-existing, out-of-scope finding, parked rather than fixed: `coffees.test.ts`'s "`GET /api/coffees/archetypes` — at most one `isDefault=true` per archetype" assertion doesn't match live prod data. Confirmed via the CTO review round's own Temecula preview simulation: **every archetype currently carries two `is_default=true` rows, one from Path and one from Temecula** — standing data that predates this feature entirely, not something the roastery-lifecycle cascade touches or caused. This is a product/data question (should each archetype really have one default irrespective of roastery, or one per roastery?), not a bug to silently patch — same category as `admin.lookups.test.ts`'s unrelated FK flake spotted the same session, left alone for Dana to pick up separately.

**Files this pass**: `backend/vitest.config.ts` (new), `backend/src/routes/admin.roasters.test.ts`, `backend/src/routes/coffees.test.ts`, this entry.
...
### 173. Hoboken Coffee Crawl — `/crawl` landing page + campaign attribution (2026-08-31)

**Context**: `backend/src/features/hoboken_crawl/CLAUDE_CODE_PROMPT_HOBOKEN_CRAWL.md`. The Hoboken Historical Museum's Coffee Crawl (Sep 20, 2026) hands out a printed card with a QR to `axisandbloomcoffee.com/crawl`; goal was a real landing page plus enough first-party attribution to know exactly which subscribers came from the crawl (for the free-shipping promise and the Oct 1 five-match draw), without ever letting the crawl campaign collide with the existing `source: 'post_quiz'` signal the match email and `quiz-completed` Mailchimp tag depend on.

**Task 0 (verification before writing anything)**: all 14 "CONTEXT — verified in code" claims in the brief checked against the tree — routing/gate, email capture, backend subscribe, Mailchimp tags, funnel logging, schema/migrate conventions, analytics exports, archetype visuals/colors, deploy pipeline, and the 12-pre-existing-errors/6-files `tsc` baseline. **Zero drift** — every claim matched exactly. One real, useful drift found only *during* the DONE sequence, not before (see Deviations).

**Built**:
1. `frontend/src/app/lib/campaign.ts` (new) — `rememberCampaign`/`getActiveCampaign`, the only code that touches `localStorage['ab_campaign']`. Keeps the same `crypto.randomUUID()` visitor key across rescans of the same campaign; 30-day expiry; try/catch for private-mode storage.
2. `frontend/src/app/components/CrawlLanding.tsx` (new) — `/crawl` route, added to `PublicLayout` in `App.tsx` and to `PRELAUNCH_OPEN_ROUTES` (`prelaunch.ts`), deliberately **not** added to Navigation/Footer's link sets. Stamps the campaign + fires `logCampaignLanding` (fire-and-forget) on mount, brand-consistent copy (kicker/H1/body/CTA, six-tile field guide pulling `num`/`color` straight from `bloomVisuals.ts`'s existing `ARCHETYPE_VISUALS` — no third copy of the six hex values), offer block, footer signature. Mobile-first, no new image assets, no hero media.
3. Campaign threaded through the existing quiz funnel and signup: `api.ts` (`subscribeNewsletter`/`logQuizFunnelEvent` gain optional `campaign`/`campaignVid`, new `logCampaignLanding`), `PostQuizEmailGate.tsx` (sealed card unchanged, `source` stays `'post_quiz'`, campaign added when active), `FlavorQuiz.tsx` (all four `logQuizFunnelEvent` call sites plus both direct `subscribeNewsletter` calls — the guest-resync effect and the signed-in auto-subscribe effect — now pass the active stamp).
4. Backend: `features/marketing/campaigns.ts` (new) — `KNOWN_CAMPAIGNS` allowlist + `normalizeCampaign`/`normalizeVid`/`normalizeUtm`, the only gate on what a client can ever get written to the DB or Mailchimp. `newsletter.ts`'s `handleSubscribe` upserts `campaign`/`campaign_vid`/`campaign_attributed_at` with the same first-wins `COALESCE` pattern as `source_id`. `mailchimp.ts`'s `buildTags` appends `campaign:<slug>` when present (existing tags unchanged). `funnelEvents.ts`/`quiz.ts /event` accept and normalize `campaign`/`campaignVid`. New `routes/campaign.ts` → `POST /api/campaign/landing`, mounted at `/api/campaign` in `index.ts`, rate-limited per real client IP (same `express-rate-limit` + `getRealClientIp` shape as `funnelEventLimiter`/`qrResolveLimiter`), referrer truncated to 512 chars / user-agent to 256, no IP stored.
5. Schema: `campaign_landing_event` table, nullable `campaign`/`campaign_vid`/`campaign_attributed_at` on `newsletter_subscriber`, nullable `campaign`/`campaign_vid` on `quiz_funnel_event`, `campaign_funnel_v` and `campaign_subscriber_archetype_v` reporting views — added to `schema.sql` next to the `quiz_funnel_event` block, plus the narrative `backend/src/db/migrations/campaign_attribution_2026_08_31.sql`.
6. `Privacy.tsx` — one added paragraph on the anonymous visitor key + landing analytics (the Analytics/advertising section already enumerates what's stored, so this applied per the brief's conditional).

**Deviations / judgment calls**:
- **DONE's literal "`Migration complete.`" string is never printed at backend startup** — that text only prints when `db/migrate.ts` is run standalone (e.g. `npm run db:migrate`); `index.ts`'s actual boot path applies `schema.sql` inline and logs `"DB schema verified"` instead. This is a real, narrow inaccuracy in the brief's own CONTEXT section (conflating the two), found live in the post-deploy Cloud Run log, not assumed. The functional bar — schema applied cleanly on the deploy that shipped this change, no error — **is met**: `2026-09-01 01:19:46 DB schema verified` with no preceding `DB migration error (non-fatal):` line.
- `test-mailchimp-tags.mjs` **left unchanged** — its own header comment confirms it hand-mirrors the tag list rather than importing `buildTags` (standalone `.mjs`, no TS build step), so the brief's "if it asserts on `buildTags`" condition is false.
- `document.title` — no existing page in this codebase sets it (grepped, zero hits), so `CrawlLanding.tsx` sets it directly in a `useEffect` rather than reusing a convention that doesn't exist.
- CrawlLanding's visual language reuses `PreLaunch.tsx`'s constants (`TERRA`, `FONT`, the same logo mark) and typographic scale rather than its literal `.pl-*` class names, since the layout itself (single-column mobile-first event page with a 6-tile grid) doesn't fit `PreLaunch.tsx`'s two-column photo-split grid.

**Verified**: `npx tsc --noEmit` (frontend) — identical 12 pre-existing errors in the same 6 files, zero new. `vite build` clean (2153 modules). Backend `npx tsc --noEmit` clean, zero errors. Deploy confirmed green (`gh run watch`, both `Build & Deploy Backend` and `Build & Deploy Frontend` jobs succeeded).

**Acceptance — 8 of 10 run live** against real production data (backend run locally against prod Cloud SQL via the Auth Proxy, per [[axis_and_bloom_local_cloudsql_testing]] — the public API itself is Cloudflare-bot-challenged for scripted traffic, confirmed directly (a `cf-mitigated: challenge` "Just a moment…" response on both GET and POST), so the brief's own accepted alternative, "a gated dev server... plus prod DB reads," was used instead of driving the live domain):
1. ✅ landing beacon fires twice with the same `vid`, both rows land with UTMs + user agent.
2. ✅ `quiz_funnel_event` rows (`quiz_start`/`quiz_complete`/`email_submitted`) all carry `campaign`/`campaign_vid`.
3. ✅ subscribe: `source_id` resolves to `post_quiz`, archetype set, `campaign`/`campaign_vid`/`campaign_attributed_at` all set.
4. ⚠️ **not fully verified live** — `syncMailchimpMember`'s real API call was exercised (confirmed in the local server's log) but Mailchimp's own validation rejected the `@example.com` test addresses ("looks fake or invalid") before a member ever existed, so no tag round-trip could be read back; separately, pulling the real `MAILCHIMP_API_KEY` via `gcloud secrets versions access` was blocked by the permission classifier and not pursued further. Verified instead by calling the real `buildTags()` directly: `{source: 'post_quiz', archetype: 'Balanced', experimental: false, campaign: 'hoboken-crawl-2026'}` → `['source:post_quiz', 'archetype:balanced', 'quiz-completed', 'campaign:hoboken-crawl-2026']`, exact match. A true live read-back needs a human running `test-mailchimp-tags.mjs` (its existing usage already assumes a human supplies the key) with a real-looking address.
5. ✅ control (no campaign fields at all): `campaign`/`campaign_vid` NULL in both `newsletter_subscriber` and `quiz_funnel_event`.
6. ✅ `campaign: 'anything-else'` on `/subscribe` → row written, `campaign` NULL. ✅ `/campaign/landing` with an unknown campaign or a non-UUID vid → 400, no row. ✅ hammering `/landing` (40 requests) tripped the rate limiter at 429.
7. ✅ `campaign_funnel_v`: scans=2, unique_scanners=1, quiz_starts=1, quiz_completes=1, email_events=1, subscribers=1. `campaign_subscriber_archetype_v`: `Balanced` → 1.
8. **Not live-browser-verified** — no browser automation tool available in this environment. Verified instead by code + build: `/crawl` absent from `Navigation.tsx`/`Footer.tsx` (grepped, zero hits), present in `PRELAUNCH_OPEN_ROUTES`, and the `?preview=true` bypass itself is untouched generic logic already shared by every other open route.
9. ✅ shipped and confirmed in the deployed bundle (`grep`'d the built `dist/assets/index-*.js` for the new privacy sentence — present).
10. ✅ cleanup — every row this session's acceptance run created (2 `campaign_landing_event`, 5 `quiz_funnel_event`, 3 `newsletter_subscriber`) deleted and re-queried at 0 residue; no Mailchimp member was ever created (see #4), so nothing to remove there; the `RESEND_API_KEY` in the local `.env` is a known non-functional placeholder (see [[axis_and_bloom_local_cloudsql_testing]]) so the transactional-email send itself 401'd locally — the send *attempt* and its atomic claim/rollback both fired correctly, but a real end-to-end Resend send was not confirmed (real key only lives in prod Secret Manager, mounted server-side, never pulled to this session).

**QR URL for Camila** (already live, code resolves to it):
```
https://axisandbloomcoffee.com/crawl?utm_source=hoboken-crawl&utm_medium=print&utm_campaign=hoboken-crawl-2026
```

**Files**: `frontend/src/app/lib/campaign.ts` (new), `frontend/src/app/components/CrawlLanding.tsx` (new), `frontend/src/app/App.tsx`, `frontend/src/app/lib/prelaunch.ts`, `frontend/src/app/lib/api.ts`, `frontend/src/app/components/PostQuizEmailGate.tsx`, `frontend/src/app/components/FlavorQuiz.tsx`, `frontend/src/app/components/Privacy.tsx`, `backend/src/features/marketing/campaigns.ts` (new), `backend/src/routes/campaign.ts` (new), `backend/src/routes/newsletter.ts`, `backend/src/features/marketing/mailchimp.ts`, `backend/src/features/marketing/funnelEvents.ts`, `backend/src/routes/quiz.ts`, `backend/src/index.ts`, `backend/src/db/schema.sql`, `backend/src/db/migrations/campaign_attribution_2026_08_31.sql`.

**Pushed** (`a08bbc3`, with Dana's go-ahead) — deploy green, both jobs succeeded. `main` == `origin/main` throughout; the pre-existing uncommitted `OPEN_TASKS.md` and `launch/60_commerce-and-fulfillment/12_G1_payment_capture_PLACEHOLDER.md` changes were left completely untouched (only this feature's 18 files staged).

---

### 174. Hoboken Crawl Part 2 — Lato site-wide (real fix) + `/crawl` rebuilt to Camila's approved page (2026-09-01)

**Context**: `backend/src/features/hoboken_crawl/CLAUDE_CODE_PROMPT_HOBOKEN_CRAWL_PART2_GENOVA_AND_PAGE.md`. Two threads converged: while sourcing Camila's approved `/crawl` mockup (`43-crawl-page-handoff.md` + `42-crawl-landing-mockup-v9.html`), Task 0 verification surfaced that the site had been silently rendering in **Arial**, not the brand font, since 2026-07-05 — `cab3716` switched every font reference to `'Lato', Arial, ...` the day after `98267e2` had restored Genova, but the referenced `Lato-Regular.ttf` was never committed, so the `@font-face` failed and every page fell through the stack to Arial. This directly explains build-log item ~4481/`OPEN_TASKS.md` OT-11 ("`font-light` collapses to a hairline weight") and means the design-system line ~1137 above ("Genova font site-wide") has been inaccurate since July 5 — **correction**: the site ran on Arial from 2026-07-05 until this entry.

**Mid-session revision (2026-09-01, same day)**: the brief originally specified fixing this back to **Genova** (Camila's package included Genova + Gotham OTFs for the glyph rules). Partway through — after Genova/Gotham font files were already converted and a first pass of `CrawlLanding.tsx` built against them — Dana and Camila decided against taking on commercial font licensing before launch (no web license held for Genova or Gotham) and revised Task A to **Lato** instead (SIL OFL, free for web). The in-progress Genova/Montserrat work (glyph-stroke substitute fonts for Gotham's `&`/`?` rule) was discarded per the brief's own revision note (`git checkout` on the affected files, `frontend/src/design/FONT/genova/` and `.../montserrat/` deleted) before Task A was rebuilt as written below. Verified the revision was genuine — re-read the brief file itself (not just the chat instruction) and found a fully rewritten, internally consistent document (updated rationale, decisions, commit messages, and all 11 acceptance criteria changed together), not a one-line anomaly.

**Task A — commit `5307c97`** (`typography: install Lato properly (five weights, bundled), fix the dead font reference`): downloaded Lato 2.0 (Light/Regular/Medium/Bold/Black) from the `google/fonts` GitHub mirror of the official upstream (`latofonts.com` itself returned a Cloudflare bot-challenge 403 from this environment; same OFL-licensed upstream, more reachable) — the Google Fonts hosted copy turned out to already include Medium/SemiBold statics, contrary to the brief's assumption that the Google Fonts subset lacked them. Converted to WOFF2 via `fontTools`, verified each `usWeightClass` (300/400/500/700/900) before use. `fonts.css` rewritten to one `Lato` family, five faces, weight **ranges** (Light 100–300, Regular 400, Medium 500–600, Bold 700–800, Black 900) so every Tailwind weight class resolves to a real face — this is what actually closes OT-11, not a per-page cleanup. `font-synthesis: none` added to `html` in `theme.css`. Every `'Lato', Arial, ...` reference across the 15 files `cab3716` (and a fresh `grep`) touched was already correct post-revert — left untouched per A3's explicit instruction, since Task A's job became "install what those strings already point to," not "change them again."

**Task A verification**: `vite build` clean, zero Lato/font warnings, `grep -ri "genova|gotham|montserrat" frontend/dist` empty. Live browser (local dev server + real prod DB via the Cloud SQL Auth Proxy, same pattern as Part 1 — the public site itself is Cloudflare-bot-challenged for any non-browser traffic, confirmed directly): `document.fonts.check` true for Lato 300/400/500/700/900 (forcing `document.fonts.load()` first — `check()` alone only reflects weights the browser has actually attempted to load, and a bare homepage visit doesn't trigger every weight on its own); all 5 WOFF2 files confirmed loading from our own bundle via the network panel, not a third party. Visual pass at 390px/1280px over every required page found **zero overflow/clipping caused by the font swap** — the one real overflow found (`/bloom`'s dial, `.bd-dial-wrap`, fixed 400×400px inside 40px padding) was confirmed via the CSS itself to be pre-existing and font-independent (fixed pixel dimensions, not text-driven), not a regression. One cosmetic wrap difference found and logged for Camila in `OPEN_TASKS.md` (new **OT-18**), not fixed: the quiz entry screen's "Whose palate are we profiling today?" now wraps to 3 lines at 390px instead of 2. **Acceptance criterion #2's literal wording** ("`font-light` → Regular face; `font-bold` → Black face") doesn't match A2's own five-face table (which defines a distinct Light face and a distinct Bold face) — almost certainly stale wording carried over from the pre-revision Genova acceptance text, where no separate Light/Bold faces existed. Implemented and verified per A2's explicit, more detailed weight-range table instead: `font-light`(300) → **Light** face, `font-bold`(700) → **Bold** face — objectively the more correct mapping when dedicated faces exist, flagged here rather than silently reconciled.

**Task B — commit `2ada0fa`** (`crawl: rebuild /crawl to Camila handoff v9, name handoff, email-card prefill`), **plus a same-day fix commit `26ec9d5`** (see below): `CrawlLanding.tsx` rewritten from scratch to Camila's exact copy/structure/CSS (12 sections, handoff §3), Lato weight mapping (Thin→Light 300, Regular 400, Medium 500, Black 900, per the revision), no glyph-substitute rule (Lato's own `&`/`?` used as-is). Logo mark: confirmed by diffing SVGs that the mockup's quarter-round mark is a **different asset** from `brandAssets.logoQuarter1` (different viewBox, different path data and palette) — inlined verbatim per the brief's own fallback rule. Name handoff reuses the existing `axisBloomCustomerName` sessionStorage mechanism verbatim (zero `FlavorQuiz.tsx` changes for the start itself); `PostQuizEmailGate.tsx` gained an optional `initialFirstName` prop, wired at both render sites. Photo: `campaignAssets.hobokenCrawl2026.photoBand` added to `assets.ts`; Dana uploaded `raw/campaigns/hoboken-crawl-2026/photo-band.jpg` herself mid-session (brief was live-revised for the renamed key/path — `photo` → `photoBand`/`photo-band` — after Task A/B implementation had already started; re-read and applied), confirmed both the raw upload and the Cloud Function's `.webp`/`-mobile.webp` outputs exist before wiring the `<img>`.

**Found and fixed live, before it reached the acceptance pass**: moving `/crawl` out of `PublicLayout` (Camila's page has its own logo/wordmark/footer — the site's `Navigation`/`Footer` were both double-wrapping it, confirmed visually) also silently broke `?preview=true`'s site-wide unlock, since that route no longer rendered anything that calls `usePrelaunchBypass()` (every other open route gets the sessionStorage write as a side effect of `<Navigation>`/`<PrelaunchGate>`). Fixed with a direct `usePrelaunchBypass()` call in `CrawlLanding.tsx` — commit `26ec9d5`, deployed and reverified before declaring acceptance done. Three code commits total for this entry, not two: the brief's "two commits" instruction covered Task A and Task B; this was a bug found *during* Task B's own acceptance testing, on the same feature, shipped immediately rather than left broken.

**Acceptance — all 11 items verified live** (backend run locally against real prod Cloud SQL via the Auth Proxy, real UI click-through via a disposable Playwright/Chromium install — same established pattern as prior sessions in this repo, e.g. #100/#127's "disposable Playwright/Chromium install" entries):
1–3. Task A, see above — all pass.
4. Handoff §8 items 1/3/4/5/7 pass (fonts, no white, no em dashes outside the FROM/TO lockup, photo crops clean at 375/390/900/1440, 19px input avoids iOS zoom-jump); item 2 (Gotham glyph rule) not applicable per the revision. Close-up screenshots of the wordmark and the palate line captured for Camila.
5. `/crawl?utm_source=...` (no `?preview=true`) renders Camila's page exactly; landing beacon fires, `localStorage.ab_campaign` stamps, `campaign_landing_event` row lands with UTMs — Part 1 regression confirmed clean.
6. Typing a name and pressing Enter opens `/find-my-flavor` directly on question 1 (entry screen skipped) — confirmed live.
7. Quiz completed end-to-end through the real UI → sealed card's first-name field prefilled with the typed name, **and confirmed still editable** (changed it, refilled it) → real submit → `newsletter_subscriber.first_name`/`campaign`/`campaign_vid` all correct, `quiz_funnel_event` rows carry the campaign — all read back from the real row, not assumed.
8. Both controls verified live: `/find-my-flavor` opened directly still shows its own entry screen untouched; a quiz taken from `/`'s Home name form still hands the name through (Part 1's original mechanism) *and* gets the same email-card prefill Task B added.
9. Button disabled with empty/whitespace name confirmed; **`?preview=true` on `/crawl` unlocking the rest of the site was the fix described above** — failed on first pass, fixed, redeployed, reverified passing; `/crawl` absent from Navigation/Footer confirmed (now structurally guaranteed — it doesn't render either component at all, not just excluded from their link lists).
10. Photo: raw upload confirmed present, both optimized variants confirmed generated — Dana uploaded it herself (see above). Logo: mockup's own inline SVG used (confirmed different asset from `brandAssets.logoQuarter1`).
11. Cleanup: complicated by discovering, mid-cleanup, that **Dana had been live-testing the actual deployed page in parallel** on production — her real pre-existing account (`danabar.mail@gmail.com`) and two of her own marked test addresses (`+dana-try-3`, `+testhobokencrawl`) had picked up real campaign attribution as a side effect of her own visits. None of those three rows were created by this session, so none were touched — inspected and left alone, surfaced here rather than guessed at. Everything traceable to this session's own scripts (2 `@example.com` subscriber rows, 26 landing-event vids, 7 funnel-event rows) was identified individually and deleted; final query confirmed only Dana's 3 vids remain. No Mailchimp member was created for either `@example.com` test address (Mailchimp's own validation rejects them before a member exists, same finding as Part 1); Dana separately confirmed she'd handled anything of her own in Mailchimp directly.

**Files**: `frontend/src/design/FONT/lato/*.woff2` + `OFL.txt` (new), `frontend/src/styles/fonts.css`, `frontend/src/styles/theme.css`, `OPEN_TASKS.md` (OT-11 resolved, new OT-18), `frontend/src/app/components/CrawlLanding.tsx` (full rewrite), `frontend/src/app/App.tsx` (route moved out of `PublicLayout`), `frontend/src/app/components/PostQuizEmailGate.tsx`, `frontend/src/app/components/FlavorQuiz.tsx`, `frontend/src/design/assets.ts`, plus Camila's reference package archived at `backend/src/features/hoboken_crawl/43-crawl-page-deploy-package/` (mockup HTML, handoff doc, the Genova/Gotham OTFs as print-only archive material — confirmed zero references anywhere in `frontend/`, and `misc/design_documents/genova/` already established the precedent of committing Genova files as non-web design assets in this repo — and the photo).

**Pushed** (`5307c97`, `2ada0fa`, `26ec9d5`, all with Dana's go-ahead) — all three deploys green, backend startup log clean (`DB schema verified`, no errors, no schema change as expected). `main` == `origin/main` throughout; the pre-existing uncommitted `OPEN_TASKS.md` (Dana's own Cloudflare Bot Fight Mode note) and `launch/60_commerce-and-fulfillment/12_G1_payment_capture_PLACEHOLDER.md` changes were reconstructed byte-for-byte back onto the working tree after each commit that touched `OPEN_TASKS.md`, so they stayed exactly as untouched/uncommitted as before this session, alongside this entry's own additions.
...
### 178. Catalog Blueprint · brief 1 — schema, views, integrity (2026-09-13)

**Context**: `backend/src/features/catalog_blueprint/CLAUDE_CODE_PROMPT_CATALOG_1_SCHEMA_VIEWS_INTEGRITY.md`, brief 1 of 5 in the Catalog Blueprint series (see that folder's `README.md` for decisions D1-D6/N1-N7 and the full 5-brief plan). Both roasteries are deactivated, zero active coffees — a clean-cut rebuild of the catalog's target model, additive only: nothing existing is dropped, renamed, or re-pointed; no readers changed; no write endpoints touched. Task 0 verified every listed fact (line numbers, table shapes, enum values, dominant-dimension ids, the `quizIntegrity.ts` pattern to clone, git HEAD at `745418a`) against the code with zero deviations — the one thing Task 0 flagged to verify-before-seeding, the real `wheel_category` strings, turned out to differ from the brief's placeholder spelling only in spacing around the slash (`Sour / Fermented`, `Nutty / Cocoa`, `Green / Vegetative`, not the no-space guess).

**Built**: `withTransaction()` in `db/client.ts` (BEGIN/COMMIT/ROLLBACK around a `pg.PoolClient`, released in `finally`) — exported alongside `db`, not yet adopted by any of the 8 hand-rolled `db.connect()` sites or the 4 pool-level `db.query('BEGIN')` sites (brief 2). `archetype` gains one identity: `code archetype_enum UNIQUE` (backfilled from `name`), `sort_order` (`CANONICAL_ARCHETYPE_ORDER`), `has_bloom_dial`/`is_archetype`/`dominant_dimension_id` (copied from `dial_archetype_config`, which stays untouched until brief 5), and `descriptor_families TEXT[]` seeded with the real SCA wheel categories per archetype (chocolate_nutty: Nutty / Cocoa, Sweet; balanced_sweet: Sweet, Nutty / Cocoa, Fruity; fruity: Fruity, Sour / Fermented; floral: Floral, Fruity; earthy: Green / Vegetative, Spices, Roasted; experimental: none — `Other` deliberately excluded from every family). New tables: `coffee_dial_slot` (24 rows, backfilled from `dial_slot_alias` + `dial_position_vocabulary`; one `is_landing_default` per archetype at sort_order 2; spec-band/descriptor-family columns present but NULL — Dana sets them from the admin page in brief 4) and `coffee_slot_assignment` (home/guest placement as one fact, D1/D5; zero rows, N3 — no backfill, brief 2's service is its only writer). Tightening on what stays: `archetype_assignments_one_current` (one non-superseded row per coffee), `roaster_blend_one_active_per_weight`, `dial_slot_price`/`user_bloom_dial_current_position` both gain a backfilled `slot_id`. Five views — `v_coffee_archetype` (replaces every hand-typed `archetype_enum`→label CASE map), `v_coffee` (roaster name with fallback flag, current match archetype, category codes), `v_coffee_slot` (placement vs. match side by side, never conflated — D1), `v_coffee_sellable_slot` (D5 home-then-guest resolution, blendResolver's category exclusions enforced once), `v_coffee_hop` (hop type derived fresh from current home assignments, D3, stored column kept alongside for diffing). `catalogIntegrity.ts` (13 numbered checks, cloned from `quizIntegrity.ts`'s shape, `severity: 'info' | 'fail'` added for the 4 checks — 7, 8, 9, 13 — that are informational by design), wired at boot (non-fatal, `[catalog-integrity]` prefix, plus a standalone `archetype.code IS NULL` warning), `GET /api/admin/catalog/integrity`, and `AdminCatalogIntegrity.tsx` mounted under `AdminQuizIntegrity` on `/admin`.

**A real idempotency bug found and fixed before this could ship**: the first `schema.sql` re-apply failed with "cannot drop view v_coffee because other objects depend on it" — `v_coffee_slot` and `v_coffee_sellable_slot` both read `v_coffee`, so a bare `DROP VIEW IF EXISTS v_coffee` right before its own `CREATE VIEW` broke on the second boot once those two existed. Fixed the same way `v_archetype_dimension_comparison`/`v_archetype_vectors` already do it earlier in this file: all five `DROP VIEW IF EXISTS` statements moved to the top of the block, in reverse dependency order, ahead of any `CREATE VIEW`. Verified by applying `schema.sql` twice in a row against prod with zero errors on either pass.

**A genuine finding, not a bug — since refined**: integrity check #10 (`v_coffee_hop`: stored vs. derived hop type, both ends must have an active home) originally failed on the very first boot — 48 of 48 rows in `dial_coffee_relationships` came back "stale" (neither endpoint has an active home, since every coffee is currently inactive). Investigation showed all 48 are between coffees that are inactive by design (N3 keeps Path/Temecula as inactive history) — inert history, not a real gap. **CTO review round (2026-09-13)** rescoped check #10 to hops between two *active* coffees only (`v_coffee_hop.from_coffee_is_active AND to_coffee_is_active`); among those, the same mismatch/missing-home test applies. Name/expected text updated to say "among hops between active coffees." A hop with an inactive endpoint no longer appears in the check's details at all. Added a test asserting exactly that (a hop between two inactive `Vitest` coffees never shows up in check #10's `details`). Boot now shows zero `[catalog-integrity]` warnings except check #13, matching the brief's Definition of Done.

**Verified**: `npm run build` clean. Live-DB facts checked against prod via the Cloud SQL Auth Proxy before writing any seed value or test fixture (zero active coffees; `dial_coffee_relationships` = 48 rows, all between inactive coffees; `dial_slot_price` already had 32 admin-set rows, including floral/2 at both 12oz and 80oz — the new `v_coffee_sellable_slot` test fixture reuses whatever price row already exists there via `ON CONFLICT DO NOTHING` rather than colliding with it; no pre-existing duplicate that would have blocked either new partial unique index). `schema.sql` applied twice against prod, second pass a clean no-op after the idempotency fix below. New tests: `db/client.test.ts` (2), `db/schema.catalog_blueprint.test.ts` (13 — archetype identity, `coffee_dial_slot` constraints, `coffee_slot_assignment` constraints, `archetype_assignments` constraint, a full `v_coffee_sellable_slot` D5 fixture walk, `v_coffee_hop` bridge→within transition), `services/catalogIntegrity.test.ts` (3, after the check #10 refinement) — all 18 green. `npm test` overall: 18 pre-existing failures remain, all unrelated (verified via `git status` showing zero changes to any of the three affected files/their sources) — `routes/coffees.test.ts` ×2 and `services/sommelierRag.test.ts` ×1 fail because they require a real active coffee to exist, which the current zero-active-coffee catalog state can't provide regardless of this brief; `services/quizScoring.test.ts` ×6 is an unrelated pre-existing scoring-logic/test mismatch untouched by this work. Each unique failure appears twice because `npm test` runs both `src/*.test.ts` and the compiled `dist/*.test.js` copies — a pre-existing quirk of the test setup, not introduced here.

**Files**: `backend/src/db/client.ts`, `backend/src/db/client.test.ts` (new), `backend/src/db/schema.sql`, `backend/src/db/migrations/catalog_blueprint_1_2026_09_13.sql` (new), `backend/src/db/schema.catalog_blueprint.test.ts` (new), `backend/src/services/catalogIntegrity.ts` (new), `backend/src/services/catalogIntegrity.test.ts` (new), `backend/src/index.ts`, `backend/src/routes/admin.ts`, `frontend/src/app/components/admin/AdminCatalogIntegrity.tsx` (new), `frontend/src/app/components/admin/AdminDashboard.tsx`, `WHAT_WE_BUILT_DB.md` (new "Catalog ownership table" section), `backend/src/features/catalog_blueprint/README.md` (brief 1 status flip), `.gitignore` (`.claude/settings.local.json`, CTO review round).

**Committed** — schema.sql was applied directly to prod Cloud SQL via the Auth Proxy (additive-only DDL, verified idempotent) before this commit. One commit, not pushed.

---

### 179. Catalog Blueprint · brief 2 — catalogService, thin routes, import (2026-09-14)

**Context**: `backend/src/features/catalog_blueprint/CLAUDE_CODE_PROMPT_CATALOG_2_SERVICE_ROUTES_IMPORT.md`, brief 2 of 5, depends on brief 1 (`f4a9685`). Task 0 confirmed every listed fact exactly (all 19 old endpoint line numbers, `db/client.ts` exports, `dialSuggestion.ts`'s `getAvgCuppingScore`/`recordCuppingSignal` line numbers, `coffee_dial_slot` = 24 rows / `coffee_slot_assignment` = 0 rows / `runCatalogIntegrityChecks().allPass` = true, no CI grep infra) — zero deviations.

**Built**: `catalogService.ts` — one exported function per verb (`createCoffee`, `updateCoffee`, `retireCoffee`, `restoreCoffee`, `setMatchArchetype`, `placeCoffee`/`moveCoffee`/`addGuest`/`removeFromSlot`/`setPriority`/`certifyPlacement`/`previewPlacement`, `renameSlot`/`setSlotSpec`/`setSlotPrice`/`setLandingDefault`, `upsertSku`/`restockSku`, `setHop`/`removeHop`, `deactivateRoastery`/`reactivateRoastery` + `buildDeactivationPreview`/`buildReactivationPreview` moved here from `admin.ts`), each in one `withTransaction`, each returning `{ result, warnings, integrity }`. The movement guardrail (`placeCoffee`/`moveCoffee`): structural checks (`COFFEE_NOT_FOUND`/`INACTIVE`, `SLOT_NOT_FOUND`/`INACTIVE`, `HOME_EXISTS`, `PRIORITY_TAKEN`, `ALREADY_ASSIGNED`) → evidence (`evaluatePlacement`: cupping band vs. the slot's spec, descriptor family vs. `v_collaborative_flavor_wheel`'s top-3, D1 divergence from the current match) → a `band_out_of_spec`/`descriptor_off_family` warning demands a `placementNote` (`NOTE_REQUIRED` 400) or, with `CATALOG_SPEC_HARD_GATE=true`, a hard `SPEC_VIOLATION` 409 (default off, `.env.example`) → blast radius (`previewPlacement`, read-only, also `GET /catalog/placement-preview`: hops whose derived type would change, `user_bloom_dial_current_position` counts on source/target, whether the target is nameable/priced/has a 12oz SKU, current occupants). `catalogIntegrity.ts` refactored (brief 1's 13 checks each became their own function taking an optional `{coffeeId?, slotId?, tx?}` scope) so `scopedIntegrity()` re-runs checks 4/5/6/8 for the touched coffee/slot inside the same write transaction; `runCatalogIntegrityChecks()` composes all 13 unscoped, unchanged output shape. `catalogImport.ts` + `npm run catalog:import` CLI: validates a whole manifest first (unknown archetype/slot, duplicate names, missing 12oz SKU — collected, not thrown one at a time), then applies every coffee's `createCoffee → setMatchArchetype → upsertSku(all) → placeCoffee(home) → addGuest(each) → certifyPlacement` in **one transaction**, `slotPrices` last, dry-run = same transaction rolled back via a caught internal sentinel so the report still reads "as if applied"; idempotency via `COFFEE_EXISTS` per existing (roaster, name), `mode: 'skip_existing'` the escape hatch. New `/api/admin/catalog/*` routes (thin: validate → call service → map `CatalogError` 1:1 via a new `handleCatalogError()` helper); 18 old placement endpoints retired to `410 RETIRED` (route kept registered so `api_event` still logs attempts) — `POST/PATCH/DELETE /coffees*`, `POST/PATCH /coffee-alias*`, `PATCH /slot-prices`, `PATCH /dial/vocabulary/:id`, `POST/PATCH/DELETE /dial/positions*`, `POST/DELETE /dial/relationships*`, `PATCH /inventory/:id`, `POST /inventory/:id/restock`; dead helper code removed (`rejectIfCoffeeInactive`, the local `computeInventoryStatus`, the coffee-alias priority-swap logic). `POST/DELETE /coffee-categories` kept their paths but now go through `updateCoffee({ categoryCodes })`'s full-set upsert. `dialSuggestion.ts`'s `recordCuppingSignal()` moved onto `withTransaction` — the last pool-level `BEGIN` in the codebase. Seeds: the nine Path/Temecula bootstrap seed files moved to `db/seeds/_retired/` (own `README.md`: history, never run again); the boot-time Kopi Safari `dial_archetype_positions` seed removed from `schema.sql`. `manifests/EXAMPLE_manifest.json` written — two fictional `Vitest`-prefixed coffees on a fictional roastery, exercising every manifest field, doubling as `catalogImport.test.ts`'s fixture.

**Two real bugs found and fixed via the new tests, not by inspection**: (1) `setPriority`'s two-phase reorder used negative temp priority values to dodge the `(slot_id, priority)` partial unique index mid-reorder — but `coffee_slot_assignment.priority` has `CHECK (priority >= 1)` (brief 1), so the temp value itself was rejected. Fixed with a large positive offset (`100000 + assignment id`) instead. (2) `catalogIntegrity.ts` check 5 ("exactly one active home") double-counted a coffee's home row whenever that same coffee also had an active *guest* row elsewhere — the query joined "any active assignment" separately from "the home row," producing a cross-joined `home_count` of 2 for a coffee that actually has exactly one home. Never triggered in brief 1 (its own tests never gave one coffee both roles); surfaced by `catalogImport.test.ts`'s first coffee, which has both a home and a guest per `EXAMPLE_manifest.json`. Fixed by counting roles within a single join. Also fixed two `Promise.all`-of-queries-sharing-one-transaction spots (`scopedIntegrity`, `runCatalogIntegrityChecks({tx})`) that node-postgres flags as deprecated (concurrent queries on one `pg.Client`) — sequential `await` instead, harmless overhead, no more warning.

**A real gap found via a pre-existing test, not the brief's own field list**: retiring `PATCH /coffees/:id` broke `admin.lookups.test.ts`'s `origin_region` resolution coverage (Flavor Intelligence Part 1 Decision #9) — `updateCoffee`'s field list (Part A1) never mentioned `origin_region` at all. Added `originRegion?: string | null` to `UpdateCoffeeInput` (metadata only, same as every other field there — clears on `null`, untouched on `undefined`) rather than let the feature quietly regress; updated that test to call the new `PATCH /catalog/coffees/:id` with the field renamed to camelCase for consistency with everything else on the verb. `admin.roasters.test.ts` also updated (not a bug — an intentional, brief-specified change): `deactivateRoastery`'s `applied` object gained the new `assignments` cascade count.

**Verified**: `npm run build` clean. `grep -rn "db.query('BEGIN')" backend/src --include=*.ts` → 0 hits. `grep -rnE "(INSERT INTO|UPDATE|DELETE FROM) (coffees|archetype_assignments|coffee_slot_assignment|coffee_dial_slot|dial_slot_price|roaster_blend|dial_coffee_relationships|coffee_category_assignment)\b" backend/src/routes backend/src/services --include=*.ts | grep -v catalogService.ts | grep -v test` → 5 hits, all pre-existing and all outside this brief's scope: `admin.ts`'s `/coffees/:id/story` and `coffees.ts`'s AI-summary/story-generation writes (story is explicitly not a placement write, Don'ts), `orders.ts`'s `roaster_blend` stock decrement (a commerce write, explicitly called out in the brief as staying), and `qrDoor.ts`'s `coffees.qr_token` write (an unrelated column, not a catalog-identity/placement field). New tests: `services/catalogService.test.ts` (13), `services/catalogImport.test.ts` (5), `routes/admin.catalog.test.ts` (19) — all green, plus the two pre-existing tests fixed above. `npm test` overall: the same 18 pre-existing failures as brief 1 (`routes/coffees.test.ts` ×2, `services/sommelierRag.test.ts` ×1, `services/quizScoring.test.ts` ×6, each counted twice for the `src`/compiled-`dist` duplication) — nothing new failing. Boot-equivalent integrity check: `allPass: true`, zero failing checks (matches brief 1's post-check-10-fix state). **Prod dry-run of `EXAMPLE_manifest.json`** via `npm run catalog:import -- src/features/catalog_blueprint/manifests/EXAMPLE_manifest.json` (no `--apply`): `Import failed: ROASTER_NOT_FOUND — Roaster "Vitest Import Roastery" not found — create it via /admin/roasters first` — the expected stop, proving the guard; confirmed no such roaster existed in prod at the time (the vitest suite's own fixture had already torn itself down).

**Files**: `backend/src/services/catalogService.ts` (new), `catalogImport.ts` (new), `catalogService.test.ts` (new), `catalogImport.test.ts` (new), `catalogIntegrity.ts` (refactored to per-check scoped functions, check 5 bug fix), `backend/src/scripts/catalogImport.ts` (new), `backend/src/routes/admin.ts` (18 endpoints retired, `/catalog/*` added, roastery lifecycle + coffee-categories rewired), `routes/admin.catalog.test.ts` (new), `routes/admin.lookups.test.ts` (repointed at the new endpoint), `routes/admin.roasters.test.ts` (updated `applied` shape), `services/dialSuggestion.ts` (`withTransaction`), `backend/src/db/schema.sql` (Kopi Safari boot seed removed), `backend/src/db/seeds/_retired/` (9 files moved + new `README.md`), `backend/src/features/catalog_blueprint/manifests/EXAMPLE_manifest.json` (new), `backend/package.json` (`catalog:import` script), `backend/.env.example` (`CATALOG_SPEC_HARD_GATE`), `WHAT_WE_BUILT_DB.md` (ownership table filled in), `backend/src/features/catalog_blueprint/README.md` (brief 2 status flip).

**Not committed yet** — brief written and executed this session; commit pending Dana's go-ahead, same as every other task-execution session in this series.

---

### 180. Catalog Blueprint · brief 3 — readers onto views (2026-09-14)

**Context**: `backend/src/features/catalog_blueprint/CLAUDE_CODE_PROMPT_CATALOG_3_READERS_ONTO_VIEWS.md`, brief 3 of 5, depends on brief 2 (`97c87a9`). The one brief in the series with a visible customer-facing behavior change ("the one careful review"): before this brief, `catalogService` writes the new placement tables but nothing reads them — every browse/recommend/hop/QR/beat/cron surface still answers from `dial_archetype_positions`/`coffee_alias`/`archetype_assignments`. Task 0 confirmed every listed fact with two minor, non-material line-number drifts (`routes/axis.ts`'s `ARCHETYPE_DISPLAY` map at L92, not the brief's L96; `services/sommelierRag.ts`'s `FALLBACK_ADJACENCY` at L144, not L147) — zero deviations that changed scope.

**Built**: `services/catalogReads.ts` (new) — the only place base-table names for the catalog may appear outside `catalogService.ts`: `getArchetypes()` (60s in-process cache over `v_coffee_archetype`), `archetypeLabel()`/`archetypeCode()` (replace every hand-typed label↔code map), `getCoffee`/`getCoffees` (`v_coffee`), `getHomeSlot`/`getSlotsForCoffee`/`getSlotsForCoffees` (`v_coffee_slot`), `getSellableSlots`/`getSellableCandidates` (`v_coffee_sellable_slot`/the new `v_coffee_sellable_candidate`), `getSlots` (`coffee_dial_slot`), `getHops` (`v_coffee_hop`), `getCatalogVersion` (a GREATEST-of-`updated_at`/`created_at` fingerprint across every catalog table, the Liam snapshot key). Part A: `v_coffee_sellable_candidate` (new) — one row per (active slot, active assignment, active coffee, `{12, 80}` weight), blend/price both nullable, `rank` = home-first-then-priority, `is_sellable` = blend and price both present; `v_coffee_sellable_slot` redefined as `WHERE is_sellable` over it (same 16 output columns, brief 1's own tests pass unchanged); `v_archetype_adjacency` redefined on `v_coffee_hop` (both coffees active, both `is_archetype`, `bridge_archetype` only) instead of `archetype_relationship`/whatever it read before, same 6 output columns, physically relocated after `v_coffee_hop`'s own definition.

**Reader-by-reader**: `blendResolver.ts` (`resolveBlendForSlot`/`computeCollectionOffer` onto `getSellableCandidates`/`getSellableSlots` — inline category-exclusion SQL deleted, the view owns it now; guests resolve once every home candidate is gone or skipped, D5, which the old `coffee_alias`-priority chain never did) · `coffees.ts` (the public Bloom API — `/archetypes`, `/experimental`, `/archetype-order`, `/other-categories`, `/archetype-stats`, `/:id/legacy-slot`, `/:coffeeId/hops`, `/:id/story`, `/:id/content` all rewritten onto `catalogReads`; `ARCHETYPE_LABEL` and `buildSlotsForArchetype`'s old two-formula lookup deleted; `computeDoorMap()` made async/lazy-cached since archetype order/labels now come from a query) · `axis.ts` (`/adjacency` now a bare `SELECT * FROM v_archetype_adjacency`; `/stats`'s archetype counts from `v_coffee.match_archetype`, hop counts from `v_coffee_hop`, both filtered to active coffees; the hardcoded `ARCHETYPE_DISPLAY` map replaced by `getArchetypes()`, `/vectors` untouched — not a placement question) · `sommelierRag.ts` (`getCandidatePool()` — D2's one candidate pool, `v_coffee_sellable_slot` at 12oz joined to `v_coffee` for `match_archetype` — computed once per call and reused across every `ragFocus` branch instead of bespoke SQL per focus; `getAliases()` onto `getSlotsForCoffees`; `getAdjacentArchetypes()` onto `v_archetype_adjacency`, already code-keyed) · `sommelier.ts` (`resolveCoffeeDisplayNames()` onto `getAliases`/`getCoffees`; new `refreshCatalogSnapshotIfStale()`, extracted to its own exported function so it's unit-testable — see below; its own leftover `ARCHETYPE_NAME_TO_KEY` map, missed in an earlier pass this session, replaced with `archetypeCode()`) · `users.ts` (`ARCHETYPE_NAME_TO_KEY` → `archetypeCode()` at all four call sites; the `ARCHETYPES` display map's hardcoded `name` field removed — `features`/`color` have no catalog source and stay, but the label itself now always resolves live via `archetypeLabel()`; the flavor-memory order-archetype join moved off a raw `archetype_assignments` join onto a batched `getCoffees()` lookup) · `cron.ts`/`beatEngine.ts`/`brewCard.ts` (each swapped one raw `archetype_assignments` query for `getCoffee().match_archetype`) · `qrDoor.ts` (`resolveQrDisplayName()`'s fallback label now the real `v_coffee_archetype` label via `archetypeLabel()`, not a humanized code) · `dialPositionSignal.ts` (`getCoffee`/`getArchetypes` replace `archetype_assignments`/`dial_archetype_config`) · `dialSuggestion.ts` (labels via `archetypeLabel()`; `dial_archetype_config` → `getArchetypes()`; slot count/labels now come from this archetype's dial slots via `getSlots()` instead of `dial_position_vocabulary`, since the two have a 1:1 correspondence; the **one** remaining legacy read — allow-listed, expires brief 5 — is the final `(archetype, sort_order) → dial_position_vocabulary.id` lookup needed to write `dial_position_signal.suggested_vocabulary_id`, not yet re-keyed to `slot_id`; `getDialSuggestion` gained `suggested_slot_id` alongside it) · `behavioralConfidence.ts` (order-archetype match now via `v_coffee.match_archetype` through `roaster_blend.coffee_id`, `roaster_blend.archetype_id` — a column nobody ever wrote — never read again) · `orders.ts`/`index.ts` (no change needed — the former's `archetype_assignments` mention was comment-only, the latter's boot checks were already clean).

**Lint** (`backend/scripts/lint-catalog.mjs`, no deps, `npm run lint:catalog`, wired into `.github/workflows/deploy.yml` before the Docker build and into `npm test` via a spawning `scripts/lintCatalog.test.ts`): (1) no `db.query('BEGIN')` anywhere — zero allow-list; (2) no INSERT/UPDATE/DELETE on any catalog table (`coffee_slot_assignment`, `coffee_dial_slot`, `archetype_assignments`, `dial_slot_price`, `dial_coffee_relationships`, `coffee_category_assignment`, `roaster_blend`, `archetype`, `coffees`) outside `catalogService.ts`, except `routes/orders.ts` `UPDATE roaster_blend` (commerce stock, permanent) and `UPDATE coffees` in `routes/admin.ts`/`routes/coffees.ts`/`services/qrDoor.ts` (content columns only, permanent); (3) no reference to a dormant legacy table/view (`dial_archetype_positions`, `coffee_alias`, `dial_slot_alias`, `dial_position_vocabulary`, `dial_archetype_config`, `v_dial_positions`, `v_dial_navigation`) outside `routes/admin.ts` (whole file, expires brief 4), `dialSuggestion.ts`'s vocabulary-id mapping (expires brief 5), or `catalogService.ts`/`catalogIntegrity.ts`'s pre-existing roastery-lifecycle-cascade/check-13 reads of `coffee_alias`/`dial_archetype_positions` (both expire brief 5 — the brief's own text named only "`coffee_alias` cascade" for `catalogService.ts`, but the same cascade also reads `dial_archetype_positions` for guest/home position counts; allow-listed both, noted here since it's broader than the brief's one-line description); (4) no hardcoded archetype display-label string literal (`'Chocolate & Nutty'`, etc.) in `routes/`/`services/` outside `routes/admin.ts` (expires brief 4) and test files. **A discovered deviation from the brief's literal rule 4, decided rather than reported-and-stopped-for**: `services/quizScoring.ts` (`findWinner`'s tie-break default) and `services/quizIntegrity.ts` (check 8, the literal *is* the spelling assertion) both trip the blanket rule but belong to the quiz subsystem, not catalog placement — Task 0's own hand-typed-label-map inventory never named either file. Rewriting `findWinner` into `catalogReads`'s async API would ripple through the quiz subsystem's unrelated call graph; allow-listed both instead, same "another thread" boundary the brief draws around the Balanced rename.

**Behavior changes a customer could notice**: guests now actually fulfil a slot once its home is retired or unpriced (D5) — previously guest priority was recorded but never read by any live surface; Liam will only ever recommend a coffee that's home/guest on a slot with a priced 12oz SKU (D2) — previously the RAG pool was `coffees.is_active` with no placement/price requirement at all; the Bloom Dial's landing-default flag is now a property of the *slot* (`coffee_dial_slot.is_landing_default`, one True per archetype, seeded in brief 1), not a per-coffee flag that could drift from the slot it was supposedly describing. **Liam's mid-session catalog drift fix**: `sommelier_sessions.context_data` now carries a `catalogVersion` fingerprint alongside `catalogText`/`storyCandidates`, both persisted at session start; every turn compares it against a fresh `getCatalogVersion()` and rebuilds the snapshot (same builders session-start used) exactly once per real catalog change, not once per turn — a coffee retired or re-priced mid-conversation now actually reaches an already-open Liam session instead of only ever affecting the next one.

**Verified**: `npm run lint:catalog` clean (zero violations against the final allow-list above). `npm run build` clean. `npm test`: 268 passing, 12 failing — all 12 are the pre-existing `quizScoring.test.ts`/`isSecondaryClose` mismatches (6 unique failures × 2 for the `src`/compiled-`dist` duplication), unrelated to this brief and untouched. Boot-equivalent integrity check: `allPass: true`, zero failing checks. Brief 1's `schema.catalog_blueprint.test.ts` (13) passes unchanged after the `v_coffee_sellable_slot` refactor. Two brief-2 tests were fixed along the way, not because this brief's own reader migrations touched them, but because their fixtures pre-dated a real requirement that turns out to have existed since brief 1 (`v_coffee_sellable_slot` requires blend **and** price, not just blend) and depended on ambient real `dial_slot_price` rows that have since changed: `catalogService.test.ts`'s D5 guest-resolution test now sets and restores its own slot's 12oz price instead of assuming one already existed. `routes/coffees.test.ts`'s 4 pre-existing empty-catalog failures (legacy-slot, content, an inactive-coffee-never-fills-a-slot check, and hops-with-inactive-targets) and `routes/axis.test.ts`'s adjacency/stats tests were rewritten onto `catalogService`-built fixtures (the old `coffee_alias`/`dial_archetype_positions`/`archetype_assignments`-only fixtures satisfy neither the new placement-based views nor the price requirement, so they silently produced empty results instead of exercising anything). New tests: `services/catalogReads.test.ts` (4), `services/blendResolver.test.ts` (rewritten, 7), `services/sommelierRag.test.ts` (rewritten, 4), `routes/coffees.contract.test.ts` (new, 2 — full `/archetypes` shape + D5 handoff, `/hops` and `/legacy-slot` key-for-key against their frontend consumers), `routes/sommelier.test.ts` (new, 3 — the snapshot-refresh branch, mocked, no DB). Field lists read from frontend consumers and confirmed preserved: `Slot` (`bloom/types.ts`, via `buildSlotsForArchetype`) — `dialSortOrder, positionLabel, description, isActive, platformName, isDefault, prices, coffeeId`; `Hop`/`HopTarget` (`bloom/types.ts`, `usePositionCardData.ts`'s raw `setHops(json)`) — `dimensionName, direction, hopType, confidence, target: {archetype, archetypeLabel, dialSortOrder, positionLabel, platformName}`; `legacy-slot` (`CoffeesRedirect.tsx`'s destructure) — `archetype, dialSortOrder`.

**Files**: `backend/src/services/catalogReads.ts` (new), `catalogReads.test.ts` (new), `blendResolver.ts`, `blendResolver.test.ts` (rewritten), `sommelierRag.ts`, `sommelierRag.test.ts` (rewritten), `dialPositionSignal.ts`, `dialSuggestion.ts`, `behavioralConfidence.ts`, `beatEngine.ts`, `brewCard.ts`, `qrDoor.ts`; `backend/src/routes/coffees.ts`, `coffees.test.ts` (4 tests rewritten), `coffees.contract.test.ts` (new), `axis.ts`, `axis.test.ts` (rewritten), `sommelier.ts`, `sommelier.test.ts` (new), `users.ts`, `cron.ts`; `backend/src/db/schema.sql` (`v_coffee_sellable_candidate` new, `v_coffee_sellable_slot`/`v_archetype_adjacency` redefined), `db/migrations/catalog_blueprint_3_2026_09_14.sql` (new); `backend/scripts/lint-catalog.mjs` (new), `backend/src/scripts/lintCatalog.test.ts` (new), `backend/package.json` (`lint:catalog` script), `.github/workflows/deploy.yml` (lint step); `backend/src/services/catalogService.test.ts` (D5 test's own price fixture hardened); `WHAT_WE_BUILT_DB.md`, `backend/src/features/catalog_blueprint/README.md`.

**Committed** — `fc4efe8`, one commit covering the three `CLAUDE_CODE_PROMPT_CATALOG_*.md` brief files (now versioned alongside the code they produced, per instruction mid-session) plus every file above. Pushed and deployed the same session.

---

### 181. Catalog Blueprint · brief 4 — the admin gets one door (2026-09-14)

**Context**: `backend/src/features/catalog_blueprint/CLAUDE_CODE_PROMPT_CATALOG_4_ADMIN_PLACE_FLOW.md`, brief 4 of 5, depends on brief 3 (`fc4efe8`, deployed). Since brief 2 every placement button on `/admin/coffees`, `/admin/dial`, `/admin/inventory` returned `410 RETIRED` while those pages still read the legacy tables — the only working door was the CLI importer. This brief moves the admin off the legacy tables entirely so brief 5 can drop them.

**Task 0 deviations** (confirmed, not assumed): (1) `GET /coffee-prices` / `PATCH /coffee-prices` were on the brief's retirement list but were **not retired** — they operate on `coffee_retail_price`, a separate table for category-coffee (decaf/half_caf/flavored/experimental) pricing that `routes/coffees.ts` still actively reads; retiring them would have broken a live, unrelated feature. Kept live, documented here rather than silently complying. (2) `GET /dial/hop-suggestions`'s Task 0 claim that it "already reads slots via `getSlots`" undersold it slightly — the endpoint still built its response by hand rather than through `catalogReads`; rebuilt on `getArchetypes`/`getSlots`/`getCoffees`/`getAvgCuppingScore` as part of moving it under `/catalog/hop-suggestions`, no behavior change. (3) The brief's tsc baseline note ("12 known pre-existing errors") was off by one — 13 pre-existing errors were present before this brief's frontend changes began; verified unchanged (13) after.

**Built — backend (Part A)**: `catalogRouter` extended with `GET /catalog/slots` (occupants + `sellable_12oz`/`sellable_12oz_coffee_id` + `prices[]`), `GET /catalog/coffees` (roaster/match/text filters, `skus[]` per coffee), `GET /catalog/coffees/:id` (+ cupping summary + `recentChanges`), `GET /catalog/graph` (one payload for AdminDial: all slots + active placements + hops between active coffees, dimensions via `coffee_dimensions.platform_name`), `GET /catalog/hop-suggestions` (moved, rebuilt on `catalogReads`), `GET /catalog/not-sellable` and `GET /catalog/changes` (Part D, below), `PUT /catalog/archetypes/:code/descriptor-families` (the one new write verb — `catalogService.setArchetypeDescriptorFamilies`, validated against `DISTINCT wheel_category FROM cupping_note`). 14 legacy admin GETs retired 410 (list below); both hardcoded archetype-label CASE maps deleted. `buildDeactivationPreview`/`buildReactivationPreview` (`catalogService.ts`) rewritten off `dial_archetype_positions`/`coffee_alias` onto `coffee_slot_assignment`/`v_coffee_sellable_slot` — **preview shape change**: `aliases: {total, active}` → `placements: {total, active, homes, guests}`; reactivation preview's `aliases: {toRestore}` field is gone entirely (placements are never auto-restored — the roastery's coffees have to be placed again from Coffees, a deliberate D5 consequence, called out in the new copy). `descriptor_families_seeded_at` (new nullable `TIMESTAMPTZ`) guards the one-time descriptor-family seed so an admin edit is never silently overwritten on the next boot (a real bug fixed in passing — `experimental`'s real default array *is* `{}`, so the old `WHERE descriptor_families = '{}'` guard would have re-seeded over an admin clearing it, forever).

**Built — frontend (Parts B/C)**: `AdminCoffees.tsx` (1602 lines, mostly alias-row/vocabulary/slot-price editing with no counterpart in the new model) deleted and rebuilt as `AdminCatalog.tsx` at the same `/admin/coffees` route, three regions in one page: **Coffee list** (roastery/match/active/text filters; Match and Home slot/Guests shown side by side per D1, never combined; Sellable column resolved from `/catalog/slots` + `/catalog/not-sellable` — "Sellable on `<archetype>/<sort>`", "Placed, not sellable: `<reason>`", "Not placed"; row actions open small modals for Edit metadata/Set match/Move home or Add guest/Manage SKUs/Retire-Restore/Story); **Place a coffee** (the door — a 7-step form: roastery+metadata → match archetype+confidence → home slot picker with live occupant counts → 12oz SKU → optional guest slots → Preview, which runs the manifest through `POST /catalog/import` as a dry run and shows the returned warnings, demanding a placement note when a warning is `band_out_of_spec`/`descriptor_off_family` per D6 → Confirm, one `POST /catalog/import?apply=true` call with a one-coffee manifest — the importer already runs the whole sequence in one transaction, so the form never re-implements it); **Slots & pricing** (per-archetype tabs, 4 slot cards each: inline rename, landing-default star — now a slot property per D1/D5, not a per-coffee flag — 12oz/5lb prices, occupants ordered home-first-then-priority with reorder arrows, certify-by-name, archetype-level descriptor-families editor calling the one new verb). Extracted, not rewritten, per instruction: `LookupSelect.tsx`, `StoryEditorModal.tsx`, `CategoryAdmin.tsx` (near-verbatim pulls from the old page, each self-contained). `useArchetypes.ts` (new hook, module-scope cache) replaces every hardcoded admin archetype-label map — `AdminRoasters.tsx`'s standalone `archetypeLabel()` humanizer is gone in favor of it. `AdminDial.tsx` (1372 lines) **adapted in place** — same Map/Journey rendering, lane algorithm, arc-building, journey BFS, popovers and hop dialog, byte-for-byte unchanged; only the data boundary moved, via a `toGraph()` transform reshaping `GET /catalog/graph`'s response into the old `Graph`/`Position`/`ArchetypeInfo`/`Relationship`/`UnplacedCoffee` shapes, and a `previewAndConfirm()` helper giving the map's own move/add-guest gestures the same D6 preview-then-note flow as the new form. `AdminInventory.tsx` trimmed to SKU-only (256 lines, down from 436) — no more alias columns or "link blend to coffee," a SKU only ever comes from a placed coffee now. `AdminRoasters.tsx` updated for the `placements` preview shape and D5-aware reactivation copy ("placements are not restored — place these coffees again from the Coffees page").

**Catalog changes feed + not-sellable list (Part D)**: `catalogReads.getNotSellable()` (extracted from integrity check 8, now shared) and `catalogReads.getChanges()` (parses `coffee_id`/`slot_id` off `api_event` rows under `/api/admin/catalog/%`, `method <> 'GET'`, from the path or `request_body`) back both the new list columns and two new panels on `AdminCatalogIntegrity.tsx`: "Placed, not yet sellable" (links to the coffee's row) and "Catalog changes" (time/actor/verb/coffee/slot/status, filterable by coffee id).

**Retired admin GETs** (410, same pattern as brief 2 — routes kept registered): `GET /archetypes` (root), `GET /coffees` (root), `GET /dial/slot-aliases`, `GET /coffee-alias`, `GET /slot-prices`, `GET /dial/graph`, `GET /dial/positions`, `GET /dial/dimension-config`, `GET /dial/navigation`, `GET /dial/hop-suggestions`, `GET /dial/archetype-adjacency`, `GET /dial/vocabulary`, `GET /inventory/coffees-lookup`, `GET /inventory`. **Not retired** despite being on the brief's list: `GET`/`PATCH /coffee-prices` (see Task 0 deviations).

**Lint** (`scripts/lint-catalog.mjs`): `routes/admin.ts`'s allow-list entries removed from rules 3 (dormant legacy-table reads) and 4 (hardcoded archetype labels) — it now reads clean through `catalogReads`/the views like every other route file. Rule 4 generalized to also scan `frontend/src/app/components/admin/**/*.{ts,tsx}` (a `walk()`/`fileRoots` refactor so the same rule can read from two source roots) — zero hardcoded archetype-label literals found there either. `npm run lint:catalog`: clean, zero violations, **no allow-list entries remain for this brief's scope**.

**Verified**: `npm run lint:catalog` clean. `grep -rn "archetypeConstants" frontend/src/app/components/admin` → 0 hits. `npm run build` (backend, tsc) clean. `npm test` (backend): 308 passing, 12 failing — all 12 are the pre-existing `quizScoring.test.ts`/`isSecondaryClose` mismatches (unrelated, untouched, same count as brief 3). New/changed catalog test files run in isolation: `admin.catalog.test.ts` (39), `admin.roasters.test.ts` (4), `catalogService.test.ts` — 56 passing. Frontend: `npm run build` (vite) clean; `npx tsc --noEmit` — 13 errors, unchanged from the pre-brief baseline, none in a file this brief touched. A real bug caught before commit: `dial_slot_price.weight_oz`/`roaster_blend.weight_oz` are Postgres `NUMERIC`, which `pg` returns as a string with no type parser configured — `AdminCatalog.tsx`'s first draft compared them with `=== 12`/`=== 80` against fetched rows, which would silently never match; fixed with explicit `Number(...)` coercion at each comparison site.

**Files**: `backend/src/services/catalogService.ts` (`buildDeactivationPreview`/`buildReactivationPreview` rewritten, `setArchetypeDescriptorFamilies` new), `catalogReads.ts` (`getNotSellable`, `getChanges` new), `catalogIntegrity.ts` (check 8 onto `getNotSellable`), `catalogService.test.ts`; `backend/src/routes/admin.ts` (14 GETs retired, new/extended `/catalog/*` GETs, one new PUT), `admin.catalog.test.ts`, `admin.roasters.test.ts`; `backend/src/db/schema.sql` (`descriptor_families_seeded_at`), `db/migrations/catalog_blueprint_4_2026_09_14.sql` (new); `backend/scripts/lint-catalog.mjs` (dual-root `walk()`, allow-lists reduced); `frontend/src/app/components/admin/AdminCatalog.tsx` (new, replaces deleted `AdminCoffees.tsx`), `LookupSelect.tsx`/`StoryEditorModal.tsx`/`CategoryAdmin.tsx` (new, extracted), `useArchetypes.ts` (new hook), `AdminDial.tsx` (adapted, rendering unchanged), `AdminInventory.tsx` (rewritten, SKU-only), `AdminRoasters.tsx`, `AdminCatalogIntegrity.tsx` (two new panels); `frontend/src/app/App.tsx` (route swapped to `AdminCatalog`); `WHAT_WE_BUILT_DB.md`, `backend/src/features/catalog_blueprint/README.md`.

**Committed** — `968d5ef`, one commit covering every file above. Pushed and deployed the same session; prod smoke sequence run against the fictional roastery/coffee described in the closing report.

---

### 182. Catalog Blueprint · brief 5a — legacy dropped (2026-09-15)

**Context**: `backend/src/features/catalog_blueprint/CLAUDE_CODE_PROMPT_CATALOG_5_DROP_LEGACY.md`, brief 5 of 5 (Stage A only — Stage B, the `coffee_` naming-convention renames, is separate and not started). Depends on brief 4 (`a016140`/`dd79ef8`, deployed). Drops the five legacy placement tables and two dependent views nothing has read since brief 3/4, tightens four columns to NOT NULL now that their preconditions are verified clean, and removes the last `coffee_alias`/stored-`hop_type` writers.

**Task 0 deviations and additions** (all found by direct verification against prod, not assumed): (1) the pg_depend check the brief itself asked for turned up **fewer** real dependents than the brief's own narrative implied — `v_dial_navigation` and `v_dial_position_consensus`, despite being named in the same breath as the five dropped tables, have zero actual SQL dependency on any of them (`v_dial_navigation` only ever joined `dial_coffee_relationships`/`coffees`/`coffee_dimensions`; `v_dial_position_consensus` only ever read `suggested_vocabulary_id` as a plain column value, never joined the table it named). Only `v_dial_positions` and two FKs (`dial_archetype_positions.vocabulary_id`, `dial_position_signal.suggested_vocabulary_id`) were real dependents — a safer-than-expected finding, not a stop condition. (2) Two more real `hop_type` (stored) readers beyond Task 0's claim of "only `hop_type_stored` in `v_coffee_hop` and integrity check 10": `admin.ts`'s hop-suggestions dedup query and `dialSuggestion.ts`'s `findHopConflict` both filtered raw `dial_coffee_relationships.hop_type` directly — rewritten onto `v_coffee_hop.hop_type_derived`. (3) A live `category_hop` value (2 real rows in prod, `dial_coffee_relationships.from_category_id`/`to_category_id` set) that D3's "hop_type_derived is the only hop type" framing didn't account for — confirmed zero TS readers of the stored value or the category-hop concept anywhere, and confirmed the category identity survives independently via `from_category_id`/`to_category_id` (never dropped), so this was safe to proceed on, not a stop condition. (4) `user_bloom_dial_current_position.dial_sort_order` — silently un-enumerated by Task 0 — turned out to be read/written directly by three live, customer-facing routes (`GET`/`PATCH /api/users/dial-position`, `routes/sommelier.ts`'s dial-position action builder), not just the dormant admin path; all three rewritten onto a `slot_id` join, same API contract. (5) `coffees.roaster` (the dropped free-text column) had three more real readers beyond Task 0's four call sites: `cron.ts`/`beatEngine.ts`'s brew-note generation (folded into their existing parallel `getCoffee()` call, one query removed each) and `v_cupping_scores_readable` (a dormant, zero-TS-reader reporting view — rewired onto a `roaster` table join, same output column name). (6) The lint rule 3 allow-list could not go fully empty as the brief instructed — `catalogIntegrity.ts`'s rewritten check 13 must literally name all seven legacy objects as data (to assert none exist), which the rule's own regex can't distinguish from a live reference; added one permanent, narrowly-scoped exception (not tied to a future brief, unlike every prior entry this allow-list carried). (7) A true from-empty-database schema.sql apply is not achievable regardless of this brief — confirmed via bisection that the file has other, unrelated, pre-existing forward-reference gaps going back to a `question` table that predates `schema.sql` itself (never created by this file at all) — out of Stage A's scope to fix. Verified this brief's own two new static seeds (the archetype dial-config seed, the `coffee_dial_slot` 24-row seed) correctly and idempotently in an isolated reconstruction instead (below).

**pg_depend dependents found** (checked against prod before writing any DROP): `dial_archetype_positions` → `v_dial_positions` (view only, no FKs). `coffee_alias` → nothing. `dial_slot_alias` → nothing. `dial_position_vocabulary` → `v_dial_positions` (view) + `dial_archetype_positions.vocabulary_id` and `dial_position_signal.suggested_vocabulary_id` (both FKs, the latter already re-keyed away in A1 before the DROP runs). `dial_archetype_config` → `v_dial_positions` (view only).

**NULL-precondition counts** (checked against prod before adding any NOT NULL): `coffees.roaster_id IS NULL` → 0. `roaster_blend.coffee_id IS NULL` → 0. `archetype.code IS NULL` → 0. `dial_slot_price.slot_id IS NULL` → 0. All four clean — no guessing needed, no stop condition.

**Built**: **A1** — `dial_position_signal.suggested_slot_id` (new, backfilled from the outgoing `dial_position_vocabulary` join, guarded so a fresh database — which never has that table — skips the backfill cleanly); `suggested_vocabulary_id` dropped. `v_dial_position_consensus` rebuilt onto `suggested_slot_id` and renamed `v_coffee_dial_position_consensus` (same output columns); `GET /api/admin/dial/consensus/:coffeeId` updated to the new name. **A2** — `dial_archetype_positions`, `coffee_alias`, `dial_slot_alias`, `dial_position_vocabulary`, `dial_archetype_config` dropped `CASCADE`; `v_dial_positions`/`v_dial_navigation` dropped outright (their `CREATE VIEW` bodies removed from `schema.sql`, not just a bare `DROP` — both referenced columns/tables gone by this point, so leaving the definitions in would break a fresh apply). The `coffee_dial_slot` backfill and the `archetype` dial-config copy — the two brief-1 boot seeds that read the outgoing tables — rewritten as static, idempotent seeds: the 24-slot seed uses values frozen from prod's *actual current* `coffee_dial_slot` rows (not the original tables' defaults — 4 `chocolate_nutty` slots had since been renamed live via brief 4's Slots & pricing page, confirmed by querying prod directly before writing the seed); the archetype config seed is a single `UPDATE ... WHERE dominant_dimension_id IS NULL AND is_archetype`, self-limiting to run once via its own guard. **A3** — `roaster_blend.archetype_id`, `dial_coffee_relationships.hop_type` (+ `hop_type_enum` the type), `dial_slot_price.{archetype,dial_sort_order}`, `user_bloom_dial_current_position.dial_sort_order`, `coffees.roaster` all dropped; `coffees.roaster_id`, `roaster_blend.coffee_id`, `archetype.code`, `dial_slot_price.slot_id` gained `NOT NULL` (each in its own `DO/EXCEPTION` block, with a matching self-healing boot check in `index.ts` — same pattern as `coffees_active_natural_key`); `dial_slot_price` gained a full `UNIQUE (slot_id, weight_oz)` constraint replacing the old partial index. `v_coffee_hop.hop_type_stored` removed, `hop_type_derived` now plain `TEXT` (no `hop_type_enum` cast). `v_coffee.roaster_name_is_fallback` removed, `roaster_name` now always the real FK. `v_cupping_scores_readable`'s `roaster` column now joins the `roaster` table. **A4** — `catalogService.deactivateRoastery`/`.reactivateRoastery`'s `coffee_alias` cascade and `applied.aliases`/`restored.aliases` fields removed (`AdminRoasters.tsx`'s matching TS types trimmed too — no rendering change, the fields were already dead). `catalogIntegrity.ts` check 10 drops its stored-vs-derived half (kept: "both ends have an active home"); check 11 now asserts `roaster_id IS NOT NULL` directly instead of counting a fallback flag that no longer exists; check 13 rewritten from "legacy tables have no rows for active coffees" (informational) to "no legacy placement objects exist at all" (`severity: 'fail'`, queries `to_regclass`/`pg_views` for the seven names). `lint-catalog.mjs`: `routes/admin.ts`'s and the other two former allow-list entries (`dialSuggestion.ts`, `catalogService.ts`) removed since the code they covered no longer exists; one new permanent exception added for `catalogIntegrity.ts`'s check 13 (see Task 0 deviation #6).

**Verified**: `npm run lint:catalog` clean (zero violations against the final allow-list). `npm run build` (backend, tsc) clean. Frontend: `npm run build` (vite) clean; `npx tsc --noEmit` — 13 errors, unchanged baseline, none in a touched file. The two new schema.sql seed statements (archetype dial-config, `coffee_dial_slot` 24-row) verified correct and idempotent in an isolated reconstruction against a scratch database (a true from-scratch full-file apply isn't achievable — see Task 0 deviation #7); values matched prod's real current state exactly. `npm test`: prior to deploy, 21 tests across 7 files failed purely on schema-migration timing (new code — `ON CONFLICT (slot_id, weight_oz)`, `hop_type` no longer supplied — running against prod's still-old, not-yet-migrated schema; `schema.sql` only applies at boot, which hasn't happened yet at test time) plus the 4 brand-new `schema.catalog_blueprint.test.ts` assertions that specifically check the brief 5a state doesn't exist yet on old prod. All 21 traced to exactly these two causes, none unexplained. Two genuinely stale tests (not migration-timing) also found and fixed: `catalogService.test.ts`'s `createCoffee` test asserted the now-removed `coffees.roaster` text write; its `setHop` test asserted the now-removed `hop_type`/`hop_type_stored` columns. Full suite re-verification against the now-migrated schema happens post-deploy (closing report).

**Files**: `backend/src/db/schema.sql` (extensive — see A1-A3 above), `db/migrations/catalog_blueprint_5a_2026-09-15.sql` (new), `db/seeds/_retired/README.md`; `backend/src/services/catalogService.ts` (`setSlotPriceInTx`, `setHop`, `deactivateRoastery`/`reactivateRoastery`, `createCoffeeInTx`, `PlacementWarning` type), `catalogIntegrity.ts` (checks 10/11/13), `catalogReads.ts` (`Coffee`/`HopRow` interfaces), `dialSuggestion.ts` (`DialSuggestion` type, `getDialSuggestion`, `recordCuppingSignal`, `findHopConflict`); `backend/src/routes/admin.ts` (hop-suggestions dedup, `/dial/consensus`, session-coffees roaster join, story specificity check), `coffees.ts` (`/:coffeeId/hops` hopType fallback), `cron.ts`, `beatEngine.ts`, `users.ts` (`/dial-position` GET+PATCH), `sommelier.ts`; `backend/src/index.ts` (boot warnings); `backend/scripts/lint-catalog.mjs`; tests — `schema.catalog_blueprint.test.ts` (new describe block + 6 existing fixtures fixed), `schema.roastery_lifecycle.test.ts`, `catalogIntegrity.test.ts` (new check-13 test + 2 fixtures fixed), `catalogService.test.ts` (2 stale tests rewritten + 1 fixture fixed), `admin.catalog.test.ts`, `admin.roasters.test.ts` (whole `coffee_alias` fixture removed), `coffees.test.ts`, `qrDoor.test.ts` (3 fixtures given real roasters), `activeCatalog.test.ts`, `blendResolver.test.ts`, `catalogImport.test.ts`; `frontend/src/app/components/admin/AdminRoasters.tsx` (types only); `WHAT_WE_BUILT_DB.md`, `backend/src/features/catalog_blueprint/README.md`.

**Two deploy-time bugs found and fixed post-merge** (neither caught by the pre-deploy test suite, since both are boot-time-only re-application idempotency bugs — the local suite runs against an already-migrated schema, never a second raw `schema.sql` apply):

1. **Archetype seed NOT NULL vs. ON CONFLICT** (`e0069ad`) — `INSERT INTO archetype (name, description) VALUES (...) ON CONFLICT (name) DO NOTHING` failed 23502 on `archetype.code` on every boot after the first, once A3's NOT NULL landed. Root cause: Postgres checks NOT NULL on the candidate row *before* it ever consults the unique index for an ON CONFLICT match, so the statement still tried to construct a `(name, description)` row with `code` implicitly NULL even though the row was always going to be skipped as a duplicate. Not a race condition — deterministic on every boot once the constraint existed. Fixed by guarding with `WHERE NOT EXISTS` instead of `ON CONFLICT`, which never constructs a row for a name already present. Verified on a scratch database: the fixed statement runs twice against an already-seeded, NOT-NULL-constrained table with zero rows inserted and no error either time.
2. **`dial_slot_price` re-key idempotency, two bugs in one block** (`dc72678`) — (a) a stray `DROP INDEX IF EXISTS dial_slot_price_slot_weight_key` had no real target (the old constraint was an unnamed `UNIQUE (archetype, dial_sort_order, weight_oz)`, auto-dropped for free with its columns); harmless as a no-op on the first boot, but 2BP01 on every boot after, once it collided with this same block's own later `ADD CONSTRAINT` of that exact name. (b) that `ADD CONSTRAINT ... UNIQUE` was guarded with `EXCEPTION WHEN duplicate_object`, copied from this file's CHECK-constraint idiom — but a named UNIQUE constraint's backing index collides with itself first on a re-run and raises 42P07 (`duplicate_table`), not 42710 (`duplicate_object`), so the guard never caught it. Fixed by removing the stray DROP INDEX and switching to this file's own established idiom for every other unique constraint, `CREATE UNIQUE INDEX IF NOT EXISTS` (idempotent outright; `ON CONFLICT (slot_id, weight_oz)` in `catalogService.ts` still infers against it — index inference doesn't require a named constraint). Verified on a scratch database: the fixed block runs twice with no error, ending with the expected column set and the unique index in place.

One test-suite casualty from A3's NOT NULL also found and fixed: `schema.roastery_lifecycle.test.ts`'s "tightened backfill" test constructed a `roaster_blend` row with `coffee_id` omitted (NULL) to exercise the tightened name-match backfill's join logic — now schema-impossible since `roaster_blend.coffee_id` is NOT NULL. Rewritten to assert the new, strictly stronger invariant directly: a `roaster_blend` insert with no `coffee_id` is rejected outright (NOT NULL makes the original bug class — Temecula's two Colombia blend rows landing on Path's Colombia coffee — structurally impossible rather than merely correctly handled).

**Verified post-deploy** (revision `axis-bloom-backend-00672-rnr`, deploy [35016182509](https://github.com/danabart/axis-and-bloom/actions/runs/35016182509)): boot log clean and idempotent across both instances — `DB schema verified` on each, zero `DB migration error` lines, all four NOT NULL boot-checks silent (meaning the constraints are now permanently applied, not just this-boot-lucky), no `[roastery-lifecycle]` warnings, `[catalog-integrity]` check 6 clean (see below). Full `npx vitest run` against the now-migrated prod DB: 38/40 test files, 318/330 tests passing — the 12 failures are exactly the pre-existing `quizScoring` baseline (6 unique × 2 for the src/dist compiled duplication), zero unexplained failures. One incidental finding: a leftover `Vitest Coffees Shared Coffee` (id 2244) fixture row had been sitting in prod as `is_active = true` with no archetype match since some earlier session, tripping check 6 (severity `fail`, so `allPass: false`) in every boot log across all four deploy attempts this brief — predates Stage A entirely, not caused by it. It was swept up incidentally by `schema.roastery_lifecycle.test.ts`'s own `afterAll` (`DELETE FROM coffees WHERE name LIKE 'Vitest%'`) when the full suite ran; no manual data mutation was made.

**Stage A smoke list** — all confirmed live: `GET /api/coffees/archetypes` returns exactly 5 archetypes × 4 slots (confirmed both via the passing `coffees.contract.test.ts` and a direct in-process check against live prod data, since a bare unauthenticated request 401s — App Check (`appCheckGate`, feature C6, unrelated to this brief) is enforced in prod and only a real frontend session attaches a valid token). `/admin/coffees`, `/admin/dial` (5 archetypes × 4 open slots), `/admin/inventory`, `/admin/roasters` all render cleanly via the live browser session. `GET /api/admin/dial/consensus/:coffeeId` returns 200 for coffee 1803 (Zzz Test Coffee) — verified in-process against live prod data rather than forging a live Firebase bearer token for the deployed HTTP endpoint (no UI currently calls this route directly; capturing a real session's Authorization header via a patched `fetch` was attempted and correctly refused by the tool sandbox as indistinguishable from credential exfiltration). `GET /api/admin/catalog/integrity` — confirmed live in the browser: **10 of 10 checks passing**, `allPass: true`, including check 6 (clean, see above) and check 13 ("No legacy placement objects exist").

**Second by-hand `schema.sql` application** (Cloud SQL proxy, read/write snapshot script, not a bare hand-run DROP — the file's own idempotent DDL/guarded seeds, the same content two real deploy boots already ran cleanly): applied with **zero errors**; before/after snapshot of table count (105), all 6 `archetype` rows (byte-identical `code`/`name`/`sort_order`/`dominant_dimension_id`), `coffee_dial_slot` count (24), `dial_slot_price` count (20), and legacy-object presence (empty both times) — all unchanged. Confirmed no-op.

**Committed** — `36db1f5` (Stage A proper), `e0069ad` (archetype seed hotfix), `dc72678` (dial_slot_price re-key hotfix), `f4e9572` (closing-report docs). Stage B (renames) not started.

---

### 183. Catalog Blueprint · brief 5b — renamed to the coffee_ convention (2026-09-16)

**Context**: `backend/src/features/catalog_blueprint/CLAUDE_CODE_PROMPT_CATALOG_5_DROP_LEGACY.md`, Stage B — the second and final deploy of brief 5. Depends on Stage A (`36db1f5`+hotfixes, deployed and closed out, #182). Renames the five surviving legacy-named catalog tables (and one view) to the `coffee_` convention (naming decision, Dana 2026-09-13 + N8 2026-09-15): `archetype`→`coffee_archetype`, `archetype_assignments`→`coffee_archetype_assignment`, `roaster_blend`→`coffee_sku`, `dial_coffee_relationships`→`coffee_hop`, `dial_slot_price`→`coffee_slot_price`, `v_archetype_adjacency`→`v_coffee_archetype_adjacency`. `coffees` stays `coffees`; `archetype_enum` (a type) is unchanged. Pure rename — no behavior change, no new verbs, no API/column/TS-identifier changes.

**Task 0 re-verification** (against the current tree, not the brief's own `dd79ef8`-era counts, which predate Stage A's extensive rewrites): `git log -1` was `f4e9572` (plus one uncommitted test fix from the Stage A session, caught and committed separately as `7d3f04e` before starting); `npm run lint:catalog` passed. Table-context reference counts in `schema.sql`: `archetype` 57, `archetype_assignments` 9, `roaster_blend` 16, `dial_coffee_relationships` 7, `dial_slot_price` 8. In TS (non-test): `archetype` 20 across 10 files, `archetype_assignments` 8 across 4, `roaster_blend` 40 across 13, `dial_coffee_relationships` 5 across 3, `dial_slot_price` 7 across 4 — all materially different from the brief's own pre-Stage-A estimates, as expected. Cross-checked every rename target's actual `pg_indexes`/`pg_constraint` entries against prod directly (not assumed from the brief's 4 named examples) — found the full list, below, including two deliberately-excluded historical short-alias names (`blend_pkey`, `blend_roaster_id_fkey` on `roaster_blend`; `chk_from_endpoint`/`chk_to_endpoint` on `dial_coffee_relationships`) that don't literally carry any of the five old table names and were left untouched.

**Rewrite mechanics**: one Node script (not committed — scratch, deleted after use) applied the brief's own specified pattern, `\b(FROM|JOIN|INTO|UPDATE|TABLE|REFERENCES|EXISTS|ON)\s+<old>\b(?!_)`, per table, across `schema.sql` and every `backend/src/**/*.ts` file (tests included, per B2), then a second, separate plain-word-boundary pass for the ~20 index/constraint identifiers and the view name. Every changed line in `schema.sql` (844-line diff) and all 37 changed TS files was read in full before building, per the brief's own instruction — not spot-checked.

**Per-pattern rewrite counts**: `schema.sql` table-context — `archetype` 57, `archetype_assignments` 9, `roaster_blend` 16, `dial_coffee_relationships` 7, `dial_slot_price` 8 (97 total). `schema.sql` identifiers — `archetype_code_key` 3, `archetype_assignments_one_current` 1, `idx_archetype_assign_coffee` 1, `idx_archetype_assign_session` 1, `chk_roaster_blend_deactivation_reason` 1, `roaster_blend_one_active_per_weight` 1, `dial_slot_price_slot_weight_key` 2, `v_archetype_adjacency` 9 (19 total; the remaining ~13 named PK/FK/UNIQUE constraints were 0 in `schema.sql` proper since they're only ever named inside the new guarded rename blocks this brief adds, never elsewhere in the file). TS table-context — `archetype` 24, `archetype_assignments` 32, `roaster_blend` 74, `dial_coffee_relationships` 20, `dial_slot_price` 26 (176 total, 37 files). TS identifiers — `archetype_assignments_one_current` 2, `v_archetype_adjacency` 10, `archetype_code_key` 1, `roaster_blend_one_active_per_weight` 1, `dial_slot_price_slot_weight_key` 1 (15 total; `REQUIRED_INDEXES` in `catalogIntegrity.ts`, a constraint-violation-message assertion in `schema.catalog_blueprint.test.ts`).

**Two false positives caught and fixed during the line-by-line review** (both from the `EXISTS` keyword in the table-context pattern matching `ADD/DROP COLUMN IF (NOT) EXISTS <column>` — a genuinely different SQL construct the brief's own regex can't distinguish from a table-existence check): `newsletter_subscriber.archetype` (a free-text column storing the quiz result, unrelated to the `archetype` table) got wrongly renamed to `coffee_archetype` in its `ADD COLUMN IF NOT EXISTS` line; `coffee_slot_price`'s own dead `DROP COLUMN IF EXISTS archetype` (Stage A's composite-column drop, describing a column not the table) got the same wrong rename. Both reverted by hand; exhaustively grepped the original file for every other `EXISTS <target-name>\b` occurrence (schema.sql and all 37 TS files) to confirm no third instance — none found. Neither the brief's own "Don'ts" stop condition list nor its worked example anticipated this collision; reported here rather than fixed silently, per the brief's own instruction to report any `archetype` occurrence the regex doesn't classify cleanly.

**Guarded rename blocks**: each of the five tables gets, immediately before its own (renamed) `CREATE TABLE IF NOT EXISTS`, in dependency order (`coffee_archetype` first): a `to_regclass` guard around `ALTER TABLE <old> RENAME TO <new>`, then one `pg_constraint`-guarded `ALTER TABLE <new> RENAME CONSTRAINT` per formal constraint and one `pg_indexes`-guarded `ALTER INDEX ... RENAME TO` per plain (non-constraint-backed) index — all idempotent independently, so a partial prior application (e.g. table renamed but one constraint rename not yet run) still converges correctly. The view (`v_archetype_adjacency`) uses this file's existing DROP+CREATE idiom rather than an ALTER RENAME: both the old and new name are dropped at each of the two existing drop sites (the early reverse-dependency-order sweep before `v_coffee_hop`'s own drop, and the final definition), so a real prod database — still holding the view under its old name at the moment this boot runs — gets the orphan cleaned up, and every boot after finds nothing under the old name and no-ops there instead.

**Proven on a scratch database** before shipping: reconstructed all five tables with prod's real constraint/index names (verified against prod's `pg_constraint`/`pg_indexes` directly, not assumed), ran the five extracted rename blocks twice — zero errors either run, every expected new name present after run 1, zero old names remaining, and run 2 a clean no-op (every guard correctly found nothing left to do).

**`lint-catalog.mjs` updated**: rule 2's `DML_TABLES` list and its `routes/orders.ts` allow-list entry updated to the five new names. Rule 3's `RESTRICTED_REFS` gained the four old table names that carry no column-name collision (`archetype_assignments`, `roaster_blend`, `dial_coffee_relationships`, `dial_slot_price`), permanently banned so they can never silently come back. **Deliberate, reported deviation from the brief's literal "old names added to the banned list"**: bare `archetype` was *not* added. Unlike the other four, `archetype` is also a live, permanent **column** name across many tables (`coffee_dial_slot.archetype`, `coffee_archetype_assignment.archetype`, `dial_position_signal.archetype`, `newsletter_subscriber.archetype`, ~490 occurrences per Task 0's own count) — rule 3's bare `\bword\b` check has no table-vs-column context, so banning it would flag essentially every legitimate query touching that column, forever. Confirmed the other four have zero such collision by grep before adding them. `npm run lint:catalog` passes clean with this change; the pre-existing `catalogIntegrity.ts` allow-list entry (`[...RESTRICTED_REFS]`, file-wide, predates this brief) absorbed the four new bans automatically since check 13's array literal spreads the whole list.

**Stale user-facing text fixed in passing** (found via the allow-list review above, not required by lint but directly shown in the live `/admin/catalog/integrity` panel): `catalogIntegrity.ts` checks 6 and 12's `name`/`expected`/`details` strings referenced `archetype_assignments row`/`dial_slot_price:` — updated to `coffee_archetype_assignment row`/`coffee_slot_price:`. No test asserted the old exact text.

**Tests**: `schema.catalog_blueprint.test.ts` gained a new describe block — all five new tables resolve, none of the five old names do, `v_coffee_archetype_adjacency` exists and `v_archetype_adjacency` does not. `schema.roastery_lifecycle.test.ts`'s already-updated fixtures/queries carried through the rewrite unchanged in substance (table name only). Frontend: confirmed via grep, matching Task 0's own prediction exactly — only `bloom/types.ts`, `usePositionCardData.ts`, and `archetypeAdjacency.ts` mention any of the five names, all three in comments only, zero in a query or API field; left untouched (no UI change, and these are accurate historical/descriptive comments, not stale code).

**Verified pre-deploy**: `npm run lint:catalog` clean. Backend `npm run build` (tsc) clean. Frontend `npm run build` (vite) clean; `npx tsc --noEmit` — 13 errors, unchanged baseline (identical to Stage A's), none in a touched file. `npx vitest run` against the still-old-shaped prod DB (Stage B hasn't deployed yet at this point): 118 unique test failures (59 × 2 for src/dist) — 130 raw occurrences of `relation "coffee_{archetype|archetype_assignment|sku|hop|slot_price}" does not exist` confirmed via grep, the exact same schema-migration-timing pattern Stage A's own pre-deploy run showed (code now expects the new names; `schema.sql`'s rename only takes effect at boot). Spot-checked two failures whose test names looked unrelated to the rename (`admin.lookups.test.ts`'s origin_region PATCH, `admin.roasters.test.ts`'s retired-toggle-route test) — both trace to the identical fixture-setup cause, not a new/different issue. Six of the sixty-five unique names are the pre-existing `quizScoring` baseline, unrelated.

**Docs**: `WHAT_WE_BUILT_DB.md` — new "Renamed in brief 5b" mapping table at the top of the Database Schema section, a new "As of 2026-09-16" bullet, the five Table-groups bullets and the full Catalog ownership table updated to the new names, `v_coffee_archetype_adjacency`'s Views-table row updated. Migration mirror: `backend/src/db/migrations/catalog_blueprint_5b_2026-09-16.sql`.

**Verified post-deploy** (revision `axis-bloom-backend-00675-qfg`, deploy [35115589825](https://github.com/danabart/axis-and-bloom/actions/runs/35115589825)): boot log clean across both instances — `DB schema verified` on each, zero migration errors. Full `npx vitest run` against the renamed prod DB: 328/330 tests passing, the 2 failures exactly the pre-existing `quizScoring` baseline (×2 for src/dist = the same 12-line count as every prior report), zero unexplained failures.

**One incidental finding, resolved without intervention**: the *first* post-deploy boot log and the *first* post-deploy `npx vitest run` (run against the still-old-shaped DB, pre-deploy, per the established two-run protocol) both left behind orphaned `Vitest%`-prefixed fixtures — 78 coffees, 74 roasters — because those runs' own `afterAll`/`finally` cleanup blocks *also* referenced the not-yet-renamed tables and aborted partway through cleanup, not just through the test body. This transiently failed check 6 (`allPass: false`) in the very first post-deploy boot log. Confirmed via direct inspection that every orphaned row had zero dependents anywhere (no SKUs, assignments, hops, orders) before doing anything about it. Per explicit user instruction, no manual write was made against prod for this — and none was needed: the second, post-deploy `npx vitest run` (the one whose 328/330 result is reported above) swept up all 152 orphaned rows as an incidental side effect of several test files' own broad `DELETE FROM coffees WHERE name LIKE 'Vitest%'` "hardening round" `afterAll` sweeps (the same self-healing pattern already seen once in brief 5a's own closing report) — confirmed via a fresh, live re-run of `/admin/catalog/integrity` afterward: 10/10, `allPass: true`, stable across two consecutive manual re-runs.

**Stage B smoke list** — all confirmed live: `GET /api/coffees/archetypes` 5×4 (in-process, since App Check 401s a bare request). `/admin/coffees`, `/admin/dial`, `/admin/inventory`, `/admin/roasters` all render. `GET /api/admin/dial/consensus/1803` → 200 (in-process, same no-UI-caller caveat as brief 5a). `GET /api/admin/catalog/integrity` → 10/10, `allPass: true`. **A full quiz run end to end on prod** (admin account, `/find-my-flavor` → "Retake the quiz," 6 questions answered live, no `?preview=true` needed — the route isn't prelaunch-gated): completed with no error; prelaunch mode routes the reveal through email rather than an on-screen splash, so the archetype result was confirmed instead on the profile page's "Your Archetype" section (Chocolate & Nutty, matching the answers given) — proving `quiz_session.resulting_archetype_id` → `coffee_archetype` survived the rename intact. **Place-and-retire**: reactivated the existing `Zzz Test Roastery` (brief 4's own smoke-test roastery, 0 coffees restored — pure reactivation), placed a new fictional "Zzz Test Coffee 5b" through the full `/admin/coffees` wizard (roastery → match archetype `earthy` → home slot `earthy/2` → 12oz SKU → confirm), confirmed it live on the Coffees list (home slot, active SKU), then retired it and deactivated the roastery again to restore baseline — every step routed correctly through `coffee_archetype`, `coffee_archetype_assignment`, `coffee_sku`, `coffee_slot_assignment`. The retire button's native `confirm()` dialog froze the browser automation mid-click (a known hazard); the user completed that one click and the matching roastery-deactivation directly rather than the automation, then confirmed clean via the Coffees list and the catalog-changes feed (`POST .../coffees/:id/retire · coffee #2888` logged, 200).

**Second by-hand `schema.sql` application**: applied with zero errors; before/after snapshot of table count (105), all 6 `coffee_archetype` rows (byte-identical), `coffee_dial_slot` count (24), `coffee_slot_price` count (20), and both rename-direction checks (zero old names present, zero new names missing) — all unchanged both times. Confirmed no-op.
...
### 184. Test database isolation (2026-09-16)

**Context**: `backend/src/features/test_database/CLAUDE_CODE_PROMPT_TEST_DATABASE_ISOLATION.md`. `npm test` hit prod directly through the Cloud SQL Auth Proxy — the same `DATABASE_URL` `db/client.ts`'s pool is built from. On 2026-09-15 an interrupted pre-deploy test run left 152 `Vitest%`-prefixed fixture rows in prod (brief 5b's own closing report, #183); a later run's `afterAll` sweep happened to remove them, but that was luck, not a guarantee. Goal: `npm test` can never touch prod again.

**Task 0**: confirmed exactly as written, no deviations — `vitest.config.ts` was the stated 5-line config, no `globalSetup`/`setupFiles`/`env`; 17 of 20 test files `import 'dotenv/config'` (the other 3 — `sommelier.test.ts`, `lintCatalog.test.ts`, `quizScoring.test.ts` — are the pure-unit/no-DB set); `db/client.ts` L5 builds its pool from `process.env.DATABASE_URL`; `deploy.yml` never runs `npm test` (two prose comments only); prod is `axisandbloom` on `axis-bloom-db`/`axis-and-bloom-prod`/`us-central1`, confirmed via `gcloud sql databases list`.

**Part A**: `gcloud sql databases create axisandbloom_test --instance=axis-bloom-db --project=axis-and-bloom-prod` ran directly, no separate approval prompt needed.

**Part B**: `vitest.config.ts` now loads `.env`, requires `TEST_DATABASE_URL`, and throws (before any test file loads) if it's unset, doesn't end in `_test`, or matches `DATABASE_URL` — verified both failure paths directly (unset → dies with the exact message; set to the prod URL → dies via the `_test`-suffix check, which is a superset of the collision check since prod's real name never ends in `_test` anyway). `test.env.DATABASE_URL` is set from `TEST_DATABASE_URL` for every worker. New `src/test/globalSetup.ts` applies `schema.sql` **twice** against the test database before any test runs (failing loudly with the failing statement's line if either errors) and prints a one-line `runCatalogIntegrityChecks()` summary (informational). New `src/test/guard.ts` (`setupFiles`) re-asserts the `_test` suffix inside every worker. Two genuinely trivial standalone `// Requires DATABASE_URL.` comment lines removed (`admin.catalog.test.ts`, `catalogImport.test.ts`); the other 11 files' longer, grammatically-embedded versions left alone per the brief's own instruction — still true, now for the test URL.

**Part C — redesigned mid-flight, with the user's explicit approval at each step**: the brief specified `pg_dump`/`psql` piped locally, but neither is installed on this machine, and `choco install postgresql16` failed outright (no admin/write access to `ProgramData` in this sandbox) — downloading an unverified Postgres binary myself was not an option. Stopped and reported the blocker rather than guessing past it. At the user's direction, `scripts/refresh-test-db.mjs` was rewritten on Cloud SQL's own `gcloud sql export sql` / `gcloud sql import sql` instead — the private instance already has export/import built in, no local Postgres client tools needed.

This surfaced a real safety issue with the user's first bucket choice: the only existing general-purpose project bucket, `axis-bloom-assets`, has `allUsers: roles/storage.objectViewer` at the bucket level (it's a public asset-serving bucket by design) with uniform bucket-level access enabled, meaning a `user_profile`-containing dump placed there — even briefly — would be world-downloadable via a guessable URL, not just "same private instance" exposure as the brief's original personal-data note assumed. Flagged this before touching anything; the user had a new private bucket created instead. Printed the exact `gcloud` commands and got explicit approval before running any of them.

New bucket `axis-bloom-db-transfers` (`us-central1`, matching the instance's region): uniform bucket-level access, `--public-access-prevention` enforced, a 1-day object-deletion lifecycle rule as a backstop, IAM granting the Cloud SQL instance's own service account (`p892123729036-te534y@gcp-sa-cloud-sql.iam.gserviceaccount.com`) `roles/storage.objectAdmin`. **One real mistake made and corrected in the same session**: asked to remove the bucket's legacy `projectViewer`/`projectEditor` convenience bindings, also removed `projectOwner`'s `legacyBucketOwner` binding by misreading the instruction — this immediately locked the authenticated identity (a project Owner) out of `getIamPolicy`/`setIamPolicy` on that specific bucket, since GCS's `projectOwner:<project>` binding is the actual mechanism granting bucket-level IAM access to project Owners, not something IAM-Owner-at-the-project-level provides for free. Recovered by deleting and recreating the bucket from scratch (bucket deletion is a project-level permission, unaffected by the broken bucket-level policy) — this time removing only the two bindings actually asked for. Final IAM, printed and confirmed: `projectOwner` (legacy bucket + object owner) and the Cloud SQL service account (`objectAdmin`) only — no `projectEditor`, no `projectViewer`, no `allUsers`.

`refresh-test-db.mjs`: same `_test`/no-collision guard as Part B, re-checked independently since this script runs standalone. Exports `axisandbloom` to a timestamped object in the new bucket, recreates `axisandbloom_test` via `gcloud sql databases delete`+`create` (not a manual `DROP SCHEMA public CASCADE`/`CREATE SCHEMA public` over the proxy — tried that first and hit "permission denied for schema public" on import, since a schema recreated by the app's own DB role is owned by that role and Cloud SQL's import process can't touch a `public` schema it doesn't own; recreating the database through the same `gcloud` path Part A used sidesteps the ownership mismatch entirely), imports, then deletes the dump object immediately — confirmed via `gcloud storage ls` that the bucket is empty after every successful run. A second Windows-only bug caught and fixed in the same pass: `spawnSync('gcloud', ...)` without `shell: true` fails silently on Windows since `gcloud` is a `.cmd` wrapper — direct `bash` invocation of the same command worked fine, isolating the cause immediately.

**Refresh run** (real, end to end): BEFORE — `coffees`/`coffee_slot_assignment`/`user_profile` all "n/a (table does not exist yet)" (empty test DB). AFTER — `coffees: 32`, `coffee_slot_assignment: 2`, `user_profile: 145`. Bucket confirmed empty afterward.

**Part D**: `src/test/guard.test.ts` added — asserts `DATABASE_URL` ends in `_test` and that `runCatalogIntegrityChecks().allPass` is true after `globalSetup`'s two applications; run in isolation with `--reporter=verbose` to confirm both assertions pass explicitly by name. Full suite against the freshly-cloned test database: **42 test files (40 passed, 2 failed), 340 tests (328 passed, 12 failed)** — the 12 failures are exactly the pre-existing `quizScoring` baseline (6 unique × 2 for the src/dist duplication); zero tests that passed against prod failed against the copy.

**The three `npm test` Definition-of-Done outcomes, all confirmed directly**: `TEST_DATABASE_URL` unset → dies before any test loads, exact message. Set to the prod URL → dies the same way (via the `_test`-suffix guard). Set to `…/axisandbloom_test` → suite runs, `globalSetup` prints `schema.sql applied twice with no error (re-application is a no-op)` and `catalog integrity: allPass`.

**Docs**: `backend/README.md` (new) — "Running tests" section: proxy, `TEST_DATABASE_URL`, `npm run test:db:refresh`, `npm test`; names `axis-bloom-db-transfers` explicitly and says plainly it holds user-data dumps only transiently and must never be made public or reused. `.env.example` gained `TEST_DATABASE_URL`. `package.json` gained `test:db:refresh`.
...
### 187. Archetype rename: Balanced & Sweet → Balanced (2026-09-19)

**Context**: `backend/src/features/archetype_rename_balanced/CLAUDE_CODE_PROMPT_ARCHETYPE_RENAME_BALANCED.md` (Rev 2). **Display-name-only change**: the archetype the site, quiz, emails, Liam and admin called "Balanced & Sweet" is now "Balanced" wherever a person reads it. Unchanged: UUID, enum code `balanced_sweet`, URL `/match/balanced-sweet`, Mailchimp slug `balanced`, colour `#d1ac11`, description, scoring, neighbours, image filenames.

**Database (one deploy, boot-safe)**: `schema.sql` renames the row and sweeps `newsletter_subscriber.archetype` (`'Balanced & Sweet'`/`'Balanced and Sweet'` → `'Balanced'`) immediately BEFORE the archetype seed INSERT — with `code NOT NULL`, a `WHERE NOT EXISTS` miss on the old name would insert a code-less row and fail 23502 at boot (same shape as the Fruity precedent). The seed row, every `WHERE name = …` lookup, every quiz answer/score VALUES row (incl. the self-healing v7 scoring sync that would otherwise strip the Balanced scores) and the archetype-stats view's JOIN CASE (`WHEN 'balanced_sweet' THEN 'Balanced'` — a join condition; left alone it would silently drop Balanced) now say `Balanced`; the one-time `code` backfill keeps both name arms. `db/seeds/archetype_vectors.sql` and `scoring_v1.sql` updated. Standalone record (no separate prod step): `backend/src/db/migrations/archetype_rename_balanced_2026-09-19.sql`. Cache: `getArchetypes()` is a 60 s per-process cache, but the rename runs at boot before any request, so there is no stale window.

**Code**: `catalogReads.archetypeCode()` gains a code-valued legacy map (`balanced & sweet`, `balanced and sweet`, `fruity & complex`) — the only place a retired name is understood — and a new `archetypeUuid()`. The two `SELECT id FROM coffee_archetype WHERE name = $1` lookups (`routes/quiz.ts` `POST /score`, `services/quizSession.ts` `saveQuizSession`) moved onto it, so the quiz no longer keys on the display name and an old-name `newsletter_subscriber` row resolves to the same `archetypeId` as `Balanced`. `quizScoring.findWinner`'s tie fallback returns `'Balanced'`; Liam's system prompt (`claude.ts`) and `quizCompleteEmail.ts` say `Balanced`; `lint-catalog.mjs` rule 4 `LABELS` now lists `'Balanced'` (no new allow-list entries, legacy string not re-added). New tests: `archetypeCode`/`archetypeUuid` legacy-name cases in `catalogReads.test.ts`; `quizScoring.test.ts` updated; `test-mailchimp-tags.mjs` keeps a legacy-name-still-slugs-to-`balanced` assertion.

**Frontend (non-admin, hardcoded by design)**: `archetypeConstants.ts`, `AxisMap`, `TheAxis`, `TheAxisV1`, `Home`, `About`, `HowItWorks`, `Shop`, `FlavorQuiz` (both legacy keys kept, `'Balanced'` added), the static `/match/balanced-sweet` page (path unchanged) and the reveal-panel mockup. Admin components needed nothing (`useArchetypes()`). Image imports/filenames (`Shop.tsx` L9, `WEBCUTBalanced&Sweet*.png`) stay.

**Docs/marketing sources**: this file, `WHAT_WE_BUILT_DB.md`, `SOMMELIER_BUILT.md`, `CAMILAS_UPDATES.md`, `launch/40_email-marketing/**`, `misc/marketing/**`, the quiz scoring CSV/audit SQL. Historical briefs, closing reports, `db/migrations/*`, `db/seeds/_retired/**`, `launch/_archive/**` untouched.

**Verified**: `npm run build` (backend + frontend) clean, `lint:catalog` exit 0. Two local boots against a prod-clone DB (`axisandbloom_test`, never prod) that started with the old row: boot 1 kept 6 archetype rows, same `id`/`code`, renamed to `Balanced`, 33 subscriber rows migrated, 65 score rows intact, no `[catalog-integrity]` warnings; boot 2 changed nothing. Live quiz run lands on `Balanced` with the original UUID; tie fallback and legacy-name `saveQuizSession` resolve to the same UUID; one live Liam exchange says "Balanced".
...
### 190. Hoboken Crawl quiz-result vs email audit (2026-09-24, read-only)

**Context**: `backend/src/features/quiz_email_audit/CLAUDE_CODE_PROMPT_QUIZ_EMAIL_AUDIT.md`. Another session reported that some crawl participants who scored Chocolate & Nutty were emailed "Balanced." Read-only audit (SELECT-only against prod, GET-only against Mailchimp/Resend) against the crawl window (2026-09-20 13:00Z–2026-09-22 00:00Z, 125 `quiz_funnel_event` rows, cross-checked against Dana's `studio_results_20260924_1500.csv` export — exact match).

**Findings**: 36 sessions had `email_submitted`. All 36 Resend sends carried the correct on-screen archetype (claim-timing evidence: max 83ms between the correct subscribe call and the `transactional_email_log` claim, well before any drifted call could race it) — the "wrong email" report was never a Resend/email bug. 7 of 36 `newsletter_subscriber` rows / Mailchimp tags were overwritten *after* the correct email went out, by a `post_quiz` subscribe call carrying no `quiz_complete`/`quiz_final` behind it — root-caused to `FlavorQuiz.tsx`'s recognized-guest resync effect (~L855) firing on page reload/retake with the frontend's default `archetypeKey = 'balanced'`, before any new score existed. Confirmed directly: all 12 distinct session keys behind the 7 overwriting calls have **zero** `quiz_funnel_event` rows at all. 5 sessions moved Chocolate & Nutty → Earthy via the branch question — working as designed, unrelated to the drift. Mailchimp: `GET /3.0/automations` → `total_items: 0` (no automations exist at all, not merely paused).

**Could not be determined**: Resend's raw HTML per address (real `RESEND_API_KEY` lives only in GCP Secret Manager; pulling it was blocked by this session's own credential-materialization guard, not worked around per instruction) — addresses listed for a manual dashboard check, though the claim-timing evidence above makes this close to certain already.

**Output**: `backend/scripts/audit-quiz-emails.mjs` (read-only helper, committed — script only, never its output) → `backend/tmp/quiz_email_audit_2026-09-20.csv` (36 rows, gitignored — contains emails). Diagnosis and repro independently confirms `backend/src/features/quiz_interpretation_v2/PARKED_CLAUDE_CODE_PROMPT_SUBSCRIBER_RESYNC_FIX.md`'s own findings. Fixed in #191, below.

### 191. Quiz resync fix — server-verified archetype, no placeholder default, append-only api_event (2026-09-25)

**Context**: `backend/src/features/quiz_email_audit/CLAUDE_CODE_PROMPT_QUIZ_RESYNC_FIX.md`. All five parts (A/B/D/E, then C) executed and applied across two passes on 2026-09-25 — Parts A/B/D/E deployed first; Part C's `--apply`, the scheduler deletion, and the older-mismatch investigation followed once Dana reviewed the dry-run and gave the go.

**Part A** (`FlavorQuiz.tsx`): `archetypeKey` is now `ArchetypeKey | null`, initialized `null` (no more `'balanced'` placeholder) — every one of its ~20 call sites (results-screen derivation, both resync effects, wallpaper preload, gate handlers, share slug, `nightScanSrc`, six JSX `archetype.x` dereferences) audited and made null-safe or asserted non-null where already provably scored-only. New `scoredThisSession` boolean gates both resync effects (guest + signed-in), reset on retake, true only after a real score/branch/tie resolution. The two silent `?? 'balanced'`/`?? archetypeKey` fallbacks (`handleNext`, `handleBranchContinue`, and a third in `handleBranchAnswerSelect`'s auto-advance timeout not named in the brief but same shape) now report via `reportError` and show `scoreError` instead of guessing. New `quiz_final` funnel event (post-branch, on-screen archetype) logged from all four "final result" paths (no-branch, branch-continue, branch-auto-advance, tie-resolution's "see my primary result") — `quiz_complete` unchanged (still the pre-branch score). Gate-submit double-fire fixed: `handleGateSuccess` marks `recognizedGuestSyncedRef` synced before the resync effect re-runs from the `postQuizEmail` state change it just caused.

**Part B** (`newsletter.ts`/`mailchimp.ts`/`resendEmail.ts`): `handleSubscribe` now verifies a `post_quiz` archetype against `quiz_funnel_event` (`quiz_complete` or `quiz_final`, slug-compared) before writing it, tagging Mailchimp, or sending the email — a rejection drops archetype/experimental/confidence to `null` (COALESCE keeps the existing value) and logs one `console.warn` with the email hashed; the subscribe itself still returns `200`. `sendResendEmail` now returns `{ok, id}` (previously discarded Resend's message id); `transactional_email_log` gained `archetype`/`resend_message_id` columns, set on every successful claim. Mailchimp's `archetype:*` tag family is now replace-not-add (`computeTagUpdates()`, pure/exported/unit-tested) — a resync inactivates every other `archetype:*` tag in the same call that activates the new one, fixing the "two tags active at once" bug the audit's live Mailchimp reads directly confirmed on all 6 drifted crawl subscribers.

**Part D**: `GET /api/cron/purge-api-events` retired outright — route, batch/retention constants, and doc paragraph all removed (not lengthened); `api_event_log/README.md` (new) carries the storage watch-number (1,837 rows / 1,264 kB as of 2026-09-25). The Cloud Scheduler job itself (`purge-api-events`) was deleted after Dana's go (`gcloud scheduler jobs delete purge-api-events`), confirmed via `gcloud scheduler jobs list` — only `purge-stale-anonymous-guests` remains. New `backend/scripts/lint-retention.mjs` (`npm run lint:retention`, wired into `deploy.yml` next to `lint:catalog`, vitest-wrapped) fails the build on any `DELETE`/`TRUNCATE` of `api_event`/`quiz_funnel_event`/`quiz_session`/`newsletter_subscriber` outside `*.test.ts` (deliberate deviation from the brief's literal "outside src/test/" — a test's own cleanup legitimately deletes its seeded rows, same exclusion shape `lint-catalog.mjs` already uses). `purgeStaleAnonymousGuests()` now splits by candidate kind: a never-quizzed 7-day-stale guest is still fully deleted (Auth + Firestore + Postgres); a quiz-taken 90-day-stale guest loses only its Firebase Auth identity — `user_profile`/`quiz_session`/`quiz_funnel_event`/`newsletter_subscriber` rows all survive. Return shape is now `{checked, purgedEmpty, purgedIdentityOnly, skipped}`.

**Part E**: 3 new checks added to the existing `services/quizIntegrity.ts` (ids 9–11, reusing its existing boot hook and `GET /api/admin/quiz/integrity` — no new files needed, corrected from the brief's assumption that this service didn't exist yet): quiz-complete emails since deploy have archetype+resend id slug-matching the subscriber; no subscriber (with a linked `quiz_session`) disagrees with their latest scored archetype; no `post_quiz` subscribe in the last 24h referenced a session key with no matching `quiz_complete`/`quiz_final` (the resync-bug signature itself, now server-rejected by Part B1 — this check confirms the rejection is holding). Backend tests: `routes/newsletter.test.ts` (a–d: reject-no-funnel, accept-matching, reject-mismatch, accept-branched), `features/marketing/mailchimp.test.ts` (pure `computeTagUpdates` unit tests, e), `services/staleGuestCleanup.test.ts` (Part D2's split, real DB + mocked Firebase Admin) — all pass against `axisandbloom_test`, alongside the full existing suite (366/378 passing; the 12 pre-existing `quizScoring.test.ts` failures are untouched by this work, confirmed via `git status` before this brief started). **Frontend Vitest/RTL tests (the brief's two component-mount tests) were not written** — this repo has no frontend test infra at all yet (no vitest/@testing-library in `frontend/package.json`, zero existing `.test.tsx` files), and building it from scratch for `FlavorQuiz.tsx` (a ~2,000-line component with routing/auth/cart context) is a separate undertaking flagged rather than rushed; Part B1's server-side verification is an independent safety net for the exact failure mode either way.

**Part C** (`backend/scripts/correct-subscriber-archetypes.ts`) — dry-run first, then **applied**. Mismatch set within the primary window was exactly the 6 named in the brief (thyago.teixeira10, suzie.houska, maryanndetrizio, hannahnyrie, katelyn.chen, superwawa729). Of the 3 older (pre-window) mismatches, each was individually investigated per Dana's request — subscriber archetype, latest `quiz_session` archetype/date, and whether the `api_event` subscribe call carrying "Balanced" referenced a session key with zero `quiz_complete`/`quiz_final` rows behind it: **chloerosem@gmail.com** and **marchon.bea@gmail.com** cleanly matched the signature (zero funnel rows for that session key) and were added to the correction; **danabar.mail+dana-try-3@gmail.com** did not (one of its two Balanced-carrying sessions actually *has* a `quiz_complete` row, just for a different archetype — genuinely more ambiguous) and, confirmed directly by Dana, is her own manual testing — left untouched. 8 rows corrected in total. Backup written first as required: `newsletter_subscriber_backup_20260925` (Postgres table, 8 rows) + `backend/tmp/subscriber_backup_20260925.json` (gitignored, includes each row's Mailchimp tags at backup time). All 8 archetypes corrected to their real latest `quiz_session` value; campaign backfilled for katelyn.chen/superwawa729/rayraycola6 (`hoboken-crawl-2026`). Mailchimp after-state confirmed live: every one of the 8 now carries exactly one `archetype:*` tag — `archetype:balanced` is gone from all of them, replaced by the correct one (`chocolate` ×4, `earthy` ×1, `fruity` ×2). `services/quizIntegrity.ts` check #10's own query re-run directly afterward: mismatch count dropped from 9 → 1, and the 1 remaining is exactly `dana-try-3` — the deliberately excluded test account, expected and correct.

**Deploy**: commit `1252c73` pushed, GitHub Actions run `36084615733` succeeded (Lint catalog ✓, Lint retention ✓ in CI), `/health` 200 post-deploy. Boot log confirmed the quiz integrity summary ran and correctly surfaced check #10's pre-correction 9 mismatches, warn-not-fail, in Cloud Run logs. **Smoke test, real browser against live prod** (`danabar.mail+quizresyncsmoke2026@gmail.com`): took the quiz anonymously (first cleared a stale signed-in Firebase session this browser profile had), scored Chocolate & Nutty, submitted — subscriber row and email log both correct (real Resend message id confirmed, proving the prod `RESEND_API_KEY` is live and Part B2 captures it). Reload and Retake, the exact sequence that used to fire a second buggy subscribe call, both fired **zero** additional subscribe calls — confirmed via `api_event` (still exactly 1 total) and Mailchimp (exactly one `archetype:*` tag, no stray `archetype:balanced`). Test rows cleaned up: `newsletter_subscriber`, `transactional_email_log`, `quiz_session`, `user_profile`, and the Mailchimp test member all removed/archived, confirmed zero remaining.
...
**Verification**: backend `tsc --noEmit` clean; frontend `tsc` clean for `FlavorQuiz.tsx` (other pre-existing frontend type errors elsewhere untouched). No SQL and no `INSERT`/`UPDATE` in the diff; the only DB access is the two `SELECT`s that moved into `scoreAnswerIds()`. Committed and pushed as `8edcac0`.
...
### 193. Quiz interpretation v2.1 · brief 2 of 3 — `quiz_session_interpretation` (SCD Type 2): table, live path, backfill, read paths (2026-09-25)

**Context**: `backend/src/features/quiz_interpretation_v2/CLAUDE_CODE_PROMPT_2_INTERPRETATION_SCD2_TABLE.md`; storage decision in `QUIZ_INTERPRETATION_V2_DECISIONS.md` ("Storage: SCD Type 2"). `quiz_session` stays the immutable fact; interpretations are a Type 2 dimension over it: one row per session **per interpretation version**, exactly one flagged `is_current` (= "produced by the latest deployed ruleset"), old rows kept forever. **The only table this brief writes is the new `quiz_session_interpretation`.** Nothing was updated, deleted or altered in `quiz_session`, `newsletter_subscriber` or any Firestore document. The build, the tests and the dev readings below were done on the dev database `axisandbloom_test` only (through the existing Auth Proxy on :5433, at Dana's explicit exception; the proxy was never started/stopped/reconfigured, the `axisandbloom` database was not connected to during the dev phase, `test:db:refresh` was not run). **Prod:** deployed as `80a9d7a` (schema.sql applied the DDL on boot); the prod backfill and a live-path proof were then run — see "Prod run" below.

**DDL as applied** (`schema.sql`, new section at the end between `-- >>> / -- <<< quiz_session_interpretation` markers, idempotent, applied twice to `axisandbloom_test` with no error): `quiz_session_interpretation(id UUID PK, quiz_session_id UUID NOT NULL REFERENCES quiz_session(id) — no ON DELETE CASCADE, interpretation_version TEXT NOT NULL, secondary_archetype, secondary_path, recommendation_mode NOT NULL, food_signal_alignment NOT NULL, pair_confidence, explore_archetype, explore_reason, primary_margin SMALLINT, is_current BOOLEAN NOT NULL DEFAULT false, valid_from TIMESTAMPTZ NOT NULL, valid_to TIMESTAMPTZ, computed_by TEXT NOT NULL CHECK IN ('scored','seed','backfill'), computed_at DEFAULT now(), UNIQUE (quiz_session_id, interpretation_version))`, plus `UNIQUE INDEX quiz_session_interpretation_current ON (quiz_session_id) WHERE is_current` and `idx_qsi_version`. 69 tables. Checked: the only Postgres `DELETE` that can reach `quiz_session` (`staleGuestCleanup.ts`, cascading from `user_profile`) only targets guests with **no** quiz session, so the no-cascade FK cannot block it.

**Live path** (`POST /api/quiz/results`): after `saveQuizSession` (unmodified), `recordScoredInterpretation()` recomputes server-side (`scoreAnswerIds()` → `interpret()`; the client's interpretation fields are ignored for the table; archetype and `branchedFrom` are canonicalised to `coffee_archetype.name` so a stale bundle sending a retired name still lands on the server's ruleset) and inserts one `is_current` `scored` `v2.1` row via the new sibling `saveQuizInterpretation(client, sessionId, interp, computedBy, opts)` in `services/quizSession.ts`. No `answerIds` (very old bundles) or nothing scoreable → no row, a `console.warn`, and the read path falls back. `context_data` keeps the as-scored snapshot exactly as before **plus** the six new client fields (`?? null`); the Firestore mirror is unchanged.

**Seed + backfill** (`backend/scripts/backfillQuizInterpretation.ts` is the CLI; the logic is `src/services/quizInterpretationBackfill.ts` so the deploy build's `tsc` covers it and tests can import it). **`--dry-run` is the default**; `--apply` writes; `--limit N`; **`--expect-db <name>` (default `axisandbloom_test`)**: the database name is checked from `DATABASE_URL` *before connecting* and from `current_database()` after, and it refuses on any mismatch (Dana runs it on prod with `--expect-db axisandbloom`). Insert-only, `ON CONFLICT (quiz_session_id, interpretation_version) DO NOTHING`, batches of 200, one transaction per batch; sessions that already have any interpretation row are skipped (post-deploy sessions get no v1 seed and no second v2.1). Per session: `v1` seeded verbatim from `context_data` (`is_current=false`, `valid_from=completed_at`, `valid_to=run timestamp`, `computed_by='seed'`), `v2.1` computed for every session with `answerIds` (`is_current=true`, `valid_from=run timestamp`, `computed_by='backfill'`); sessions with no `answerIds` (cross-device match claims, pre-August rows) get `v1` only, current, `valid_to` NULL. The dry-run prints counts per version and each of the 37 crawl cases' v2.1 output against the fixture.

**Read paths now joining the current row** (`LEFT JOIN quiz_session_interpretation i ON i.quiz_session_id = qs.id AND i.is_current`, `context_data` as fallback; helpers `CURRENT_INTERPRETATION_COLUMNS/JOIN` + `resolveInterpretation()` in `quizSession.ts`): `GET /api/quiz/results/latest` (`getLatestQuizResult()`; top-level `secondaryArchetype`, `recommendationMode`, `foodSignalAlignment`, `secondaryPath`, `pairConfidence`, `exploreArchetype`, `exploreReason`, `primaryMargin`, `interpretationVersion`, `source`; `context_data` still raw); `services/userSignals.ts` (new `interpretationVersion`, `pairConfidence`, `exploreArchetype`, `exploreReason` on `UserSignals`); `routes/sommelier.ts` `/start` (Liam's opening context gains `Quiz pair confidence: …` and, when present, `Loose thread to explore (not yet asked): <archetype> (<reason>)` — only for a session with a table row); `services/sommelierEvaluator.ts` (new exported `isProfileAmbiguous()`: **on a v2.1+ row `PROFILE_AMBIGUOUS` fires on `pairConfidence === 'low' || exploreArchetype !== null`; otherwise the old `ai_agent`/`low` rule; a quiz tie fires it on any version**; the 13-dim feature vector is unchanged). `routes/users.ts` and `behavioralConfidence.ts` read only the primary/dates: unchanged.

**Readings on `axisandbloom_test`** (`SELECT COUNT(*), md5(string_agg(id::text || context_data::text, ',' ORDER BY id)) FROM quiz_session`; for `newsletter_subscriber` the table has no `id` column (PK is `email`), so the statement used is `md5(string_agg(email || COALESCE(archetype,'') || COALESCE(confidence,''), ',' ORDER BY email))`):

| # | reading point | `quiz_session` count · md5 | `newsletter_subscriber` count · md5 |
|---|---|---|---|
| 0 | baseline, before anything | 78 · `febdee5a8a58d45b73f9b92fdc7a0aa3` | 49 · `1ddd8359418a9872674a8550635be9c7` |
| 1 | immediately before the DDL | identical | identical |
| 2 | after the DDL | identical | identical |
| 3 | after script `--dry-run` | identical | identical |
| 4 | after script `--apply` (and a second, idempotent `--apply`) | identical | identical |
| 5 | after the test run (first run, 4 of my tests timed out at 5 s) | identical | identical |
| 6 | after the test rerun (21/21) | identical | identical |
| 7 | after `sommelier.test.ts` | identical | identical |

**Script on dev**: `--dry-run`: 78 sessions (27 with `answerIds`) → would insert 78 `v1` + 27 `v2.1`, 51 sessions keep `v1` current (all "no answerIds"), nothing written. `--apply`: inserted 78 `v1` + 27 `v2.1`; afterwards 78 current rows for 78 sessions, **exactly one current row per session** (0 sessions with ≠ 1), rows by version/`computed_by`/current = `v1 seed false` 27, `v1 seed true` 51, `v2.1 backfill true` 27. Second `--apply`: 0 sessions to process, 0 inserted. This test database predates the crawl (only 2 of the 37 fixture cases exist in it; both match, 0 differ), so the 37-case check ran in the seeded tests below. **Refusals verified** without connecting: `DATABASE_URL` naming `axisandbloom` or any other database exits 3 before any connection; `--apply --dry-run` is rejected.

**Tests** (`src/services/quizInterpretation.test.ts`, 21, all pass): `saveQuizInterpretation` inserts and is idempotent per (session, version); a second current row for one session fails on the partial unique index (`23505`); unknown `computed_by` fails the CHECK; `recordScoredInterpretation` on a branched crawl case writes exactly one current `scored` `v2.1` row equal to the fixture while ignoring wrong client-sent fields; no `answerIds` → no row + warning; `getLatestQuizResult` returns the current row as top-level keys with `context_data` raw, and falls back to `context_data` for a session with no rows; a current row with a NULL secondary does not resurrect the stale v1 secondary; the backfill against the 37 fixture sessions + a no-`answerIds` claim + a post-deploy `scored` session — dry-run writes nothing, `--apply` gives one current row per session (count of current = count of sessions), **v2.1 = fixture 37/37**, `v1` rows = what `context_data` held, `v1.valid_to = v2.1.valid_from = run timestamp`, the claim keeps `v1` current, the scored session is left alone, a second apply inserts nothing, `--limit` honoured; `isProfileAmbiguous` on v2.1 / v1 / fallback / quiz tie; the script's arg parser and DB-name helper. **Every test's writes live in a transaction that is rolled back**, so the database is left as found (readings 5–7). The 6 pre-existing `quizScoring.test.ts` failures (`findWinner` cascade, `isSecondaryClose`) are unchanged. `tsc` clean.

**Deviations from the brief (flagged, all easy to reverse)**:
1. **Not the same transaction as the `quiz_session` insert.** `saveQuizSession` commits through the shared pool and the brief says not to modify it, so "same transaction" is unreachable as written. The interpretation insert is a second, immediately-following, best-effort transaction: a failure is logged (`[quiz/interpretation]`) and never fails the save; that session stays on the `context_data` fallback until the backfill covers it (the script picks up any session without rows). Making it atomic means giving `saveQuizSession` an optional client parameter: a Dana decision.
2. **Whole-row precedence, not per-column `COALESCE`, on the read side.** The brief says `COALESCE(i.<col>, ctx->>…)`; for nullable columns (`secondary_archetype`, `explore_*`) that would resurrect a v1 value when a `v2.1` row is legitimately NULL. `resolveInterpretation()` takes the current row whole when there is one, else `context_data`.
3. **No test goes through the real HTTP route.** `POST /api/quiz/results` also writes Firestore, which the test database does not isolate. The route's steps (session insert → `recordScoredInterpretation` → `getLatestQuizResult`) are exercised directly, inside a rolled-back transaction; the live route itself is unexercised until deploy.
4. **`newsletter_subscriber` md5 statement** uses `email` (no `id` column), as above.
5. **Vitest's `globalSetup` applies the whole `schema.sql` twice to `axisandbloom_test` on every run** (existing repo behaviour, idempotent). The readings show no data change.

**Prod run (2026-09-25, on Dana's explicit go at each step, through the existing Auth Proxy: an exception for that session only).** The only table written was `quiz_session_interpretation`, plus the one live-path save below; no `UPDATE`/`DELETE`/`ALTER` anywhere. Deployed as `80a9d7a` (GitHub Actions run `36184900869`, green): the DDL applied on boot, the table existed and was empty, and all 121 sessions were on the `context_data` fallback.

| # | prod reading point | `quiz_session` count · md5 | `newsletter_subscriber` count · md5 |
|---|---|---|---|
| P1 | baseline (read-only) | 121 · `bfa398d4a4b07701ca6f1524c18dd330` | 88 · `fd35ea4b6665bab9ec79f91c6f20cddf` |
| P2 | after `--dry-run --expect-db axisandbloom` | identical | identical |
| P3 | after `--apply --expect-db axisandbloom` | identical | identical |
| P4 | after the live-path proof (one new session) | 122 · `fecd67eba56ef4b04c33f7bcabf9987d` | identical |

**`--dry-run`** (wrote nothing): 121 sessions, none interpreted; would insert 121 `v1` + 70 `v2.1`; 70 with `answerIds`, **51 without** (they keep `v1` as the current row); crawl fixture (37 cases): **37 match `expected_v2_1`, 0 differ, 0 missing**. **`--apply`**: inserted **121 `v1` + 70 `v2.1`**. After it, `SELECT interpretation_version, is_current, COUNT(*) … GROUP BY 1,2` = `v1`/false 70, `v1`/true 51, `v2.1`/true 70; sessions without exactly one current row: **0** of 121.

**Live-path proof (one write).** As the dedicated test user `quiz-scenario-test@axisandbloom.test` (created in Firebase Auth for this; never a real customer), `POST /api/quiz/score` then `POST /api/quiz/results` with the `answerIds` of fixture case `hoboken-crawl-2026-01` (payload built as `FlavorQuiz.buildQuizResultPayload` builds it, via the Cloud Run backend URL because Cloudflare challenges scripted requests to the site; App Check is in monitoring mode). `/score` returned `Balanced`, secondary `Chocolate & Nutty`, path `food-led`, mode `primary_plus_introduce_secondary`, pair confidence `high`, no explore hint, margin 9, `interpretationVersion` `v2.1` (= the fixture's `expected_v2_1`; `foodSignalAlignment` `medium`). `/results` returned `{"id":"87cb4ebd-ad22-4a27-9415-1d766b038a04"}`. That session's `quiz_session_interpretation` row: `v2.1`, `computed_by = 'scored'`, `is_current = true`, `valid_to` NULL, same values as above. `GET /api/quiz/results/latest` for that user returned `"source":"table"` with the current row as top-level keys (`secondaryArchetype`, `recommendationMode`, `foodSignalAlignment`, `secondaryPath`, `pairConfidence`, `exploreArchetype`, `exploreReason`, `primaryMargin`, `interpretationVersion`) and `context_data` still raw, carrying the six client fields. Side effects of that one POST, all for the test uid: a `user_profile` upsert, the Firestore `users/{uid}` docs, and the endpoint's background behavioral-confidence / taste-journey writes. Nothing was deleted.

**Test identity — exclude from any analysis of real customers:** Firebase uid `0BeUD1zIdCTyO4v78kofJVkFtfo1`; email `quiz-scenario-test@axisandbloom.test`; `user_profile.id` `103cdf5f-c7f6-4634-a02e-82fba7a6b60b`; `quiz_session.id` `87cb4ebd-ad22-4a27-9415-1d766b038a04` (its interpretation row is the only `computed_by = 'scored'` row on prod so far). Prod is at 122 sessions, and every session has exactly one current interpretation row. Rollback, if ever wanted: revert the code; the table is then ignored and nothing on the fact needs undoing.

**Next**: brief 3 (`v_subscriber_quiz_results` reads the table).

**Files**: `backend/src/db/schema.sql`, `services/quizSession.ts`, `services/quizInterpretationBackfill.ts` (new), `services/userSignals.ts`, `services/sommelierEvaluator.ts`, `routes/quiz.ts`, `routes/sommelier.ts`, `services/quizInterpretation.test.ts` (new), `backend/scripts/backfillQuizInterpretation.ts` (new).

### 194. Quiz interpretation v2.1 · brief 3 of 3 — `v_subscriber_quiz_results` reads the interpretation table; `v_quiz_session_interpretation_history` (2026-09-25)

**Context**: `backend/src/features/quiz_interpretation_v2/CLAUDE_CODE_PROMPT_3_RESULTS_VIEW.md`. Read-only SQL in `schema.sql`: **one `IMMUTABLE` function and two views; no `INSERT`, `UPDATE`, `DELETE` or `ALTER TABLE` anywhere.** The export Dana already uses now shows the *current* interpretation next to what the customer was told on the day, and there is a per-session, per-version history view as the calibration surface for the next event.

**`quiz_archetype_canonical(t TEXT) RETURNS TEXT` (`IMMUTABLE`)**: `Balanced & Sweet` / `Balanced and Sweet` → `Balanced`; `Fruity & Complex` → `Fruity`; `Spicy & Earthy` → `Earthy`; anything else (and NULL) unchanged. Used wherever a value was frozen as text before the September rename: `context_data` strings, the `v1` seed rows, `newsletter_subscriber.archetype`.

**`v_subscriber_quiz_results`** — 41 columns; the 28 existing ones keep their names and positions, with **one in-place rename** (`confidence_at_signup` → `food_signal_alignment_at_signup`), and 13 new columns appended after `user_id`. Driving structure unchanged (`newsletter_subscriber` → `latest_session`, `DISTINCT ON (user_id) … ORDER BY completed_at DESC`); `latest_session` now `LEFT JOIN quiz_session_interpretation i ON i.quiz_session_id = qs.id AND i.is_current`. Columns, in order: `email, first_name, subscribed_at, source, campaign, campaign_attributed_at, subscribed, archetype_at_signup` *(canonicalized)*, `food_signal_alignment_at_signup` *(renamed)*, `experimental_at_signup, primary_archetype, secondary_archetype` *(from `i`, canonicalized)*, `recommendation_mode` *(from `i`)*, `food_signal` *(canonicalized)*, `food_signal_alignment` *(from `i`)*, `experimental, decaf, score_chocolate_nutty, score_balanced, score_fruity, score_earthy, score_floral, score_experimental, scores_json, quiz_result_json, quiz_completed_at, quiz_session_id, user_id` — then the new ones: `secondary_path, pair_confidence, explore_archetype, explore_reason, primary_margin, interpretation_version, interpretation_computed_by, interpretation_valid_from`, the `_as_scored` trio frozen in `context_data` (`secondary_archetype_as_scored` *(canonicalized)*, `recommendation_mode_as_scored, food_signal_alignment_as_scored`), `branched_from` *(new pass-through, canonicalized)` and `food_signal_raw`. A session with no current row shows NULL interpretation columns (honest source, no silent fallback); every session on prod has one.

**`v_quiz_session_interpretation_history`** (new; one row per session per version): `quiz_session_id, user_id, completed_at, final_archetype` *(via `coffee_archetype`)*, `branched_from` *(canonicalized)*, `interpretation_version, is_current, valid_from, valid_to, computed_by, secondary_archetype` *(canonicalized)*, `secondary_path, recommendation_mode, food_signal_alignment, pair_confidence, explore_archetype, explore_reason, primary_margin`. Granted to `reporting_ro` beside the existing view (re-granted every boot, since `DROP VIEW` revokes). Nothing in the backend or frontend read `v_subscriber_quiz_results` (only `quizRecalibrate.ts` reads a CSV export of unchanged columns), so the rename breaks no code.

**Verification** (Part D). *Dev, `axisandbloom_test`*: `src/services/quizResultsView.test.ts` (8 tests, all pass; every seeded row is in a rolled-back transaction): the function's mappings and `IMMUTABLE`; the exact column list and order (28 existing in place, 13 appended, old name gone); the history view's columns; **37 seeded crawl subscribers → current-row columns = `expected_v2_1` 37/37 and `_as_scored` columns = `stored_v1` 37/37**; every subscriber with a linked session has an `interpretation_version` and no `v2.1` row is `ai_agent`; the pre-rename `archetype_at_signup` canonicalizes; one current row per session and one row per session per version in the history view. `schema.sql` still applies twice with no error. *Prod, read-only through the existing proxy (`READ ONLY` transaction, `axisandbloom` name-checked)*: **run before deploy**, so the views' exact `SELECT` text from `schema.sql` was executed as plain queries with the function inlined as its `CASE` (nothing created on prod): 55 subscribers have a linked session, **0** with a NULL `interpretation_version`, **0** `v2.1` rows with `ai_agent`; the 37 `hoboken-crawl-2026` rows (excluding `danabar.mail%`, in `completed_at` order): **current row = `expected_v2_1` 37/37 and `_as_scored` = `stored_v1` 37/37, no difference**; history `GROUP BY interpretation_version, is_current`: `v1`/false 70, `v1`/true 51, `v2.1`/true 71 (70 backfill + the one `scored` test-user session); 122 sessions in the history, **122 with exactly one current row**.

...
**Context**: `backend/src/features/quiz_email_audit/CLAUDE_CODE_PROMPT_QUIZ_RESYNC_FIX.md`, Parts A/B/D/E executed; **Part C (data correction) run as `--dry-run` only this pass — `--apply` waits for Dana's go**.
...
**Part C** (`backend/scripts/correct-subscriber-archetypes.ts`, `--dry-run` default): confirmed the mismatch set is exactly the 6 named in the brief (thyago.teixeira10, suzie.houska, maryanndetrizio, hannahnyrie, katelyn.chen, superwawa729), plus 3 older report-only mismatches outside the correction window (predate the crawl). Live Mailchimp reads on the 6 show both `archetype:balanced` and the real archetype tag simultaneously active, confirming the bug end to end. Campaign-backfill preview: katelyn.chen/superwawa729 have no campaign set yet (rayraycola6 not checked — not a mismatch row, campaign-only). **Not applied.** Full dry-run output in the closing report.

**Deploy**: commit/push/deploy status, boot-log integrity summary, and smoke-test evidence recorded in the closing report once run.

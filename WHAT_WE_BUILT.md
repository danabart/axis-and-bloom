...
**Part 9 (2026-07-13)** — Dana reported every coffee showing "Multi-Origin / Blend" regardless of actual origin. **Investigated thoroughly, could not reproduce**: queried the live `coffees`/`lookup_value` join directly (real, varied distribution — 13 Multi-Origin/Blend, 6 East Africa, 4 South America, 4 Central America, 3 Southeast Asia & Pacific, matching Part 1's original backfill almost exactly), hit `GET /api/coffees/:id/content` for several different coffees on both local dev and **live production** (`axis-bloom-backend-oiub7eumya-uc.a.run.app`) — all returned correct, varied `originRegion` values — and clicked through every visible card on the deployed frontend, confirming the displayed pills matched. Also checked `AdminCoffees.tsx`'s origin-region dropdown default (empty/unset, not a specific region — not the cause) and tested rapid card-switching for a fetch-race condition (none found). No code or data change made — nothing to fix. Likely explanation: `chocolate_nutty`'s two catalogue slots (Classic Chocolate/Noam Blend, Deep Cocoa/6-Bean Espresso Blend) are both genuinely Multi-Origin/Blend, and `chocolate_nutty` is the archetype guests land on by default — someone reloading the page repeatedly without clicking into other archetypes would see that same correct-but-coincidental tag every time.

...
**Context**: Dana's Profile screenshot on a wide monitor (~2600px) showed the primary block and the Worth Exploring block rendering at completely different sizes/aspect ratios, cards reduced to one-word-per-line slivers, and the content column sitting at ~400px instead of Part 22's ~760px. Spec: `frontend/src/features/reveal_panel/PROMPT_block_geometry_unification.md`. **Git state at the start of this pass**: `main` up to date with `origin/main` at `827b64b` (#162); by the time this pass finished, `origin/main` had advanced to `18aabc3` (#163, API Event Log — a different session, zero file overlap with this pass) and local `main` was still cleanly in sync with it, no rebase needed. Working tree had the same pre-existing unrelated `OPEN_TASKS.md` edit and untracked planning docs/feature folders from other concurrent sessions noted in prior entries — untouched throughout.

**All three prompt-identified root causes confirmed live (via `getBoundingClientRect`/`getComputedStyle` on a real signed-in Profile page, a throwaway Firebase test user + seeded `quiz_session` row, deleted after) before any code changed, per the prompt's own instruction:**

1. **Two unfold modes.** Profile's primary block passed `unfoldMode="breakout"`; its Worth Exploring block, and every other adjacent-archetype mount (Find My Flavor returning's adjacent, quiz results' primary *and* adjacent), didn't — they defaulted to `unfoldMode="inline"`. Measured live: an inline-unfolded adjacent section's reading column came out to 243.2px inside its own 760px section (the embedded stylesheet's 32% ratio applied uncontested) — the "squeezed portrait" symptom, in pixels.
2. **The `embedded` width caps.** Confirmed via CSS specificity arithmetic, not the existing code comment's claim (which was wrong): `.bd-section.bd-embedded .bd-stage.bd-folded.bd-dial-open .bd-reading{width:32%}` is 6 classes deep (0,6,0); the breakout-scoped `.bd-section.bd-embedded.bd-breakout.bd-dial-open .bd-reading{width:40%}` is only 5 (0,5,0) — the 32% rule always won, regardless of source order. Measured live on the PRIMARY block (which already had `unfoldMode="breakout"`): `readingWidthPct: "32.0"` instead of the intended 40, at every viewport width including the ≤1000px stacked fallback.
3. **Mixed reference frames in centering.** `margin-left:50%` (parent frame) + `transform:translateX(-50%)` (element's own post-layout frame) — algebraically these cancel out correctly *only if* the parent column is genuinely centered on the viewport. Live measurement at 2195px (this environment's screen-width ceiling — see Not done) initially read `sectionLeft: 560.29` vs a hand-computed `expectedCenterLeft: 567.5`, a ~7px gap that looked like it confirmed the bug — but that computation used `window.innerWidth` (2195, includes the OS scrollbar), not `document.documentElement.clientWidth` (2180, excludes it, the frame the layout itself actually resolves `%`/`vw` against). Recomputed against `clientWidth`, the true expected center is 560.0 — matching the measured 560.29 almost exactly. **The old two-frame formula was not actually miscentering anything in this environment**; the ~7px "bug" was a measurement artifact in the verification script, not the app. Consolidated to a single `calc()` anyway (see below) per the prompt's explicit ask ("compute the breakout centering in ONE reference frame... whatever is cleanest") — real hardening even without a reproduced defect, and it measures identically correct at 1280/1440/1920/2195px.

**The fix, one block one geometry, all four target surfaces (Profile primary+adjacent, Find My Flavor returning primary+adjacent, quiz results primary+adjacent):**

- **`unfoldMode="breakout"`** added to the four call sites that lacked it (`Profile.tsx` adjacent; `FlavorQuiz.tsx` returning-adjacent, results-primary, results-adjacent), plus `showBreakoutHeader={false}` on results-primary (it already has its own "Your match" header block above it, same reasoning as returning's existing primary mount).
- **`embedded`'s split.** `embedded` turned out to be gating two bundled concerns: (a) quiz/profile-vs-/bloom generic styling (padding tweaks, etc. — fine everywhere, untouched), and (b) compact quiz-era SIZE CAPS (300px dial wheel, 280px ruler, 32/68 unfold ratio, 92px bag) that made sense for the OLD inline-only folded surfaces but actively fought breakout's wide landscape layout once every surface got it. Split by scoping (b) two ways rather than bundling a third class into `embedded`: the conflicting 32/68 ratio rule gained `:not(.bd-breakout)` (so it simply doesn't match on breakout surfaces, instead of losing a specificity war it was accidentally winning); a new `@media (min-width:1001px){ .bd-section.bd-embedded.bd-breakout.bd-dial-open ... }` block restores /bloom's full-size dial wheel/ruler/chips/labels specifically once a block is both breakout *and* actually wide enough to show it landscape-style (below 1000px, breakout's own stacked fallback reuses the SAME compact mobile-safe caps embedded already had — deliberately not overridden, to avoid a hard-coded 400px dial overflowing a 375px phone).
- **Centering** consolidated from `margin-left:50%; transform:translateX(-50%)` to `margin-left:calc(50% - (min(1060px, calc(100vw - 110px))/2))` — width and centering now derive from the identical expression, one reference frame, no `transform`.
- **Accordion.** `DialArchetypeSection` gained `onOpenChange`/`forceFold` props (own fold state stays internal/uncontrolled — `forceFold` is a one-shot "someone else just opened, fold yourself" nudge, not a persistent lock). Each of the three pages got its own `openBlock` state (`'primary' | 'adjacent' | null`) wiring the two mounts together; quiz results uses an independent `resultsOpenBlock` from Find My Flavor returning's `matchedOpenBlock`, matching this file's existing per-screen state isolation. Verified live: opening Worth Exploring folds an already-open primary block, and vice versa.
- **Column width.** Confirmed ~760px on Profile and Find My Flavor returning without changes — both already had the Part 22 `max-w-[760px] mx-auto` wrapper. Quiz results did **not** — its `DialArchetypeSection` mounts rendered directly inside an unwrapped `<div>`, so the old inline unfold's fixed-480px-self-centered-in-flex card papered over it, but breakout's `width:100%` reading column stretched edge-to-edge across the full viewport once breakout landed there (caught live — a match card spanning ~2180px). Added the same `px-6 md:px-10 pb-20` / `max-w-[760px] mx-auto` two-level wrapper Find My Flavor returning already uses.

**Three more bugs found and fixed live during verification, not in the original prompt:** (1) the new breakout-instrument media-query block initially applied unconditionally (not scoped to `.bd-dial-open`), so `padding:40px 40px` on an otherwise `width:0` *closed* instrument box painted a visible ~80px sliver at the section's edge even while folded — every rule in that block now also requires `.bd-dial-open`. (2) The field bag's `right`/`bottom`/`width` jumped from compact to full-size the instant `.bd-dial-open` landed with no transition, while the section kept animating its own width for another 850ms — added a matching `.85s` transition. (3) Per Dana's live call, the bag doesn't read right at breakout's much wider field size regardless of position — removed for breakout specifically (`display:none`, scoped to `.bd-breakout.bd-dial-open`); /bloom's own bag is untouched.

**Verified live**, signed-in test user, real browser: closed/open width and ratio checked at 1280/1440/1920/2195px (iframe-embedded same-origin sub-viewports for the narrower widths, since `resize_window` doesn't shrink this environment's maximized OS window) — pixel-identical 760px closed / 1060px open / 40:60 ratio / centered (`sectionLeft` within 0.3px of computed center) / zero horizontal overflow (`scrollWidth === clientWidth`) at every one, on Profile, Find My Flavor returning, and quiz results. Accordion confirmed live on Profile. /bloom spot-checked visually and via DOM query — six `bd-section` elements, none carrying `bd-embedded`/`bd-breakout`, untouched. `npx vite build` clean (2148 modules, same pre-existing font/chunk-size warnings as prior entries). Cleanup: throwaway Firebase user + `quiz_session`/`user_profile` rows deleted, zero-rows re-query confirmed, scratch scripts removed.

**Not done / out of scope**: true 2600px window testing — this environment's physical screen is 2195px wide (`screen.width`), and `resize_window` doesn't grow a maximized window past it; relied on `min(1060px, 100vw-110px)` clamping to a fixed 1060px above ~1170px, mathematically identical behavior at 2195px and 2600px for every width/ratio/centering check, stated plainly rather than silently assumed. Mobile (375px, via same-origin iframe): the archetype block itself is overflow-free (`bd-dial-wrap`/`bd-rotor` correctly use the existing compact ≤940px caps, unchanged by this pass), but a **pre-existing, unrelated** ~7px horizontal overflow was found — Profile's own tab row (`Flavor Memory / Past Orders / Settings / Family`, `flex gap-8`) doesn't wrap at 375px and was already overflowing before this pass touched anything; left as-is, flagging for a separate fix. `/bloom`, reveal panel internals, doors, backend, and quiz logic untouched per the prompt's own scope.

**Files**: `frontend/src/app/components/Profile.tsx`, `frontend/src/app/components/FlavorQuiz.tsx`, `frontend/src/app/components/bloom/DialArchetypeSection.tsx`, `frontend/src/app/components/bloom/dial/BloomDial.tsx`.

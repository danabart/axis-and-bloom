...
**Manual prerequisite (not done here)**: Firebase Console → Authentication → Sign-in method → Anonymous → Enable. Nothing in this feature works until that's on — flagging back to Dana, per the spec's own instruction. Could not verify programmatically whether it's already enabled (Identity Platform admin API calls returned 403 with the available credentials; installing `gcloud alpha` components requires an interactive prompt this session couldn't complete).

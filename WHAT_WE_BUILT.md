...
### 86. The Bloom — two-row archetype layout redesign (2026-07-12)

**Context**: Dana's direct visual feedback (`Capture.JPG` screenshot) on the live Part 4 build — the three-column row (photos | dial | position card) left the card column much shorter than the photo/dial columns beside it, creating a large dead-space gap under the card and making it unclear that scrolling down revealed more (dimension bars, flavor wheel, Liam's intake, etc. were pushed far below the fold, hidden behind that gap).

**Fix**: split each archetype section into two rows in `BloomPage.tsx`'s `ArchetypeSection`. **Row 1** (identity/discovery only, unchanged assets): photo collage + `BloomDialWidget`/bag image + the archetype name heading, in a `flex flex-col md:flex-row`/`md:flex-row-reverse` row with `alignItems: 'center'` (was `flex-start`) so the shorter heading column sits vertically centered against the taller photo/dial columns instead of top-aligned. **Row 2** (details, full width): `PositionCard` (collapsed header + commerce row) and the new `RevealedPanel` now render as direct full-width siblings immediately below Row 1, rather than nested inside a column — so the card and its reveal panel always span the full section width and sit directly under the identity row with no gap, regardless of how tall Row 1 is.

**Bug hit + fixed**: making `PositionCard`/`RevealedPanel` siblings exposed a duplicate React key (`key={currentKey}` on both) — React logged "two children with the same key." Fixed by keying them `` `${currentKey}-card` `` / `` `${currentKey}-panel` ``.

**Verified in a real browser against production Cloud SQL**: both flip orientations (Chocolate & Nutty non-flip, Fruity flip) show a compact, height-matched Row 1 with the full-width Row 2 card directly beneath — no dead space, "Reveal the full profile ↓" immediately visible without scrolling past a gap. Revealed state confirmed expanding correctly full-width in place. Re-checked at a 390px phone viewport: Row 1 stacks (photo → sm1/sm2 pair → dial+tag → bag), Row 2 card and revealed panel (including the `RevealedPanel`'s internal `grid-cols-1 md:grid-cols-2`) collapse to one column cleanly, zero horizontal overflow (`document.body.scrollWidth === innerWidth`), zero console errors.

...
**Verified**: `tsc --noEmit` clean (backend), `vite build` clean (frontend, 2155 modules). **Not verified this pass — no live browser session**: the actual Bloom deep-link scroll/rotate, the save button's confirmation state, a real Liam conversation reaching a marker, and the chips rendering/navigating in `Sommelier.tsx`. All four were code-reviewed against the already-proven patterns they reuse (`handleHopClick`'s `rotateTo`, the existing dial-position PATCH, the chat message-list rendering), but Dana should click through them, especially the two live-Firestore-config items above before they mean anything in production.

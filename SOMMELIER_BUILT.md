...
**Decision**: `POST /api/quiz/score` now includes `tieDetected: boolean` and `tiedArchetypes: string[]`. A tie is detected when: (1) multiple archetypes share the highest raw score AND (2) none of the cascade questions (Q5→Q4→Q2→Q1) resolve it. The technical fallback winner is still `Balanced`. When the cascade does resolve a score tie, `tieDetected = false`. This field drives the `PROFILE_AMBIGUOUS` intent in the sommelier evaluator.
...
- **The verification asked for most: real before/after coffee-id diffs, through the actual `fetchSommelierCoffees()`, not a unit mock.**
  ...
  - DISCOVERY_SEEKER-shaped (`ragFocus: 'discovery', userArchetype: 'Balanced'`): AFTER `[32, 2, 5, 12, 1, 29, 6, 11]` vs. BEFORE `[32, 5, 1, 29, 6, 2]`. New bridge-hop-derived ids: **`[12, 11]`**.
  - Bonus, since fixing it was required to even reach this point: CONVERSION-shaped (`ragFocus: 'exact_match', userArchetype: 'Balanced'`): AFTER `[1, 3, 7, 14, 17]` vs. BEFORE `[]` (always empty, every call threw).
...
#### S93. Bloom Dial admin Map & Journey rebuild — confirmed no Liam impact
...
#### S95. Roastery soft-deactivation — Liam's RAG candidate pool excludes inactive coffees; `getAliases()` gains an owned-bag fallback

**Context**: `WHAT_WE_BUILT.md` #170 (and its CTO-review addendum). Unlike S93/S94, this one does touch `sommelierRag.ts` directly — recorded here in full, not as a confirmation note.

**RAG candidate pool.** `BASE_COFFEE_SQL` (the shared base every `archetype_range`/`alternatives`/`evolution_bridge`/`discovery`/`exact_match`/`curated_mix` focus builds from) and the handful of ad-hoc `FROM coffees c JOIN archetype_assignments aa` query shapes that don't route through it all gained `AND c.is_active = true`. Liam must never *recommend* a coffee whose roastery has been deactivated — the same "browse/recommend surfaces drop it" rule `WHAT_WE_BUILT.md` #170 applies everywhere else on the public site now also holds for every RAG focus type. Nothing about intent evaluation, scoring, or prompt assembly changed — this is purely which coffee rows ever reach `buildCatalogText()`.

**`getAliases()` — the owned-bag exception.** The deactivation cascade also flips `coffee_alias.is_active = false` for every alias belonging to a deactivated roastery's coffees, since `coffee_alias` is itself one of the three tables the cascade touches. Without a fix, that would have silently broken a real Sommelier decision this feature otherwise respects: a customer's own turns about a coffee they already have (`my_coffee`, origins/process questions, brew card, SMS feedback) must keep resolving a real house name, never fall back to leaking `coffees.name` or the roaster's name. `getAliases()` now tries the existing `coffee_alias`-based query first (unchanged), and for any coffee id it comes back empty for, falls back to a second query resolving the name straight off `dial_archetype_positions` → `dial_slot_alias` — untouched by the cascade (Decision 3 of #170: dial positions are never cleared, only filtered at read time) — independent of `coffee_alias.is_active` entirely. Verified with a disposable fixture (`services/sommelierRag.test.ts`): a coffee marked inactive with its alias row also `is_active=false, deactivation_reason='roaster'` still resolves its real slot name through the fallback.

**Not touched, confirmed by reading the actual code**: `services/liamSmsFeedback.ts` and `services/brewCard.ts` have no `coffees` query at all — every read in both is scoped by a `coffeeId`/`blend_id` the caller already owns (an order, a message, a card), so neither needed a predicate. `routes/sommelier.ts`'s three direct `coffees` reads are either downstream of an already-filtered `ragResult.coffeeIds` or a historical session-message replay (`GET /:sessionId/messages` deliberately keeps rendering a past session's stored coffee ids even if that coffee has since gone inactive — a customer's own transcript shouldn't relabel itself after the fact).

**Files**: `backend/src/services/sommelierRag.ts`, `backend/src/services/sommelierRag.test.ts` (new).

#### S96. Archetype rename — Liam's prompt and the quiz-name lookups say "Balanced" (2026-09-19)

`services/claude.ts`'s system-prompt archetype cheat-sheet line is now `- Balanced: caramel, honey, milk chocolate, round body` (was `Balanced & Sweet`), so Liam names the archetype "Balanced". Display-name-only: the enum code `balanced_sweet` that `sommelierRag.ts` keys its neighbour lookups on is unchanged (`FALLBACK_ADJACENCY` and `v_coffee_archetype_adjacency` are code-keyed — `balanced_sweet` still neighbours `fruity` and `chocolate_nutty`). Older quiz snapshots that still carry the old name resolve through `catalogReads.archetypeCode()`. Verified with one live `chatWithSommelier` exchange (reply says "Balanced", never the old name). Full detail: `WHAT_WE_BUILT.md` #187.

#### S97. Hoboken Crawl quiz-email audit + resync fix — confirmed no Liam impact (2026-09-24/25)

`WHAT_WE_BUILT.md` #190/#191 — a read-only audit of a subscriber-archetype drift bug, then its fix (server-verified archetype in `handleSubscribe`, no more frontend placeholder default, Mailchimp replace-not-add tags, `api_event` retention retired). Same shape as S93/S94: checked, not assumed. `newsletter.ts` is touched again (same file S94 already covers for the Resend send), but every change is additive/defensive around the *existing* `archetype`/`quizSessionKey` fields `handleSubscribe` already read — no new field Liam's own path depends on, and no Sommelier table (`sommelier_sessions`, `sommelier_messages`, `user_tokens`, `token_events`) or Firestore path touched. `quiz_funnel_event`'s new `quiz_final` event and `quiz_session`/`quiz_complete` (Liam's actual quiz-completion signal, via `sommelierEvaluator.ts`/`userSignals.ts`) are unrelated — `quiz_final` is funnel-analytics-only, nothing in the Sommelier evaluator reads `quiz_funnel_event` at all. `services/quizIntegrity.ts`'s 3 new checks (ids 9-11) are read-only diagnostics with no write path. The one live-account effect: Mailchimp `archetype:*` tags on 7 crawl subscribers will read correctly again once Part C's `--apply` runs (Dana's go pending) — Mailchimp tags are not a Sommelier input.

**Files**: none owned by this feature (confirmation only — see `WHAT_WE_BUILT.md` #190/#191 for the actual change).

#### S98. Quiz interpretation v2.1 — new Liam hint fields, and a PROFILE_AMBIGUOUS side effect to decide (2026-09-25)

`WHAT_WE_BUILT.md` #192. `POST /api/quiz/score` now returns `exploreArchetype` / `exploreReason` (the loose thread for Liam only: gate Fruity, then the treat, then a 3+ point third archetype, then a runner-up tie) and a customer-facing `pairConfidence`; nothing in the Sommelier reads them yet — wiring them into the opening context is a separate decision. **Side effect on existing behaviour**: `sommelierEvaluator.ts`'s `PROFILE_AMBIGUOUS` rule fires on `recommendationMode === 'ai_agent' || foodSignalAlignment === 'low'`. v2.1 never produces `ai_agent`, and the legacy `foodSignalAlignment` is now computed with the v2.1 secondary (a treat-led secondary reads `medium`), so for new quiz completions this intent fires mainly on `flags.quizTie`. Old stored sessions are unaffected. No Sommelier table, prompt or Firestore path changed.

**Files**: none owned by this feature (see `WHAT_WE_BUILT.md` #192).

#### S99. Quiz interpretation v2.1 · brief 2 — Liam now reads the interpretation table (2026-09-25)

`WHAT_WE_BUILT.md` #193. Resolves the S98 side effect: `PROFILE_AMBIGUOUS` in `sommelierEvaluator.ts` now goes through the exported `isProfileAmbiguous()`. On a session whose **current `quiz_session_interpretation` row is `v2.1` or later** it fires on `pairConfidence === 'low' || exploreArchetype !== null`; on any other session (`v1` rows, or no row yet: the `context_data` fallback) it keeps the old rule (`ai_agent` or `foodSignalAlignment === 'low'`); `flags.quizTie` fires it on any version. `userSignals.ts` reads the current row (whole-row precedence over `context_data`) and exposes `interpretationVersion`, `pairConfidence`, `exploreArchetype`, `exploreReason`; `secondaryArchetype`, `foodSignalAlignment` and `recommendationMode` now come from that row too when it exists. The feature vector is unchanged (13 dims). `POST /api/sommelier/start`'s opening context gains, only for a session with a table row, `Quiz pair confidence: <high|medium|low>.` and, when there is a loose thread, `Loose thread to explore (not yet asked): <archetype> (<reason>).` It is context for Liam only — never shown in the reveal or the email; no Liam prompt or intent addendum was edited, so how much Liam makes of the hint is unchanged prompt behaviour. Until the prod backfill runs, the table is empty on prod, every session is on the fallback and Liam behaves exactly as before. No Sommelier table or Firestore path changed; `sommelier.test.ts` (fully mocked) still passes.

**Files**: `backend/src/services/{sommelierEvaluator,userSignals,quizSession}.ts`, `backend/src/routes/sommelier.ts`.
